# AUDIT #2 вЂ” Security / Auth / Sandbox
...
- **File:** `src/kernel/services/compromise-webhook-service.ts`, lines 16вЂ“108
...
3. The `secret_type_display` field is used to infer a provider (line 90вЂ“102).
...
5. All chat/routing using that provider is disrupted вЂ” a denial-of-service via false compromise signals.
...
- **File:** `server/sync-server.mjs`, lines 146вЂ“153
...
- **File:** `src/kernel/services/admin-service.ts`, lines 258вЂ“277, 211вЂ“222, 230вЂ“256
...
- **File:** `src/components/ChatPanel/CodeRunner.tsx`, lines 100вЂ“246
- **Also:** `src/services/sandbox.worker.ts`, lines 206вЂ“222
...
- **File:** `src/routes.tsx`, lines 110вЂ“198
...
- **File:** `src/main.tsx`, lines 30вЂ“60
...
- **File:** `src/services/sandbox.worker.ts`, lines 200вЂ“222
...
- **File:** `src/kernel/services/mcp-service.ts`, lines 95вЂ“108
...
|---------|----------|--------|
| AES-256-GCM encryption for API keys at rest | `src/kernel/security.ts` | вњ… PBKDF2 600K iterations |
| Rate limiting on vault unlock (5 attempts + exponential backoff) | `src/kernel/security.ts:13-25` | вњ… |
| AST-based code validation in sandbox worker | `src/services/sandbox.worker.ts:14-109` | вњ… |
| SSRF protection with private IP blocking | `src/kernel/utils/network.ts` | вњ… |
| API key redaction in error messages | `src/llm/http/llm-http-client.ts:3-4,25-49` | вњ… |
| Webhook URL validation (HTTPS-only + private IP block) | `notification-webhook-service.ts:10-18` | вњ… |
| Key vault strips plaintext from memory after use | `key-vault.ts:79-88` | вњ… |
| Nginx deny-all on unknown `/proxy/*` paths | `docker/nginx.conf:128-131` | вњ… |
| Tool code validated before import/execution | `tool-executor.ts:196-200` | вњ… |
| Sync server requires `SYNC_SECRET` (fail-fast if missing) | `server/sync-server.mjs:14-19` | вњ… |
| CodeRunner iframe has `sandbox="allow-scripts"` without `allow-same-origin` | `CodeRunner.tsx:110` | вњ… |
...
|:---|:------:|:------------|
| C-01 | ? Not fixed | Webhook signature verification отсутствует |
| H-01 | ?? Mitigated | Token in URL — SYNC_SECRET required (дубль CRIT-7) |
| H-02 | ? Not fixed | AdminService destructive ops без auth |
| H-03 | ? Not fixed | CSP nginx connect-src слишком широкий |
| H-04 | ? Not fixed | CodeRunner sandbox prototype chain bypass |
| M-01 | ? Not fixed | Нет auth на client-side routes |
| M-02 | ? Not fixed | #reset wipe без confirmation |
| M-03 | ? Fixed | Key-metadata logging DEV-gated (дубль HIGH-11) |
| M-04 | ?? Partially fixed | AST check added (CRIT-6), var hoisting remains (HIGH-10) |
| M-05 | ? Not fixed | MCP service допускает localhost |
