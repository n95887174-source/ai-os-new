...
│                    Уровень сервисов                           │
│                                                              │
│  DebateService  ─── оркестрирует дебаты                       │
...
│    ├─ FeatureFlagService ─── управление возможностями в рантайме │
│    ├─ ConfigService ─── наложение конфигураций               │
│    ├─ ConsistencyChecker ─── валидация документов ↔ кода     │
│    └─ ConsistencyHealingPipeline ─── поток авто-исправления  │
└─────────────────────────────────────────────────────────────┘
```
...
### Граф зависимостей (Dependency Graph) — DebateService
...
- `DebateService.startDebate()` является основной точкой входа — валидирует входные данные, создаёт сессию, запускает вступительные заявления, затем переходит в цикл раундов
...
- `ConsistencyChecker.checkDocs()` валидирует ссылки в документации относительно манифеста кода в любой момент; `ConsistencyHealingPipeline.analyze()` объединяет обнаружение + планирование + запуск дебатов для авто-исправления
