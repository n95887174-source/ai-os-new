...
| #     | Задача                                                                                                                                                                                         | Аудиты   | Усилие   | Статус                                                 |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------- | ------------------------------------------------------ |
| P1.14 | **Разбить `debate-llm-caller.ts` (2729 строк)** — split на dispatcher + rate-backoff + token-tracker + facade                                                                                  | `[E][Z]` | M (3-5d) | ⚠️ Session 19 вынес backoffWait, но файл всё ещё 2700+ |
| P1.15 | **Разбить `memory-engine.ts` (996 строк)** — split на memory-cache + search-orchestrator + prune-scheduler + facade                                                                            | `[E]`    | M (2-3d) | ❌                                                     |
| P1.16 | **Разбить `key-service.ts` (1339 строк)** — facade уже есть, вынести логику в существующие key-management/*                                                                                    | `[E][Z]` | M (2-3d) | ❌                                                     |
| P1.17 | **Layer violation в service-registration** — `phase3-debate-runtime.ts` и `phase6-high-level.ts` импортируют adapter-фабрики из `src/stores/`. Исключение в `.dependency-cruiser.cjs` — убрать | `[A]`    | S (1d)   | ❌                                                     |
| P1.18 | **8 `@deprecated MOCK` сервисов с UI-панелями** — deploy, fine-tuning, model-distillation, health-sla, provider-migration, sleep-engine. Либо feature-flag с бейджем "Demo", либо удалить      | `[A]`    | M (2-3d) | ❌                                                     |
...
| #     | Задача                                                                                                  | Аудиты   | Усилие   | Статус |
| ----- | ------------------------------------------------------------------------------------------------------- | -------- | -------- | ------ |
| P1.20 | **Добавить streaming в live дебаты** — сейчас пользователь ждёт 30с+ без feedback                       | `[E]`    | L (1w)   | ❌     |
| P1.21 | **Маркировать cognitive-aux панели (40 шт)** — production или research? JSDoc + UI badge "Experimental" | `[E]`    | S (1d)   | ❌     |
| P1.22 | **13 `React.memo` на 644 .tsx файла** — крайне мало. Мемоизировать 10 тяжёлых list-row компонентов      | `[A]`    | M (2-3d) | ❌     |
...
| #     | Задача                                                                                     | Аудиты | Усилие  | Статус |
| ----- | ------------------------------------------------------------------------------------------ | ------ | ------- | ------ |
| P1.25 | **`npm install react-router-dom@^7.18.0 vite@latest`** — закроет 5/6 npm audit уязвимостей | `[A]`  | S (30m) | ❌     |
| P1.26 | **Удалить `build:unsafe` или переименовать в `build:skip-typecheck` с warning**            | `[A]`  | S (15m) | ❌     |
| P1.27 | **`sourcemap: 'hidden'` + upload в Sentry/Datadog**                                        | `[A]`  | S (4h)  | ❌     |
...
| ---- | -------------------------------------------------------------------------------------- | ------ | -------- |
| P2.1 | Разбить `RolesPanel/TeamWizard.tsx` (1106 строк)                                       | `[E]`  | M (2-3d) |
...
| #     | Задача                                                                                              | Аудиты   | Усилие |
| ----- | --------------------------------------------------------------------------------------------------- | -------- | ------ |
| P2.7  | Удалить AquariumPanel (881 строка, 0% unique logic) или оставить как Easter Egg с маркером          | `[A][E]` | S (2h) |
| P2.8  | 92/165 директорий компонентов (56%) содержат 1 файл — кандидаты на объединение                      | `[A]`    | L (1w) |
