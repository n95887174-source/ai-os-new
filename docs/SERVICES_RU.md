...
> SuperAgents OS v4.5.0 — 88+ DI-сервисов, 16 подсистем

## Архитектура

Все сервисы регистрируются через DI-контейнер (`resolver.ts`) и следуют lifecycle: `init()` → `start()` → `destroy()`. Коммуникация — через EventBus. Сервисы не импортируют друг друга напрямую — только через контракты (`contracts/`).
...
- **Назначение**: Управление feature flags
