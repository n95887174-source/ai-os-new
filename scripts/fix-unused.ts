...
let tscOutput = '';
try {
  tscOutput = execSync('npx tsc --noEmit -p tsconfig.app.json', { cwd: ROOT, encoding: 'utf-8' }).toString();
} catch (e: any) {
  tscOutput = e.stdout?.toString() || '';
}

// Parse errors from tsc output (line-based, format: file(line,col): error TScode: message)
interface Err { file: string; line: number; code: string; name: string; }
const errors: Err[] = [];
for (const rawLine of tscOutput.split('\n')) {
  const line = rawLine.trimEnd();
  const m = line.match(/^(.+?)\((\d+),(\d+)\):\s+error\s+(TS\d+):\s+(.+)$/);
  if (!m) continue;
  const code = m[4], msg = m[5];
  if (code === 'TS6192') {
...
for (const [filePath, fileErrors] of byFile) {
  ...
  for (const err of fileErrors) {
    ...
    if (replacement !== null) {
      ...
      console.warn(`  SKIP ${err.file}:${err.line} '${err.name}' — ${line.trim().slice(0, 80)}`);
    }
  }
...
function removeFromImportLine(trimmed: string, ws: string, name: string): string | null {
  ...
  // import X from 'y'
  // import X, { A } from 'y'
  const m = trimmed.match(/^(import\s+(?:type\s+)?)(?:(\w+)\s*,\s*)?\s*\{([^}]*)\}\s*from\s+(['"`][^'"]+['"`])/);
  if (m) {
    const prefix = m[1];
    ...
    const idx = specs.findIndex(s => {
      const clean = s.replace(/^type\s+/, '').replace(/\s+as\s+\w+$/, '').trim();
      return clean === name;
    });
    ...
    if (idx !== -1) {
      ...
      if (specs.length === 0 && defaultName) return ws + `${prefix}${defaultName} from ${fromPart}`;
      return ws + `${prefix}${defaultName ? defaultName + ', ' : ''}{ ${specs.join(', ')} } from ${fromPart}`;
    }
...
function removeFromDestructuring(line: string, name: string): string | null {
  ...
  let inner = line.slice(start + 1, end - 1);
  const items = inner.split(',').map(s => s.trim());
  const idx = items.findIndex(s => {
    const clean = s.replace(/^(\.\.\.|type\s+)/, '').replace(/\s*[:=].*$/, '').trim();
    return clean === name;
  });
...
function isStandaloneDecl(trimmed: string, name: string): boolean {
  const e = esc(name);
  if (new RegExp(`^(export\\s+)?(const|let|var)\\s+${e}(\\s*[:=!;]|\\s*$)`).test(trimmed)) return true;
  if (new RegExp(`^(export\\s+)?(async\\s+)?(function|class)\\s+${e}(\\s*[<(]|\\s*$)`).test(trimmed)) return true;
