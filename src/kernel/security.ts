...
export class SecurityService implements ISecurityService {
    ...
    private checkRateLimit(userId: string): void {
        ...
        if (record.count >= this.MAX_FAILED_ATTEMPTS) {
            ...
            const backoffMs = Math.min(
                this.BACKOFF_BASE_MS * Math.pow(2, record.count - this.MAX_FAILED_ATTEMPTS),
                300000,
            );
    ...
    private recordFailedAttempt(userId: string): void {
        ...
        if (
            this.failedAttempts.size >= this.MAX_TRACKED_USERS &&
            !this.failedAttempts.has(userId)
        ) {
        ...
        if (reEncrypt) {
            ...
            try {
                ...
                try {
                    ...
                    LOGGER.error('Security', 'Password change rollback failed', {
                        error: rollbackError,
                    });
                }
                return false;
