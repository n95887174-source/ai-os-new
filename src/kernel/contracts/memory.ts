...
export interface IToolExecutor {
  getTools(): unknown[];
  execute(toolId: string, input: unknown): Promise<{ status: string; data?: unknown; error?: string; timestamp: number; duration?: number }>;
  getExecutionHistory(toolId?: string): unknown[];
  getEnabledTools(): unknown[];
  getToolCapabilities(): { maxExecutionTime: number; maxConcurrentExecutions: number; supportedCategories: string[] };
  tryExecute?(toolId: string, input: unknown): Promise<Result<{ status: string; data?: unknown; error?: string; timestamp: number; duration?: number }, Error>>;
}
