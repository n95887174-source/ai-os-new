...
export const registerPhase1: Phase = (helpers, ctx) => {
    ...
    register(
        ...
        new SettingsService({
            ...
            get routerService() {
                return get<SettingsServiceDeps['routerService']>('routerService');
            },
            get kernel() {
                return get<SettingsServiceDeps['kernel']>('kernel');
            },
        }),
    );
    ...
    register(
        'keyStateStore',
        new KeyStateStore(get<IEventBus>('eventBus'), get<IDatabaseService>('database')),
    );
    ...
    const unsubCb = eventBusForSync.onSafe<{ provider: string; status: string }>(
        EVENTS.PROVIDER_CIRCUIT_BREAKER_SYNCED,
        (payload) => {
            registryForSync.syncCircuitBreakerState(payload.provider, payload.status);
        },
    );
    const unsubRl = eventBusForSync.onSafe<{ provider: string; remaining: number }>(
    ...
    register(
        ...
        new KeyService(
            asDeps<ConstructorParameters<typeof KeyService>[0]>({
                ...
                keyStateStore: get<KeyStateStore>('keyStateStore'),
                get advisorService() {
                    return _container.get<AdvisorService>('advisorService');
                },
            }),
        ),
    );
