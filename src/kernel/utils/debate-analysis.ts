...
const FALLACY_RULES: FallacyRule[] = [
  {
    ...
    heuristic: (t) => /\b(you|your)\b/i.test(t) && /\b(always|never|just|simply)\b/i.test(t) && !/\b(because|since|therefore|thus)\b/i.test(t),
  },
  {
    type: 'straw_man',
  ...
  },
  {
    ...
    severity: 'medium',
    patterns: [
      /\bno (true|real|genuine) .* would\b/i,
      /\breal (.*) don\u2019t\b/i,
    ],
  },
  {
    ...
    severity: 'medium',
    patterns: [
      /\byou (do|the same|too|as well)\b/i,
...
export function computePersuasion(args: ArgumentLite[]): PersuasionResult {
  ...
  const overallShift = byAgent.length === 0
    ? 0
...
const NEGATIVE_LEXICON = new Set([
  'bad', 'terrible', 'awful', 'horrible', 'hate', 'worst', 'sad', 'fail', 'failure', 'ugly', 'wrong', 'negative', 'no', 'never', 'cannot', 'impossible', 'reject', 'against', 'weak', 'poor', 'harmful', 'dangerous', 'stupid', 'idiotic',
  'плохо', 'ужасно', 'кошмар', 'ненавижу', 'худший', 'грустно', 'провал', 'неудача', 'уродливый', 'неправильно', 'негативный', 'нет', 'никогда', 'невозможно', 'отвергаю', 'против', 'слабый', 'бедный', 'вредный', 'опасный', 'глупый',
]);

const INTENSIFIERS = new Set(['very', 'extremely', 'absolutely', 'totally', 'completely', 'utterly', 'highly', 'really', 'so', 'too',
  'очень', 'крайне', 'абсолютно', 'совершенно', 'совсем', 'чрезвычайно', 'сильно', 'реально', 'так']);
...
export function analyzeTone(args: { round: number; content: string; confidence?: number }[]): ToneAnalysis {
  ...
  const avg = sentiments.length === 0 ? 0 : sentiments.reduce((s, x) => s + x, 0) / sentiments.length;
  let variance = 0;
  ...
  if (sentiments.length >= 2) {
    ...
    const firstAvg = firstHalf.length === 0 ? 0 : firstHalf.reduce((s, x) => s + x, 0) / firstHalf.length;
    const secondAvg = secondHalf.length === 0 ? 0 : secondHalf.reduce((s, x) => s + x, 0) / secondHalf.length;
...
export interface DebateAnalysis {
  fallacyStats: { type: string; count: number; severity: FallacyMatch['severity']; description: string }[];
...
export function analyzeDebate(args: { agentId: string; content: string; confidence: number; round: number; parentId?: string }[]): DebateAnalysis {
  const fallacyMap = new Map<string, { type: string; count: number; severity: FallacyMatch['severity']; description: string }>();
  ...
  for (const a of args) {
    ...
    for (const f of found) {
      ...
      if (!fallacyMap.has(key)) {
        fallacyMap.set(key, { type: key, count: 0, severity: f.severity, description: f.description });
      }
      fallacyMap.get(key)!.count++;
    }
    if (found.length > 0) {
      fallacyScoreByAgent[a.agentId] = (fallacyScoreByAgent[a.agentId] ?? 0) + fallacyScore(found);
    }
  }
