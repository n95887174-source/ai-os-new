...
export interface IKernel {
  ...
  setBaseWeights(weights: { ttft: number; tps: number; reliability: number }, tx?: ITransaction): void;
...
export interface IRouterService {
  init(): Promise<void>;
  destroy(): void;
  getRankedProviders(strategy: string, prompt: string, priority?: string, agentId?: string, probeResults?: Map<string, unknown>, overrideState?: SystemState, suppressEmit?: boolean, origin?: string, sessionId?: string): ApiKey[];
  ...
  getActiveProfile(): { name: string; ttft: number; tps: number; reliability: number; description?: string };
  ...
  classifyRequest(prompt: string): { complexity: string; isCode: boolean; isLong: boolean; isMultimodal: boolean; intent: string; language: string };
