...
import type { IProviderAdapter, IAdapterRegistry, ProviderRuntimeStatus } from '../contracts/provider-adapter';
...
describe('LLMClientService', () => {
  ...
  it('should use default provider and model', async () => {
    ...
    expect(adapter.sendMessage).toHaveBeenCalledWith(
      [{ role: 'user', content: 'hi' }], 'gpt-4', 'sk-test', undefined, undefined,
    );
  });
  ...
  it('should pass explicit provider and model', async () => {
    ...
    expect(adapter.sendMessage).toHaveBeenCalledWith(
      [{ role: 'user', content: 'hi' }], 'gemini-pro', 'sk-test', undefined, undefined,
    );
  });
  ...
  it('should use apiKeyOverride when provided', async () => {
    ...
    await client.chat([{ role: 'user', content: 'hi' }], { provider: 'openai', apiKeyOverride: 'sk-override' });
    expect(adapter.sendMessage).toHaveBeenCalledWith(
      expect.any(Array), 'auto', 'sk-override', undefined, undefined,
    );
  });
  ...
  it('should pass temperature and maxTokens as adapterOptions', async () => {
    ...
    expect(adapter.sendMessage).toHaveBeenCalledWith(
      expect.any(Array), 'auto', 'sk-test', undefined,
  ...
  it('should forward tool role messages to adapter', async () => {
    ...
    await client.chat([
      ...
      { role: 'assistant', content: '', toolCalls: [{ id: '1', type: 'function', function: { name: 'get_weather', arguments: '{}' } }] },
    ...
    expect(adapter.sendMessage).toHaveBeenCalledWith(
      ...
      'auto', 'sk-test', undefined, undefined,
    );
  });
  ...
  describe('sendMessage alias', () => {
    it('should return simplified response shape', async () => {
      ...
      const registry = mockRegistry(adapter);
      const client = new LLMClientService({ resolveApiKey: () => 'sk-test' }, registry);
      const result = await client.sendMessage([{ role: 'user', content: 'hi' }], { provider: 'openai' });
      expect(result.content).toBe('hello');
