...
export class ResearchScheduler implements ILifecycle {
    ...
    update(
        id: string,
        patch: Partial<Pick<ScheduledResearch, 'cronExpression' | 'params' | 'enabled'>>,
    ): boolean {
    ...
    private tick(): void {
        ...
        for (const s of this.schedules) {
            ...
            this.onRun?.(s.module, s.params).catch((e) =>
                LOGGER.warn('ResearchScheduler', 'Run failed', { error: e }),
            );
        }
        this.persist();
    }
    ...
    private persist(): void {
        try {
            this.storage?.setItem(STORAGE_KEY, JSON.stringify(this.schedules));
        } catch {
            /* full */
        }
    }
