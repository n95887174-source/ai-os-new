...
function mockAdapter(overrides?: Partial<IProviderAdapter>): IProviderAdapter {
  return {
    id: 'mock',
    sendMessage: vi.fn().mockResolvedValue({ content: 'E2E response', latency: 50, tokens: 20 }),
...
describe('Provider Stack E2E', () => {
  ...
  it('should support streaming through LLM client via adapter registry', async () => {
    ...
    const onChunk = vi.fn();
    const result = await streamClient.chat(
  ...
  it('should emit and record events through the full stack', async () => {
    ...
    const sessionSnap = eventSourcing.replay.snapshot();
    expect(sessionSnap).toBeDefined();
    expect(sessionSnap.status).toBe('idle');
  });
});
