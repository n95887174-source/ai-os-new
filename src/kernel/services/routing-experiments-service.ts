...
export class RoutingExperimentsService implements IRoutingExperimentsService {
  ...
  computeComparison(results: RoutingExperimentResult[]): StrategyComparison[] {
    ...
    return Object.entries(grouped).map(([strategy, items]) => ({
      ...
      avgCost: Math.round((items.reduce((sum, r) => sum + r.cost, 0) / items.length) * 1000) / 1000,
      avgErrorRate: Math.round((items.reduce((sum, r) => sum + r.errorRate, 0) / items.length) * 100) / 100,
      avgUniqueness: Math.round(items.reduce((sum, r) => sum + r.uniqueness, 0) / items.length),
    }));
  }
