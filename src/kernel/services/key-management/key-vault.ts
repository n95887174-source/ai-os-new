...
export class KeyVault implements IKeyVaultService {
    ...
    async unlock(password: string): Promise<boolean> {
        ...
        if (ok) {
            ...
            eventBus.emit(EVENTS.NOTIFICATION, {
                message: 'Key vault unlock failed — invalid password',
                type: 'error',
            });
        }
        return ok;
    ...
    async encryptAllKeys(keys: ApiKey[]): Promise<ApiKey[]> {
        // 🔴 SECURITY: When vault is locked, strip plaintext instead of returning
        // keys as-is. Prevents plaintext API keys from being persisted to IndexedDB
        // via saveKeys() → doSaveKeysWithSnapshot() when vault is locked.
        if (this.isLocked()) return this.stripPlaintextKeys(keys.map((k) => ({ ...k })));
        return Promise.all(
            keys.map(async (k) => {
    ...
    purgeKey(key: ApiKey): void {
        ...
        eventBus.emit(EVENTS.NOTIFICATION, {
            message: `Key purged from vault: ${key.id}`,
            type: 'warning',
        });
    }
    ...
    purgeAll(keys: ApiKey[]): void {
        ...
        eventBus.emit(EVENTS.NOTIFICATION, {
            message: `All keys purged from vault (${keys.length})`,
            type: 'warning',
        });
    }
}
