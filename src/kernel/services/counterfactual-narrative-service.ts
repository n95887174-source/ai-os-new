...
import type { ICounterfactualNarrativeService, NarrativeExplanation } from '../contracts/counterfactual-narrative';
...
function describeDelta(provider: string, component: string, delta: number): string {
  ...
  const magnitude = Math.abs(delta) > 0.15 ? 'significantly ' : Math.abs(delta) > 0.05 ? 'moderately ' : 'slightly ';
...
export class CounterfactualNarrativeService implements ICounterfactualNarrativeService {
  generate(explanation: DecisionExplanation): NarrativeExplanation {
    ...
    if (helpingSimWinner.length > 0) {
      ...
      chain.push(`Amplifier: ${simulatedWinner} gained (${label(amp.component)} +${amp.contribution.toFixed(3)})`);
    }
    ...
    if (hurtingOriginal.length > 0) {
      ...
      chain.push(`Downside: ${originalWinner} weakened (${label(hurt.component)} ${hurt.contribution.toFixed(3)})`);
    }

    // Margin interpretation
    const marginLabel = classifyMarginShift(marginShift);
    chain.push(`Margin shift ${marginShift > 0 ? '+' : ''}${marginShift.toFixed(3)} — ${marginLabel} change`);

    // Summary sentence
    const primaryProvider = primary?.provider ?? originalWinner;
    const primaryComponent = primary ? label(primary.component) : 'score';
    const summary = marginLabel === 'decisive'
    ...
    const confidence = marginAbs > 0.2 ? 0.9 : marginAbs > 0.1 ? 0.7 : marginAbs > 0.03 ? 0.5 : 0.3;
