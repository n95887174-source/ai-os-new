...
const SEED_HYPOTHESES: ResearchHypothesis[] = [
  {
    ...
    description: 'The debate-service.ts file exceeds 1400 lines. Splitting into orchestration, parser, metrics, and state modules would improve maintainability.',
    category: 'arch',
    status: 'accepted',
  ...
  },
  {
    ...
    description: 'Argument strategies are injected at multiple levels across agents. A single strategy registry would reduce duplication.',
    category: 'prompt',
    status: 'proposed',
  ...
  },
  {
    ...
    description: 'Router weights do not account for open circuits. Adding breaker state as a multiplier would avoid wasted requests.',
    category: 'routing',
    status: 'debating',
...
export class HypothesisService implements IHypothesisService {
  ...
  async update(id: string, patch: Partial<ResearchHypothesis>): Promise<ResearchHypothesis | null> {
