...
class ProviderCatalogService {
    ...
    try {
      ...
      if (response.ok && entry.autoDetected) {
        ...
        if (data.data && Array.isArray(data.data)) {
          entry.models = data.data.slice(0, 20).map((m: { id?: string }) => m.id || 'unknown');
        }
      }
   ...
   * Add custom provider
   */
  async addCustom(entry: Omit<ProviderCatalogEntry, 'autoDetected' | 'lastChecked' | 'status'>): Promise<void> {
  ...
  async detectLocalProviders(): Promise<void> {
    ...
    for (const port of localPorts) {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          const models = data.data?.slice(0, 10).map((m: { id?: string }) => m.id || 'unknown') || [];
          ...
          LOGGER.info('ProviderCatalog', 'Local provider detected', { id: port.id, models: models.length });
        }
      } catch {
        // Port not available
      }
