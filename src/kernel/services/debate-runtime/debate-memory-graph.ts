...
export class DebateMemoryGraph {
  ...
  getEvolution(): Array<{ round: number; nodeCount: number; edgeCount: number; newIdeas: string[] }> {
    const rounds = new Map<number, { nodeCount: number; edgeCount: number; newIdeas: string[] }>();
    for (const node of this.graph.nodes) {
      const r = node.round;
  ...
  private semanticOverlap(a: string, b: string): number {
    const wordsA = new Set(a.toLowerCase().replace(/[^a-zа-яё\s]/g, '').split(/\s+/).filter(w => w.length > 3));
    const wordsB = new Set(b.toLowerCase().replace(/[^a-zа-яё\s]/g, '').split(/\s+/).filter(w => w.length > 3));
  ...
  private inferRelation(a: DebateArgument, b: DebateArgument, overlap: number): EdgeRelation | null {
