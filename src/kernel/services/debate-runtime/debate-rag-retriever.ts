...
export class DebateRAGRetriever {
  ...
  async retrieveRelevantDebates(
    ...
    const sameDebateResults = await this.deps.embeddingPipeline.searchSimilar(query, topK, currentSessionId);
    ...
    if (results.length < topK) {
      const crossDebateResults = await this.deps.embeddingPipeline.searchSimilar(query, topK * 2);
  ...
  async injectMemoryIntoDebate(
    ...
    const memoryBlock = context.chunks
      .map((r, i) => {
        const source = r.source === 'same_debate' ? 'this debate' : `debate "${r.chunk.sessionId}"`;
