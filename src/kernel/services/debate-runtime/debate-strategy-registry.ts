...
const BUILTIN_STRATEGIES: StrategyDefinition[] = [
  ...
  },
  {
    ...
    description: 'Question-driven inquiry that probes assumptions through iterative questioning',
    version: '1.0.0',
  ...
  },
  {
    ...
    parameters: [
      { name: 'constraint', type: 'enum', default: 'facts_only', options: ['facts_only', 'data_driven', 'ethical_framework', 'first_principles'], description: 'Constraint applied to all arguments' },
    ],
  },
];
...
function validatePrimitive(primitive: StrategyPrimitive, path: string): ValidationError[] {
  ...
  if (!primitive.id || typeof primitive.id !== 'string') {
    errors.push({ path: `${path}.id`, message: 'Primitive must have a string id', code: 'MISSING_ID' });
  }
  ...
  switch (primitive.type) {
    case 'sequence': {
      ...
      if (!Array.isArray(seq.steps) || seq.steps.length === 0) {
        errors.push({ path: `${path}.steps`, message: 'Sequence must have at least one step', code: 'EMPTY_SEQUENCE' });
      } else {
        seq.steps.forEach((step, i) => {
          if (!step.stepId) {
            errors.push({ path: `${path}.steps[${i}].stepId`, message: 'Step must have a stepId', code: 'MISSING_STEP_ID' });
          }
          if (!step.primitive) {
            errors.push({ path: `${path}.steps[${i}].primitive`, message: 'Step must have a primitive', code: 'MISSING_PRIMITIVE' });
          } else {
            errors.push(...validatePrimitive(step.primitive, `${path}.steps[${i}].primitive`));
          }
        });
      }
      break;
    ...
    case 'debate_graph': {
      ...
      if (!Array.isArray(graph.agents) || graph.agents.length < 2) {
        errors.push({ path: `${path}.agents`, message: 'Debate graph must have at least 2 agents', code: 'INSUFFICIENT_AGENTS' });
      }
      ...
      if (graph.maxRounds !== undefined && graph.maxRounds < 1) {
        errors.push({ path: `${path}.maxRounds`, message: 'maxRounds must be >= 1', code: 'INVALID_ROUNDS' });
      }
      if (graph.convergenceThreshold !== undefined && (graph.convergenceThreshold < 0 || graph.convergenceThreshold > 1)) {
        errors.push({ path: `${path}.convergenceThreshold`, message: 'convergenceThreshold must be 0-1', code: 'INVALID_THRESHOLD' });
      }
      break;
    }
    case 'critic_loop': {
      ...
      if (!loop.proponent) {
        errors.push({ path: `${path}.proponent`, message: 'Critic loop must have a proponent', code: 'MISSING_PROPONENT' });
      }
      if (!loop.critic) {
        errors.push({ path: `${path}.critic`, message: 'Critic loop must have a critic', code: 'MISSING_CRITIC' });
      }
      if (loop.maxIterations < 1 || loop.maxIterations > 20) {
        errors.push({ path: `${path}.maxIterations`, message: 'maxIterations must be 1-20', code: 'INVALID_ITERATIONS' });
      }
      break;
    }
    case 'voting': {
      ...
      if (!Array.isArray(vote.voters) || vote.voters.length < 2) {
        errors.push({ path: `${path}.voters`, message: 'Voting must have at least 2 voters', code: 'INSUFFICIENT_VOTERS' });
      }
      const validMechanisms = ['simple_majority', 'supermajority', 'unanimous', 'ranked_choice', 'weighted'];
      if (!validMechanisms.includes(vote.mechanism)) {
        errors.push({ path: `${path}.mechanism`, message: `Invalid mechanism: ${vote.mechanism}`, code: 'INVALID_MECHANISM' });
      }
      break;
    }
    case 'peer_review': {
      ...
      if (!Array.isArray(pr.authors) || pr.authors.length === 0) {
        errors.push({ path: `${path}.authors`, message: 'Peer review must have at least 1 author', code: 'MISSING_AUTHORS' });
      }
      if (!Array.isArray(pr.reviewers) || pr.reviewers.length === 0) {
        errors.push({ path: `${path}.reviewers`, message: 'Peer review must have at least 1 reviewer', code: 'MISSING_REVIEWERS' });
      }
      if (!Array.isArray(pr.criteria) || pr.criteria.length === 0) {
        errors.push({ path: `${path}.criteria`, message: 'Peer review must have at least 1 criterion', code: 'MISSING_CRITERIA' });
      }
      break;
    }
  }
...
function validateCompatibility(a: StrategyDefinition, b: StrategyDefinition): Incompatibility[] {
  ...
  if (a.compatibility?.incompatibleWith?.includes(b.id)) {
    conflicts.push({ primitiveA: a.id, primitiveB: b.id, reason: `${a.name} is explicitly incompatible with ${b.name}`, severity: 'error' });
  }
  if (b.compatibility?.incompatibleWith?.includes(a.id)) {
    conflicts.push({ primitiveA: b.id, primitiveB: a.id, reason: `${b.name} is explicitly incompatible with ${a.name}`, severity: 'error' });
  }
  ...
  const getVotingPrimitives = (def: StrategyDefinition): VotingPrimitive[] => {
    ...
    const walk = (p: StrategyPrimitive) => {
      ...
      if (p.type === 'sequence') (p as SequencePrimitive).steps.forEach(s => walk(s.primitive));
    };
    walk(def.root);
    return result;
  };
...
export class StrategyRegistry implements IStrategyRegistry {
  ...
  validate(definition: StrategyDefinition): ValidationResult {
    ...
    if (!definition.id) errors.push({ path: 'id', message: 'Strategy must have an id', code: 'MISSING_ID' });
    if (!definition.name) errors.push({ path: 'name', message: 'Strategy must have a name', code: 'MISSING_NAME' });
    if (!definition.root) errors.push({ path: 'root', message: 'Strategy must have a root primitive', code: 'MISSING_ROOT' });
    ...
    const checkCycle = (p: StrategyPrimitive): void => {
      if (seen.has(p.id)) {
        errors.push({ path: `root.${p.id}`, message: `Cycle detected: ${p.id} appears twice`, code: 'CYCLE_DETECTED' });
        return;
      }
      seen.add(p.id);
      if (p.type === 'sequence') {
    ...
    definition.parameters?.forEach((param, i) => {
      if (!param.name) errors.push({ path: `parameters[${i}].name`, message: 'Parameter must have a name', code: 'MISSING_PARAM_NAME' });
      if (param.type === 'number' && param.min !== undefined && param.max !== undefined && param.min > param.max) {
        errors.push({ path: `parameters[${i}]`, message: 'min > max', code: 'INVALID_RANGE' });
      }
    });
  ...
  importJson(json: string): { success: boolean; definition?: StrategyDefinition; errors?: ValidationError[] } {
    try {
      const def = JSON.parse(json) as StrategyDefinition;
      ...
      return { success: false, errors: [{ path: 'json', message: 'Invalid JSON', code: 'PARSE_ERROR' }] };
    }
  }
  ...
  describePrimitive(type: StrategyPrimitiveType): string {
    const descriptions: Record<StrategyPrimitiveType, string> = {
      ...
      debate_graph: 'Multi-agent interaction with defined edges (sequential, challenge, refine, broadcast).',
      critic_loop: 'Iterative refinement between a proponent and critic until convergence or max iterations.',
