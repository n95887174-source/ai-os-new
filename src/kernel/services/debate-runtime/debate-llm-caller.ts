...
import type { IBoPTrackerService, UnmetBurden } from '../../contracts/debate-bop';
import type { IConsistencyService, Contradiction } from '../../contracts/debate-consistency';
import type { SourceCredibility, ICredibilityScorer } from '../../contracts/debate-credibility';
...
const sessionRToMMap = new Map<string, import('../../contracts/debate-rtom').IRToMGraphService>();

// Module-level strategy fingerprint service instances keyed by sessionId
const sessionFingerprintMap = new Map<
    string,
...
const sessionCausalGraphMap = new Map<
    string,
    ...
    try {
        while (retries < getMaxRetries()) {
            ...
            try {
                ...
                let anchors:
                    import('../../contracts/debate-entanglement').AnchorClaim[] | undefined;
                if (
                ...
                let vulnerabilityTargets:
                    | import('../../contracts/debate-vulnerability').VulnerabilityTarget[]
                    | undefined;
                if (
                    ...
                    try {
                        ...
                        if (opponentClaims.length > 50) {
                            ...
                            if (adversarialWarnings && adversarialWarnings.length > 0) {
                                LOGGER.debug(
                                    'DebateLlmCaller',
                                    'Adversarial source warnings found',
                                    {
                ...
                if (
                    ...
                    try {
                        ...
                        beliefConflicts = deps.beliefMiningService.mineConflicts(
                            miningInput,
                            session.round,
                        );
                ...
                if (
                    ...
                    try {
                        minimaxMove = deps.minimaxPlanner.plan(
                            participant.agentId,
                            currentName,
                            session.round,
                        );
                ...
                let tacticalDirective:
                    | import('../../contracts/debate-meta-agent').TacticalDirective
                    | null
                    | undefined;
                if (
                    isQ('meta-agent') &&
                ...
                if (isQ('steelman') && deps.steelmanService && previousArguments.length >= 2) {
                    try {
                        ...
                        steelmanTarget = deps.steelmanService.selectTarget(
                            participant.agentId,
                            miningInput,
                        );
                        if (steelmanTarget) {
                ...
                if (isQ('stance-drift') && deps.driftDetector) {
                    try {
                        ...
                        const prevDrift = deps.driftDetector.getDrift(
                            participant.agentId,
                            session.round - 1,
                        );
                ...
                if (isQ('replay') && deps.replaySelector && session.round > 1) {
                    try {
                        ...
                        replayText = deps.replaySelector.getFormattedReplay(
                            session.round,
                            session.language,
                        );
                    } catch {
                ...
                if (isQ('enthymeme') && deps.logicalFormExtractor && session.round > 1) {
                    try {
                        const prevArgs = previousArguments.filter(
                            (a) => a.round === session.round - 1,
                        );
                        for (const a of prevArgs) {
                            deps.logicalFormExtractor.analyzeArgument(
                                a.agentId,
                                a.round,
                                a.content,
                            );
                ...
                if (isQ('frame') && deps.frameTracker && session.round > 1) {
                    try {
                        ...
                        for (const a of prevRound) {
                            deps.frameTracker.registerFrame(
                                a.agentId,
                                a.agentName,
                                a.round,
                                a.content,
                ...
                if (isQ('role-reversal') && session.round >= 3 && session.round % 4 === 0) {
                    ...
                    if (opponents.length > 0) {
                        const target =
                            opponents[
                                Math.abs(participant.agentId.charCodeAt(0)) % opponents.length
                            ];
                ...
                const evidenceRevelationRound = isQ('evidence-revelation')
                    ? session.round
                    : undefined;
                ...
                if (isQ('audience') && session.round >= 2 && previousArguments.length >= 3) {
                    ...
                    for (const a of lastArgs) {
                        ...
                        const hasEvidence =
                            /\b(study|research|data|evidence|according to|statistics?)\b/.test(
                                lower,
                            );
                        if (hasEvidence) {
                        ...
                        const isWeak =
                            /\b(maybe|perhaps|i think|not sure|possibly|might be wrong)\b/.test(
                                lower,
                            );
                        if (isWeak) {
                        ...
                        const isWitty = /\b(funny|humor|joke|ridiculous|absurd|laugh)\b/.test(
                            lower,
                        );
                        if (isWitty) laughs++;
                        const isAggressive =
                            /\b(attack|destroy|stupid|idiot|wrong|nonsense)\b/.test(lower);
                        if (isAggressive) {
