        ...
        try {
            ...
            if (
                ...
                try {
                    ...
                    LOGGER.warn('DebateLlmCaller', 'Adversarial source check error', { sessionId });
                }
            }
            ...
            if (isQ('causal-graph') && session.round >= 1) {
                try {
                    ...
                    LOGGER.warn('DebateLlmCaller', 'Causal graph analysis error', { sessionId });
                }
            }
            ...
            if (isQ('audience') && session.round >= 2 && previousArguments.length >= 3) {
                ...
                for (const a of lastArgs) {
                    ...
                    const hasEvidence =
                        /\b(study|research|data|evidence|according to|statistics?)\b/.test(lower);
                    if (hasEvidence) {
                    ...
                    const isWeak =
                        /\b(maybe|perhaps|i think|not sure|possibly|might be wrong)\b/.test(lower);
                    if (isWeak) {
                    ...
                    if (isWitty) laughs++;
                    const isAggressive = /\b(attack|destroy|stupid|idiot|wrong|nonsense)\b/.test(
                        lower,
                    );
                    if (isAggressive) {
            ...
            if (isQ('shadow-opponent') && deps.shadowOpponent) {
                try {
                    ...
                    if (
                        ...
                        LOGGER.debug('DebateLlmCaller', 'Shadow opponent strengthened argument', {
                            agentId: participant.agentId,
