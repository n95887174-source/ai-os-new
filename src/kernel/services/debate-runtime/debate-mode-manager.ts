...
export class DebateModeManagerPersistent extends DebateModeManager {
  ...
  async importMode(json: string): Promise<{ success: boolean; mode?: DebateMode; error?: string }> {
    try {
  ...
  private async persistModesIndex(): Promise<void> {
    if (!this.storage) return;
    try {
      const modes = this.list().filter(p => !p.builtin).map(p => p.mode.id);
  ...
  private async loadVersionHistories(): Promise<void> {
    ...
    try {
      ...
      for (const preset of modes) {
        const history = await this.storage.config.get<ModeVersion[]>(`${MODE_VERSION_PREFIX}${preset.mode.id}`);
        if (history) {
