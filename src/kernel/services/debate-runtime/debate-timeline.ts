...
import type { TimelineEntry, IDebateTimeline, ReasoningTrace } from '../../contracts/debate-runtime';
...
export class DebateTimeline implements IDebateTimeline {
  ...
  private extractUncertainties(content: string): Record<string, number> {
    ...
    const phrases = [
      ['возможно', 0.6], ['вероятно', 0.7], ['не уверен', 0.4], ['может быть', 0.5],
      ['предположительно', 0.5], ['perhaps', 0.6], ['possibly', 0.6], ['maybe', 0.5],
      ['uncertain', 0.4], ['likely', 0.7], ['probably', 0.7],
    ] as const;
