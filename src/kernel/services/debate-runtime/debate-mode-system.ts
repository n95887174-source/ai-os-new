import type { DebateMode, DebateModePreset, PolicyType, ModePolicy } from '../../contracts/debate-mode-system';
...
const BUILTIN_MODES: DebateMode[] = [
  {
    ...
    policies: [
      ...
      { type: 'temperature_cap', value: 0.3, description: 'Low temperature for deterministic output' },
      { type: 'contradiction_penalty', value: 0.2, description: 'Contradictions reduce score by 20%' },
    ],
  ...
  },
  {
    ...
    description: 'Peer review process with structured evaluation criteria. Authors vs Reviewers.',
    icon: 'microscope',
    color: '#8b5cf6',
  ...
  },
  {
    id: 'brainstorming',
    name: 'Brainstorming',
    description: 'Open creative exploration. All ideas welcome. High diversity, low constraint.',
    icon: 'lightbulb',
    color: '#f59e0b',
    constraint: 'none',
    policies: [
      { type: 'max_rounds', value: 8, description: 'Extended rounds for idea generation' },
      { type: 'temperature_cap', value: 0.9, description: 'High temperature for creative output' },
      { type: 'require_evidence', value: false, description: 'Evidence not required for brainstorming' },
  ...
  },
  {
    id: 'jury_trial',
    name: 'Jury Trial',
    description: 'Prosecution vs Defense with jury voting. Adversarial format with structured verdict.',
    icon: 'gavel',
    color: '#ef4444',
  ...
  },
  {
    ...
    policies: [
      ...
      { type: 'temperature_cap', value: 0.5, description: 'Moderate temperature for focused inquiry' },
  ...
  },
  {
    ...
    policies: [
      ...
      { type: 'require_evidence', value: true, description: 'Concrete attack vectors required' },
  ...
  },
  {
    ...
    policies: [
      ...
      { type: 'auto_summarize', value: true, description: 'Auto-generate consensus statement' },
      { type: 'contradiction_penalty', value: 0.1, description: 'Mild penalty for contradictions' },
  ...
  },
  {
    ...
    policies: [
      ...
      { type: 'temperature_cap', value: 0.8, description: 'High temperature for diverse views' },
...
export class DebateModeManager {
  ...
  listBuiltin(): DebateMode[] {
    return this.list().filter(p => p.builtin).map(p => p.mode);
  }
  ...
  importJson(json: string): { success: boolean; mode?: DebateMode; error?: string } {
    try {
      const mode = JSON.parse(json) as DebateMode;
  ...
  getPolicyValue<T extends number | boolean | string>(modeId: string, policyType: PolicyType, fallback: T): T {
  ...
  resolveStrategy(modeId: string, registry: { get(id: string): StrategyDefinition | undefined }): StrategyDefinition | undefined {
    const mode = this.get(modeId);
