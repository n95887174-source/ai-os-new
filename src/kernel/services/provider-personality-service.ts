...
class ProviderPersonalityService {
    ...
    async init(): Promise<void> {
        const saved = await (
            await this.db()
        ).getKv<[string, ProviderPersonality][]>('personalities');
        if (saved) {
    ...
    async updateTrait(
        ...
        trait: keyof Omit<
            ProviderPersonality,
            'provider' | 'calibrationSamples' | 'lastCalibrated'
        >,
        value: number,
    ): Promise<void> {
        ...
        (personality as unknown as Record<string, unknown>)[trait] = Math.max(
            0,
            Math.min(1, value),
        );
    ...
    async calibrate(
        ...
        LOGGER.info('ProviderPersonality', 'Provider calibrated', {
            provider,
            samples: samples.length,
        });
        EventBus.emit(EVENTS.PROVIDER_PERSONALITY_CALIBRATED, {
            provider,
            samples: samples.length,
        });
    }
    ...
    private async save(): Promise<void> {
        await (await this.db()).setKv('personalities', Array.from(this.personalities.entries()));
    }
}
