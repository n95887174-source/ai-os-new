...
export class BudgetService implements IBudgetService {
  ...
  private async loadBudget() {
    ...
    } catch (e) { LOGGER.warn('BudgetService', 'Failed to load budget', { error: e }); }
  }

  private async saveBudget() {
    try {
      await this.deps.database.setKv(BUDGET_KEY, { monthlyBudget: this.monthlyBudget });
    } catch (e) { LOGGER.warn('BudgetService', 'Failed to save budget', { error: e }); }
  }
  ...
  private async saveProviderBudgets() {
    ...
    } catch (e) { LOGGER.warn('BudgetService', 'Failed to save provider budgets', { error: e }); }
  }
  ...
  private async loadHistory() {
    ...
    } catch (e) { LOGGER.warn('BudgetService', 'Failed to load cost history', { error: e }); }
  }

  private async saveHistory() {
    try {
      await this.deps.database.setKv(COST_HISTORY_KEY, this.costHistory);
    } catch (e) { LOGGER.warn('BudgetService', 'Failed to save cost history', { error: e }); }
  }
  ...
  private computeCurrentSpend(): number {
    ...
    return this.costHistory.filter(c => c.timestamp >= startOfMonth).reduce((s, c) => s + c.totalCost, 0);
  }
  ...
  getSpendSummary(): SpendSummary {
    const global = {
      ...
      pct: this.monthlyBudget > 0 ? Math.round((this.computeCurrentSpend() / this.monthlyBudget) * 100) : 0,
    };
