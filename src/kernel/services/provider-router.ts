...
export class RouterService {
    ...
    private checkLatencyHealth() {
        ...
        const degraded = sorted.filter(
            (e) => e.avg > median * this.config.latency.degradationRatio && e.avg > 0,
        );
        ...
        for (const link of chain) {
            if (excludedSet.has(link.provider.toLowerCase())) {
                ...
                skipped.push({
                    ...
                    keyLabel: link.provider,
                    keyId: undefined,
                    reason: 'Excluded (same as failed provider) — no fallback key available',
                    stage: 'exclusion',
                });
                continue;
            }
    ...
    getRankedProviders(
        ...
        if (rankedItems.length > 0) {
            ...
            if (decisionOrigin === 'live') {
                ...
                if (this.decisionHistory.length > this.config.history.maxDecisions)
                    this.decisionHistory.pop();
            } else {
                this.simulationHistory.unshift(decision);
                if (this.simulationHistory.length > this.config.history.maxDecisions)
