...
export class RoleVersionService {
  ...
  init(): void {
    try {
      ...
      if (stored) {
        const parsed = JSON.parse(stored) as RoleVersion[];
  ...
  diff(roleId: string, versionIdA: string, versionIdB: string): RoleDiff | undefined {
    ...
    for (const key of ['name', 'description', 'systemPrompt', 'baseTemperature'] as const) {
      if (aCfg[key] !== bCfg[key]) {
        changes.push({ field: key, oldValue: String(aCfg[key]), newValue: String(bCfg[key]) });
      }
    }
  ...
  private persist(): void {
    ...
    try { this.storage?.setItem(STORAGE_KEY, JSON.stringify(all)); } catch { /* full */ }
  }
}
