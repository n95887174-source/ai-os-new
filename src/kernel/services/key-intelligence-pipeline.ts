import type { IKeyIntelligencePipeline, KeyIntelligenceInput, KeyImportReport, ParsedKeyResult, KeyRiskAssessment, RiskFactor, AccountGroup } from '../contracts/key-intelligence';
import { KeyFingerprints } from './key-management/key-fingerprints';
import type { ApiKey } from '../types/metrics-types';

export interface AdapterHealthCheck {
  (provider: string, apiKey: string): Promise<{ valid: boolean; latency: number; models: string[]; error?: string }>;
}
...
export class KeyIntelligencePipeline implements IKeyIntelligencePipeline {
  ...
  async run(input: KeyIntelligenceInput): Promise<KeyImportReport> {
    ...
    for (const raw of rawKeys) {
      ...
      const accountId = provider ? this.deps.fingerprints.extractAccountId(provider, raw) : undefined;
      const accountLabel = provider ? this.deps.fingerprints.extractAccountLabel(provider, raw) : undefined;
      ...
      const parsedEntry: ParsedKeyResult = {
        ...
        fingerprint,
        isValid,
        validationError: isValid ? undefined : provider ? 'Key contains spaces or invalid characters' : 'Could not detect provider from key prefix',
        accountId,
      };
    ...
    const groups = this.buildGroups(parsed.filter(p => p.isValid), accountInfo);
  ...
  async assessRisk(fingerprint: string, provider: string | null, rawKey?: string): Promise<KeyRiskAssessment> {
    return this.assessKeyRisk(fingerprint, provider || 'Unknown', rawKey || '', this.deps.getExistingKeys());
  }
  ...
  private async assessKeyRisk(
    ...
    if (existingFps.has(fingerprint)) {
      factors.push({ type: 'duplicate', severity: 'high', description: `Key already exists for ${provider}` });
    }

    if (provider === 'Unknown' || !provider) {
      factors.push({ type: 'invalid_format', severity: 'high', description: 'Could not determine provider from key prefix' });
    }
    ...
    if (raw && provider && provider !== 'Unknown' && this.deps.verifyKey) {
      try {
        ...
        if (!health.valid) {
          factors.push({ type: 'high_error_rate', severity: 'critical', description: `Key failed health check: ${health.error || 'invalid or unreachable'}` });
        } else if (health.models.length === 0) {
          factors.push({ type: 'high_error_rate', severity: 'high', description: 'Key is valid but returned no available models' });
        } else {
          factors.push({ type: 'high_error_rate', severity: 'low', description: `Key verified — ${health.models.length} models available (${health.latency}ms)` });
        }
      } catch {
        factors.push({ type: 'high_error_rate', severity: 'critical', description: 'Health check request failed' });
      }
    }
    ...
    if (highErrorKeys.length > 0) {
      factors.push({ type: 'high_error_rate', severity: 'medium', description: `${highErrorKeys.length} existing ${provider} keys have >30% error rate` });
    }

    const highLatencyKeys = existing.filter(k => (k.stats?.avgLatency || 0) > 5000);
    if (highLatencyKeys.length > 0) {
      factors.push({ type: 'high_latency', severity: 'low', description: `${highLatencyKeys.length} existing ${provider} keys have avg latency >5s` });
    }

    const score = this.calculateRiskScore(factors);
    const level = this.riskLevel(score);

    return { fingerprint, raw: raw.slice(0, 12) + '...', provider, riskScore: score, riskLevel: level, factors };
  }
  ...
  private calculateRiskScore(factors: RiskFactor[]): number {
    ...
    return Math.min(100, factors.reduce((sum, f) => sum + (weights[f.severity] || 0), 0));
  }
  ...
  private generateRecommendations(
    ...
    for (const [provider, count] of byProvider) {
      if (count > 3) {
        recs.push(`Consider labeling ${provider} keys by purpose (prod/staging/test) for easier management`);
      }
      if (count > 5) {
        recs.push(`You have ${count} ${provider} keys — review if some are unused or can be consolidated`);
      }
    }

    for (const group of groups) {
      if (group.keyCount > 2) {
        recs.push(`Account "${group.label}" has ${group.keyCount} keys — consider distributing across multiple accounts for redundancy`);
      }
    }
    ...
    if (!hasMultiAccount && groups.some(g => g.label.endsWith('Account') && g.keyCount > 1)) {
      recs.push(`Multiple keys share the same default account — use provider-specific keys (Cloudflare account IDs, OpenAI project keys) to enable per-account management`);
    }

    const highRisks = risks.filter(r => r.riskLevel === 'high' || r.riskLevel === 'critical');
    if (highRisks.length > 0) {
      recs.push(`${highRisks.length} key(s) have elevated risk — review health check failures and error rates`);
    }

    const undetected = parsed.filter(p => !p.provider);
    if (undetected.length > 0) {
      recs.push(`${undetected.length} key(s) couldn't be auto-detected — verify they belong to a supported provider`);
    }
    return recs;
  }
}
