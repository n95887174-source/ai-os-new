...
function mockStore(overrides?: Partial<SecretStore>): SecretStore {
  ...
  return {
    ...
    set: vi.fn(async (ref, value) => { data.set(ref.path, value); return true; }),
...
function makeDeps() {
  ...
  return {
    ...
    database: {
      getKv: vi.fn(async <T>(id: string) => kv.get(id) as T ?? null),
      setKv: vi.fn(async <T>(id: string, value: T) => { kv.set(id, value); }),
    },
    storeFactories: {
