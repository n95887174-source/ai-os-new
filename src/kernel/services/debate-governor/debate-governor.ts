...
export class DebateGovernor {
  ...
  computeConvergence(): number {
    ...
    this.state.convergenceScore = Math.min(100,
  ...
  computeNovelty(): number {
    ...
    for (const cur of currentRoundClaims) {
      const isNovel = !priorClaims.some(p =>
        this.jaccard(cur.text, p.text) > 0.6,
      );
  ...
  updateDiversity(): DiversityState {
    ...
    return this.diversityScorer.update(bySpeaker, this.state.graph.edges, this.state.resolvedClaimIds);
  }
  ...
  generateSynthesis(): SynthesisResult {
    ...
    const coreDisagreement = contradictions.length > 0
    ...
    const consensus = allClaims.length > 0
