...
export class AgentGenerator {
  ...
  async generate(description: string): Promise<GeneratedConfig> {
    ...
    return {
      ...
      temperature: typeof config.temperature === 'number' ? Math.max(0, Math.min(2, config.temperature)) : 0.7,
      category: ['technical', 'creative', 'analytical', 'management', 'custom'].includes(config.category) ? config.category : 'custom',
    };
  }
...
Respond with ONLY the updated JSON object (same format as before, no markdown, no explanation).`;
    ...
    return {
      ...
      temperature: typeof config.temperature === 'number' ? Math.max(0, Math.min(2, config.temperature)) : currentConfig.temperature,
      category: ['technical', 'creative', 'analytical', 'management', 'custom'].includes(config.category) ? config.category : currentConfig.category,
    };
  }
