...
import type { ICounterfactualEngine, CounterfactualInput, CounterfactualResult, CounterfactualScoreDiff, CounterfactualOverride } from '../contracts/counterfactual';
...
function applyOverrides(state: SystemState, overrides: CounterfactualOverride): void {
  ...
  if (overrides.global?.providerHealth) {
    for (const [providerId, health] of Object.entries(overrides.global.providerHealth)) {
      ...
      if (!p) continue;
      if (health === 'offline') { p.status = 'offline'; p.reliability = 0; }
      else if (health === 'degraded') { p.reliability = Math.min(p.reliability, 0.3); }
    }
  }
...
function computeScoreDiffs(original: DecisionPayload, simulated: DecisionPayload): CounterfactualScoreDiff[] {
  const map = new Map<string, { orig?: number; sim?: number; origC?: Record<string, number>; simC?: Record<string, number> }>();
...
export class CounterfactualEngine implements ICounterfactualEngine {
  ...
  run(input: CounterfactualInput): CounterfactualResult {
    ...
    const original = toDecisionPayload({
      ...
      classification: input.baseTrace.decision.classification as RouterDecision['classification'],
    ...
    if (switchProvider) {
      const changed = scoreDiffs.find(s => s.provider === simulated.selected && s.delta > 0);
      const dropped = scoreDiffs.find(s => s.provider === original.selected && s.delta < 0);
      switchReason = `${original.selected} → ${simulated.selected}`;
      if (dropped) switchReason += ` (${original.selected} Δ${dropped.delta > 0 ? '+' : ''}${dropped.delta.toFixed(3)})`;
      if (changed) switchReason += ` (${simulated.selected} Δ${changed.delta > 0 ? '+' : ''}${changed.delta.toFixed(3)})`;
    }
