...
export class VirtualKeyService implements IVirtualKeyService {
  ...
  async create(realKeyId: string, label: string, agentId?: string): Promise<VirtualKey> {
    ...
    await this.persist();
    this.deps.eventBus.emit('virtual_key:created', { virtualKey: vk });
    return vk;
  }
  ...
  resolve(id: string): VirtualKey | undefined {
    ...
    if (vk && vk.active) {
      ...
      this.persist();
      this.deps.eventBus.emit('virtual_key:resolved', { virtualKeyId: id });
      return vk;
    }
    return undefined;
  }
  ...
  async revoke(id: string): Promise<void> {
    ...
    if (vk) {
      vk.active = false;
      await this.persist();
      this.deps.eventBus.emit('virtual_key:revoked', { virtualKeyId: id });
    }
  }
