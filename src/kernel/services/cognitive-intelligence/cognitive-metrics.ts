...
export class CognitiveMetricsEngine {
  ...
  compute(): CognitiveMetricsSnapshot {
    ...
    const avgReasoningCoherence = coherenceBySession.length > 0
      ? coherenceBySession.reduce((s, c) => s + c, 0) / coherenceBySession.length
      : 0;

    const debateQuality = Math.max(0, Math.min(1,
      (1 - avgContradictionDensity) * 0.4 + avgConsensusConfidence * 0.3 + avgReasoningCoherence * 0.3
    ));
    ...
    for (const t of topologyTypes) {
      ...
      topologyEffectiveness[`depth_${t}`] = withType.reduce((s, x) => s + x.consensusConfidence, 0) / withType.length;
    }

    const reasoningCollapseDetected = avgContradictionDensity > 0.6 && avgConsensusConfidence < 0.3;
      ...
      for (const s of latest) {
        if (s.contradictionDensity > 0.7) {
          hallucinationZones.push({
            ...
            evidence: [`Contradiction density ${(s.contradictionDensity * 100).toFixed(0)}%`],
            timestamp: Date.now(),
          });
        }
      }
