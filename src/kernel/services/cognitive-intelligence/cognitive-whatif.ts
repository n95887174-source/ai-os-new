...
export class CognitiveWhatIfEngine implements ICognitiveWhatIfEngine {
  simulateTopologyChange(current: CognitiveSessionSummary, proposedType: string): TopologyWhatIf {
    ...
    const estimatedDebateQuality = Math.max(0, Math.min(1,
      current.consensusConfidence + qualityDelta
    ));
    ...
    const estimatedRounds = Math.round(current.round * (proposed.depth / Math.max(1, currentComplexity.depth)));

    const riskScore = Math.min(1, (proposed.depth / 5) * 0.5 + (costMultiplier - 1) * 0.3 + (estimatedRounds > 10 ? 0.2 : 0));

    const recommendations: string[] = [];
    if (riskScore > 0.6) recommendations.push('High risk — consider less complex topology');
    if (estimatedDebateQuality > current.consensusConfidence) recommendations.push('May improve consensus quality');
  ...
  simulateParticipantChange(current: CognitiveSessionSummary, additionalAgents: number): {
    ...
    if (qualityDiminishingReturns < 0.5) recs.push('Quality may degrade — too many participants');
