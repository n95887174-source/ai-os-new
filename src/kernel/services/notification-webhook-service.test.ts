...
function makeDeps() {
  ...
  return {
    ...
    database: {
      ...
      setKv: vi.fn(async (_id: string, value: unknown) => { store.push(value); }),
    },
  };
}

describe('NotificationWebhookService', () => {
  it('should create and list webhooks', () => {
    const deps = makeDeps();
    const svc = new NotificationWebhookService(deps);
    const wh = svc.addWebhook({
      provider: 'slack',
      name: 'Test Slack',
  ...
  it('should remove a webhook', () => {
    ...
    const svc = new NotificationWebhookService(deps);
    const wh = svc.addWebhook({
      provider: 'telegram', name: 'Test', webhookUrl: 'https://t.me/test',
      enabled: true, events: ['system:notification'],
    });
  ...
  it('should update a webhook', () => {
    ...
    const svc = new NotificationWebhookService(deps);
    const wh = svc.addWebhook({
      provider: 'discord', name: 'Test', webhookUrl: 'https://discord.com/test',
      enabled: true, events: ['system:notification'],
    });
  ...
  it('should test a webhook', async () => {
    ...
    const svc = new NotificationWebhookService(deps);
    const wh = svc.addWebhook({
      provider: 'slack', name: 'Test', webhookUrl: 'https://hooks.slack.com/test',
      enabled: true, events: ['system:notification'],
    });
  ...
  it('should persist on write operations', () => {
    const deps = makeDeps();
    const svc = new NotificationWebhookService(deps);
    svc.addWebhook({
      provider: 'slack', name: 'Test', webhookUrl: 'https://hooks.slack.com/test',
