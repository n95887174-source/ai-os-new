...
export class DebateInterpreter {
  interpret(session: DebateSession): DebateInterpretation {
    ...
    const correlation = session.strategy === 'constrained' ? this.analyzeConstraintCorrelation(session) : undefined;
  ...
  private buildDisagreementTimeline(session: DebateSession): Array<{ round: number; intensity: number }> {
  ...
  private analyzeConstraintCorrelation(session: DebateSession): ConstraintCorrelation {
    const byConstraint: Record<string, { depths: number[]; confidences: number[]; challenges: number; compliance: number[]; count: number }> = {};

    for (const a of session.arguments) {
      const p = session.participants.find(pp => pp.id === a.agentId);
      if (!p?.constraint || p.constraint === 'none') continue;
      const key = p.constraint;
      if (!byConstraint[key]) byConstraint[key] = { depths: [], confidences: [], challenges: 0, compliance: [], count: 0 };
      ...
      const speculation = ['maybe', 'perhaps', 'likely', 'probably', 'possibly'].filter(w => lower.includes(w)).length;
      byConstraint[key].compliance.push(Math.max(0, 1 - speculation * 0.2));
    }

    const result: Record<string, { avgDepth: number; avgConfidence: number; challengeRate: number; compliance: number; count: number }> = {};
    for (const [key, data] of Object.entries(byConstraint)) {
      result[key] = {
        avgDepth: data.depths.length > 0 ? Math.round(data.depths.reduce((a, b) => a + b, 0) / data.depths.length * 10) / 10 : 0,
        avgConfidence: Math.round(data.confidences.reduce((a, b) => a + b, 0) / data.confidences.length * 100) / 100,
        challengeRate: data.count > 0 ? Math.round(data.challenges / data.count * 100) / 100 : 0,
        compliance: data.compliance.length > 0 ? Math.round(data.compliance.reduce((a, b) => a + b, 0) / data.compliance.length * 100) / 100 : 0,
        count: data.count,
      };
    }
  ...
  private generateInsights(session: DebateSession): string[] {
    ...
    if (session.convergenceScore > 80) {
      insights.push(`High convergence (${Math.round(session.convergenceScore)}%) — agents largely agreed by the end`);
    } else if (session.convergenceScore < 40) {
      insights.push(`Low convergence (${Math.round(session.convergenceScore)}%) — deep disagreement persisted throughout`);
    } else {
      insights.push(`Moderate convergence (${Math.round(session.convergenceScore)}%) — partial agreement reached`);
    }
    ...
    if (maxC - minC <= 1 && counts.length > 1) {
      ...
      if (mostActive && leastActive) {
        insights.push(`Imbalanced participation: "${mostActive}" dominated (${maxC} arguments) vs "${leastActive}" (${minC})`);
      }
    }
    ...
    if (gm) {
      if (gm.branchingFactor > 2) insights.push(`High branching factor (${gm.branchingFactor.toFixed(1)}) — arguments generated diverse sub-debates`);
      if (gm.orphanRate > 0.3) insights.push(`High orphan rate (${(gm.orphanRate * 100).toFixed(0)}%) — many arguments failed to attach to the tree; parser or agent discipline issue`);
      if (gm.challengeDensity > 0.5) insights.push(`Challenge-dominant debate (${(gm.challengeDensity * 100).toFixed(0)}% cross-position) — agents primarily contradicted each other`);
      if (gm.refinementDensity > 0.5) insights.push(`Refinement-dominant debate (${(gm.refinementDensity * 100).toFixed(0)}% same-position) — agents built on each other's ideas`);
      if (gm.maxDepth >= 4) insights.push(`Deep reasoning chains (max depth ${gm.maxDepth}) — some arguments were explored ${gm.maxDepth} levels deep`);
    }

    // Constraint insights
    if (session.strategy === 'constrained') {
      const withConstraint = session.participants.filter(p => p.constraint && p.constraint !== 'none');
      if (withConstraint.length > 0) {
        insights.push(`Constrained debate with ${withConstraint.length} unique constraints — agents operated under different reasoning restrictions`);
      }
    }
  ...
  private generateSummary(session: DebateSession): string {
    ...
    if (session.strategy === 'socratic') summary += 'Socratic method — questioners rotated each round. ';
    if (session.strategy === 'argument_tree') summary += 'Argument tree — hierarchical structure. ';
    if (session.strategy === 'constrained') {
      const constraints = [...new Set(session.participants.filter(p => p.constraint && p.constraint !== 'none').map(p => p.constraint))];
      summary += `Constrained by: ${constraints.join(', ')}. `;
    }
    if (gm) summary += `Max reasoning depth: ${gm.maxDepth}, branching: ${gm.branchingFactor.toFixed(1)}. `;
