...
export class TopologyManager implements ILifecycle {
  ...
  private hasActiveDynamicClone(agentId: string, topology: ISTopology): boolean {
    return topology.nodes.some(
      n => n.dynamic && (n.id.startsWith(`${agentId}-clone`) || n.id.startsWith(`${agentId}-scale`)),
    );
  }
  ...
  private applyLowDiversityReroute(topology: ISTopology): boolean {
    ...
    const modelKey = (n: ISNode) => `${n.config.provider || 'auto'}:${n.config.model || 'auto'}`;
    ...
    if (!alt) {
      const usedProviders = new Set(agents.map(a => (a.config.provider as string) || 'auto'));
      const fallbackProvider = ['groq', 'gemini', 'openrouter', 'nvidia'].find(p => !usedProviders.has(p));
  ...
  private evaluateTopology() {
    ...
    let modified = false;
    const newTopology: ISTopology = JSON.parse(JSON.stringify(topology)) as ISTopology;

    const unhealthyAgents = this.deps.agentHealthMonitor.getAllHealth().filter(
      h => h.health === 'unhealthy' || h.health === 'degraded',
    );

    for (const health of unhealthyAgents) {
      if (this.hasRecentClone(health.agentId) || this.hasActiveDynamicClone(health.agentId, newTopology)) continue;
