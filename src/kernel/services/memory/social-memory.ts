...
const MAX_SOCIAL = 10000;

export class SocialMemoryStore implements IMemoryStore {
    readonly type = MemoryStoreType.SOCIAL;
    ...
    async store(entry: Omit<MemoryEntry, 'id'>): Promise<string> {
        ...
        if (this.entries.length > MAX_SOCIAL) {
            this.entries.sort(
                (a, b) => (b.metadata.importance || 0) - (a.metadata.importance || 0),
            );
            this.entries = this.entries.slice(0, MAX_SOCIAL);
        }
        return id;
    }
