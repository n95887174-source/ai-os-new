import type { DebateGraphMetrics, DebateConstraint, ActivityMetrics, AgentActivityMetric, ArgumentImpact, QualityMetrics, DepthMetric, OriginalityMetric, UsefulnessMetric, DebateArgument, DebateParticipant } from '../contracts/debate-types';
import { jaccardSimilarity } from '../contracts/debate-types';

export function computeGraphMetrics(args: DebateArgument[], strategy: string): DebateGraphMetrics | undefined {
  ...
  return { totalNodes, maxDepth, avgDepth, orphanRate, branchingFactor, challengeDensity, refinementDensity };
}

export function computeActivityMetrics(args: DebateArgument[], participants: DebateParticipant[]): ActivityMetrics | undefined {
  ...
  for (const a of participants) agentMap.set(a.id, { argCount: 0, wordCount: 0, totalConfidence: 0, depths: [], childrenReceived: 0 });
  ...
  const perAgent: AgentActivityMetric[] = participants.map(a => {
...
export function computeQualityMetrics(args: DebateArgument[], topic: string): QualityMetrics | undefined {
  if (args.length === 0) return undefined;
...
export function getConstraintCompliance(args: DebateArgument[], participants: DebateParticipant[]): Record<string, number> {
