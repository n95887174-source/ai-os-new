import type { IExecutionGovernor, OperationSpec, ManagedOperation, OperationState, OperationType, OperationFilter } from '../contracts/execution-governor';
...
class ManagedOperationImpl implements ManagedOperation {
  ...
  get endedAt(): number | null { return this._endedAt; }
  ...
  fail(error: Error): void {
    ...
    this._cleanup();
    this._notify();
    LOGGER.warn('ExecutionGovernor', `Operation ${this.id} failed`, { type: this.type, error: error.message });
  }
  ...
  timeout(): void {
    ...
    this._cleanup();
    this._notify();
    LOGGER.warn('ExecutionGovernor', `Operation ${this.id} timed out`, { type: this.type, timeoutMs: Date.now() - this.startedAt });
  }

  child(spec: Omit<OperationSpec, 'parentId'>): ManagedOperation {
    return new ManagedOperationImpl(
