import type { IPressureEngine, PressureMapSnapshot, ProviderPressure, GlobalPressure, PressureLevel } from '../../contracts/advisor';

export interface PressureEngineDeps {
  keyService: {
    getKeys: () => Array<{
      id: string; provider: string; key: string; status: string; label?: string;
      latency?: number;
      stats?: {
  ...
  healthCheckService: {
    getSummary: () => { total: number; healthy: number; degraded: number; offline: number };
  };
  metricsService: {
    generateReport: () => { aggregated: { totalRequests: number; totalTokens: number; estimatedCost: number; avgLatency: number; successRate: number; errorRate: number }; providers: Array<{ id: string; status: string; reliability: number; avgTTFT: number }> };
  };
}
...
export class PressureEngine implements IPressureEngine {
  ...
  generateSnapshot(): PressureMapSnapshot {
    ...
    const providers: ProviderPressure[] = providerNames.map(name => {
      ...
      const fourSignals = signalCount > 0
        ? { latency: signalSum.latency / signalCount, throughput: signalSum.throughput / signalCount, errorRate: signalSum.errorRate / signalCount, saturation: signalSum.saturation / signalCount }
        : { latency: 0, throughput: 0, errorRate: 0, saturation: 0 };

      const totalUsed = pKeys.reduce((s, k) => s + (k.stats?.extended?.usageToday?.requests || 0), 0);
      const totalLimit = pKeys.reduce((s, k) => s + (k.stats?.extended?.rules?.quota?.requestsPerDay || 0), 0);
      const quotaPct = totalLimit > 0 ? Math.min(100, Math.round((totalUsed / totalLimit) * 100)) : 0;

      const budgetPct = pSpend ? Math.round(pSpend.pct) : 0;
      const alertCount = pKeys.reduce((s, k) => s + (k.stats?.extended?.alerts?.filter(a => !a.resolved).length || 0), 0);
      ...
      return {
        ...
        level: this.getPressureLevel(score), score,
        status: kStatus, reliability, quotaPct, budgetPct,
        keysActive: activeKeys.length, keysTotal: pKeys.length,
        latencySignal: fourSignals.latency,
        errorRateSignal: fourSignals.errorRate,
        saturation: fourSignals.saturation,
        avgLatency, remainingQuota, remainingBudget,
        forecast, alertCount,
      };
    });
    ...
    const degradedKeys = keys.filter(k => k.status === 'error' || k.status === 'quarantined' || k.status === 'compromised').length;
    const criticalAlerts = alerts.filter(a => a.severity === 'critical').length;
    const budgetPct = spendSummary.global.pct;

    const globalScore = providers.length > 0
      ? Math.round(providers.reduce((s, p) => s + p.score, 0) / providers.length) : 0;

    const global: GlobalPressure = {
      level: this.getPressureLevel(globalScore), score: globalScore,
      totalKeys, activeKeys, degradedKeys,
      totalAlerts: alerts.length, criticalAlerts,
      budgetUsagePct: Math.round(budgetPct),
      budgetRemaining: spendSummary.global.remaining,
      totalRequests: state.totalRequests, totalCost: state.estimatedCost,
    };

    const snapshot: PressureMapSnapshot = { timestamp: now, global, providers: providers.sort((a, b) => b.score - a.score) };
    this.lastSnapshot = snapshot;
    return snapshot;
  }

  getLastSnapshot(): PressureMapSnapshot | null { return this.lastSnapshot; }
