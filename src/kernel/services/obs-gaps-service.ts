...
const STATIC_SERVICES: ServiceObsInfo[] = [
  ...
  { name: 'settingsService', hasEvents: true, hasLogger: true, hasLifecycle: true, hasHealthCheck: false, hasTracing: false },
  ...
  { name: 'debateService', hasEvents: true, hasLogger: true, hasLifecycle: true, hasHealthCheck: false, hasTracing: false },
  ...
  { name: 'workspaceService', hasEvents: true, hasLogger: true, hasLifecycle: true, hasHealthCheck: false, hasTracing: false },
...
function buildRecommendations(
  ...
  if (coverage.eventScore < 100) {
    recs.push(`Add event emission to ${services.filter((s) => !s.hasEvents).map((s) => s.name).join(', ')}`);
  }
  if (coverage.loggerScore < 100) {
    recs.push(`Add ILogger to ${services.filter((s) => !s.hasLogger).map((s) => s.name).join(', ')}`);
  }
  if (coverage.healthScore < 80) {
    recs.push(`Implement health checks for more services (${coverage.withHealth}/${coverage.total})`);
  }
  if (coverage.tracingScore < 50) {
    recs.push(
      `Add tracing spans to high-value services: ${services.filter((s) => !s.hasTracing && s.hasEvents).map((s) => s.name).join(', ')}`,
    );
  }
  if (coverage.lifecycleScore < 100) {
    recs.push(
      `Ensure all services implement ILifecycle: ${services.filter((s) => !s.hasLifecycle).map((s) => s.name).join(', ')}`,
    );
  }
  if (docEventCount > 0) {
...
export class ObsGapsService implements IObsGapsService {
  ...
  analyzeServiceContent(name: string, content: string): ServiceObsInfo {
    ...
    return {
      ...
      hasHealthCheck:
        /IHealthCheck/.test(content) || /healthCheck/.test(content) || /getHealth\(/.test(content),
      hasTracing:
        /ITraceContext/.test(content) || /TraceContext/.test(content) || /traceId/.test(content),
