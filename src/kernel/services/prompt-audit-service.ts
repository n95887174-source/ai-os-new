...
function enrichAgent(
  base: Omit<AuditedAgentPrompt, 'wordCount' | 'hasTools' | 'hasKeyTerms' | 'avgWordLen' | 'qualityScore'>,
): AuditedAgentPrompt {
    ...
    for (const role of roles) {
      ...
      agents.push(
        enrichAgent({
          id: role.id,
          name: role.name,
          group: ROLE_CATEGORY_LABEL[role.metadata?.category || 'custom'] || 'Specialized',
