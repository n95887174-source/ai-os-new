...
class AgentSimilarityService {
    ...
    async init(): Promise<void> {
        const saved = await (await this.db()).getKv<[string, AgentProfile][]>('profiles');
        if (saved) {
    ...
    searchByDescription(
        query: string,
        limit = 10,
    ): Array<{ agentId: string; name: string; role: string; similarity: number }> {
        ...
        for (const [id, profile] of this.profiles.entries()) {
            ...
            // Capability match
            if (profile.capabilities.some((c) => c.toLowerCase().includes(queryLower)))
                similarity += 0.2;
            // Tool match
    ...
    private async save(): Promise<void> {
        await (await this.db()).setKv('profiles', Array.from(this.profiles.entries()));
    }
}
