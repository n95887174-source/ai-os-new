...
export class DowngradeStrategy {
  ...
  constructor(
    private routingPolicy: { getDowngradedModel: (model: string) => string | null; getDeepDowngradedModel: (model: string, steps: number) => string | null },
  ...
  evaluate(model: string, metrics: ProviderMetrics): DowngradeCandidate | null {
    const triggers: Array<{ type: DowngradeCandidate['trigger']; severity: DowngradeCandidate['severity']; reason: string }> = [];

    const quotaPct = metrics.quotaLimit > 0 ? (metrics.quotaUsed / metrics.quotaLimit) * 100 : 0;

    if (metrics.avgLatency > this.thresholds.latencyMs * 1.5) {
      triggers.push({ type: 'latency', severity: 'hard', reason: `Latency ${metrics.avgLatency}ms exceeds hard threshold ${this.thresholds.latencyMs * 1.5}ms` });
    ...
    if (quotaPct > 95) {
      triggers.push({ type: 'quota', severity: 'hard', reason: `Quota ${quotaPct.toFixed(0)}% > 95%` });
    } else if (quotaPct > this.thresholds.quotaPct) {
      triggers.push({ type: 'quota', severity: 'soft', reason: `Quota ${quotaPct.toFixed(0)}% > ${this.thresholds.quotaPct}%` });
    }
    ...
    return {
      ...
      reason: hardTriggers.length > 0 ? hardTriggers.map(t => t.reason).join('; ') : worst.reason,
      trigger: worst.type,
      severity: worst.severity,
    };
  }

  evaluateWithDeep(model: string, metrics: ProviderMetrics, maxSteps = 3): DowngradeCandidate | null {
