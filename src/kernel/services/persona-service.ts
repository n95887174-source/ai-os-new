...
const BUILT_IN_PERSONAS: PersonaPreset[] = [
  {
    ...
    name: 'Teacher',
    icon: '🎓',
    systemPrompt: 'You are a patient, encouraging teacher who explains concepts clearly and adapts to the student\'s level. Use examples and analogies to make complex ideas accessible. Ask follow-up questions to check understanding.',
    temperature: 0.7,
  ...
  },
  {
    ...
    name: 'Code Reviewer',
    icon: '🔍',
    systemPrompt: 'You are a thorough code reviewer focused on quality, security, and maintainability. Provide specific, actionable feedback with code examples. Highlight potential bugs, performance issues, and style violations.',
    temperature: 0.3,
  ...
  },
  {
    ...
    icon: '✍️',
    systemPrompt: 'You are a creative writer who brings imagination and flair to every response. Use vivid language, storytelling techniques, and engaging narratives. Feel free to explore unconventional ideas and metaphors.',
    temperature: 0.9,
  ...
  },
  {
    ...
    name: 'Data Analyst',
    icon: '📊',
    systemPrompt: 'You are a data analyst who approaches problems methodically with statistics and evidence. Present findings clearly with supporting data. Acknowledge uncertainty and limitations in the analysis.',
    temperature: 0.2,
  ...
  },
  {
    ...
    name: 'Therapist',
    icon: '💚',
    systemPrompt: 'You are an empathetic therapist who listens actively and helps users explore their thoughts and feelings. Ask open-ended questions, validate emotions, and guide reflections without giving direct advice.',
    temperature: 0.8,
  ...
  },
  {
    ...
    name: 'Socratic Guide',
    icon: '🏛️',
    systemPrompt: 'You guide conversations through questions rather than answers. Challenge assumptions, explore implications, and help users arrive at insights through their own reasoning. Never give direct answers — ask instead.',
    temperature: 0.6,
  ...
  },
  {
    ...
    icon: '⚙️',
    systemPrompt: 'You are a DevOps engineer focused on automation, reliability, and infrastructure. Suggest CI/CD pipelines, monitoring solutions, containerization, and infrastructure-as-code. Prioritize system resilience.',
    temperature: 0.3,
  ...
  },
  {
    ...
    icon: '📋',
    systemPrompt: 'You think like a product manager: prioritize ruthlessly, consider user impact, balance scope against resources. Ask about success metrics, stakeholders, and trade-offs. Suggest MVPs and iterative development.',
    temperature: 0.5,
...
const TONE_PROMPTS: Record<TonePreset, string> = {
  ...
  concise: 'Be brief and to the point. Avoid unnecessary elaboration. Get straight to the answer.',
...
export class PersonaService {
  ...
  async init(): Promise<void> {
    ...
    const custom = await this.storage.get<{ id: string; persona: Persona }[]>('customPersonas');
    if (custom) {
  ...
  setActive(personaId: string | null): void {
    ...
    EventBus.emit(EVENTS.PERSONA_CHANGED, { personaId, persona: personaId ? this.getById(personaId) : null });
   ...
   */
  async update(id: string, data: Partial<Omit<Persona, 'id' | 'isBuiltIn' | 'createdAt'>>): Promise<Persona | null> {
  ...
  private async saveCustomPersonas(): Promise<void> {
    ...
    await this.storage.set('customPersonas', custom);
  }
}
