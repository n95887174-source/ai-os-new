...
export interface AgentElo {
  ...
  draws: number;
  matches: number;
  history: Array<{ timestamp: number; elo: number; change: number; opponent?: string; result?: DebateResult }>;
}
...
export class EloRatingService {
  ...
  async init(): Promise<void> {
    ...
    if (saved) {
      try {
        const parsed = JSON.parse(saved) as AgentEloProfile[];
        for (const profile of parsed) {
          this.profiles.set(profile.agentId, profile);
        }
      } catch (e) {
        LOGGER.error('EloRating', 'Failed to parse stored profiles', e as Record<string, unknown>);
      }
    }
  ...
  updateRatings(
    ...
    const now = Date.now();
    winner.ratingHistory.push({ timestamp: now, rating: winner.rating, change: winnerChange, opponent: loserId, result });
    loser.ratingHistory.push({ timestamp: now, rating: loser.rating, change: loserChange, opponent: winnerId, result: result === 'win' ? 'loss' : result === 'loss' ? 'win' : 'draw' });

    if (winner.ratingHistory.length > 1000) winner.ratingHistory = winner.ratingHistory.slice(-500);
    if (loser.ratingHistory.length > 1000) loser.ratingHistory = loser.ratingHistory.slice(-500);
  ...
  getRecentChanges(limit = 20): Array<{ agentId: string; agentName: string; change: number; timestamp: number }> {
    const changes: Array<{ agentId: string; agentName: string; change: number; timestamp: number }> = [];
  ...
  compare(agentIdA: string, agentIdB: string): {
    ratingDiff: number;
  ...
  getHeadToHead(agentIdA: string, agentIdB: string): { winsA: number; winsB: number; draws: number } {
  ...
  private save(): void {
    this.storage.setItem('elo-ratings:profiles', JSON.stringify(Array.from(this.profiles.values())));
  }
}
