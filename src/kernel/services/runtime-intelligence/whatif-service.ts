...
export class WhatIfService implements ILifecycle, IWhatIfService {
  ...
  async simulateTopologyChange(sessionId: string, proposedType: string): Promise<TopologyWhatIf | undefined> {
    ...
    this.deps.eventBus.emit(EVENTS.WHATIF_SIMULATION_COMPLETED, { type: 'topology', sessionId, proposedType, hasResult: !!result });
    return result;
  }
  ...
  async simulateParticipantChange(
    ...
    this.deps.eventBus.emit(EVENTS.WHATIF_SIMULATION_COMPLETED, { type: 'participant', sessionId, additionalAgents, hasResult: !!result });
    return result;
  }
  ...
  async simulateBudgetChange(sessionId: string, proposedBudget: number): Promise<BudgetWhatIf | undefined> {
    ...
    this.deps.eventBus.emit(EVENTS.WHATIF_SIMULATION_COMPLETED, { type: 'budget', sessionId, proposedBudget, currentBudget, ratio });
    return result;
  }
  ...
  async simulateProviderChange(currentProvider: string, proposedProvider: string): Promise<ProviderWhatIf> {
    ...
    this.deps.eventBus.emit(EVENTS.WHATIF_SIMULATION_COMPLETED, { type: 'provider', currentProvider, proposedProvider, latencyImpact, costImpact, reliabilityImpact });
    return result;
  }
  ...
  async simulateStrategyChange(currentStrategy: string, proposedStrategy: string): Promise<StrategyWhatIf> {
    ...
    this.deps.eventBus.emit(EVENTS.WHATIF_SIMULATION_COMPLETED, { type: 'strategy', currentStrategy, proposedStrategy, estimatedQualityChange, estimatedLatencyChange, estimatedCostChange });
    return result;
  }
  ...
  async simulatePolicyDryRun(proposedPolicy: ISPolicy): Promise<PolicyDryRunResult> {
    ...
    const nodes = realNodes.length > 0 ? realNodes : [
      ...
      { id: 'node_3', latency: 1500, cost: 0.008, content: 'my email is secure@gmail.com', model: 'gemini-2.0-flash' },
    ...
    this.deps.eventBus.emit(EVENTS.WHATIF_SIMULATION_COMPLETED, { type: 'policy_dry_run', policyType: proposedPolicy.type, violationsCount, severityLevel });
    return result;
  }
