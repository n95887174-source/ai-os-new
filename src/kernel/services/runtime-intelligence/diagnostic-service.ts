...
export class DiagnosticService implements ILifecycle, IDiagnosticService {
  ...
  getSystemDiagnostic(): SystemDiagnostic {
    ...
    const health = criticalCount > 0 ? 'critical' as const : highCount > 0 ? 'degraded' as const : 'healthy' as const;
  ...
  getAllActiveIssues(): CognitiveIssue[] {
    ...
    const providerIssues = Array.from(this.providerDiagnostics.values()).flatMap(d => d.issues);
  ...
  async runDiagnostic(scope: DiagnosticScope = 'system'): Promise<DiagnosticRunRecord> {
    ...
    const score = Math.max(0, Math.min(1, 1 - issues.filter(i => i.severity === 'critical').length * 0.3));
