...
export class ProviderTracker implements IProviderTracker {
  ...
  start(eventBus: IEventBus): void {
    this.unsubs.push(
      ...
      eventBus.on(EVENTS.STREAM_END, (raw: unknown) => {
        const data = raw as { provider?: string; latency: number; tokens?: number; fullContent?: string; model?: string; ttft?: number };
  ...
  private persistMetrics(): void {
    ...
    void this.database.setKv(ProviderTracker.METRICS_KEY, obj)
  ...
  getMetrics(_provider: string, keyId: string): {
    ...
    return {
      ...
      totalRequests: state.health.consecutiveErrors,
  ...
  getProviderRankings(catalogProviders: string[] = []): ProviderRanking[] {
    ...
    const pushRanking = (id: string, p: ProviderState) => {
      ...
      rankings.push({
        provider: norm, score, reliability,
        avgLatency: p.avgTTFT, requests: p.totalRequests,
        costPerRequest, recommendation, installed: installed.has(norm),
      });
    };
  ...
  private keyStateToHealthEvents(state: KeyState): HealthEvent[] {
    ...
    if (state.health.consecutiveErrors >= 3 || state.flags.authFailed || state.flags.circuitOpen) {
      const reason = state.flags.authFailed ? 'auth failure'
        : state.flags.circuitOpen ? 'circuit open'
        : `${state.health.consecutiveErrors} consecutive errors`;
      events.push({
        provider, type: 'error_burst',
  ...
  private detectErrorBurst(provider: string, _state: ProviderState): void {
    ...
    if (count === 5) {
      this.recordHealthEvent(provider, 'error_burst', `5 consecutive errors`);
