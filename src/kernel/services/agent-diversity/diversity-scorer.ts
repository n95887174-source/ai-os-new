...
import { pairwiseAgentSimilarities, computeSemanticDiversityScore, findRedundantPairs } from './semantic-distance';
...
export class DiversityScorer {
  ...
  update(
    ...
    const clusterableProfiles = new Map<string, { agentId: string; reasoningPattern: import('./types').ReasoningPattern; semanticSims: number[]; influence: number }>();
    ...
    const clusters = computeClusters(clusterableProfiles, this.config.clusterSimilarityThreshold);
    ...
    const globalDiversityIndex = profiles.length > 0
      ? Math.round((profiles.reduce((s, p) => s + p.overallScore, 0) / profiles.length) * 100) / 100
      : 0;
  ...
  shouldSuppressAgent(agentId: string, diversityThreshold = 0.3): boolean {
    ...
    return profile.overallScore < diversityThreshold && profile.redundancyScore > this.config.redundancyThreshold;
  }
