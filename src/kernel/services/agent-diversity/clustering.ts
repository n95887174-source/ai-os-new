...
function buildAgentVectors(
  profiles: Map<string, { reasoningPattern: ReasoningPattern; semanticSims: number[]; influence: number }>,
): AgentVector[] {
  ...
  return [...profiles.entries()].map(([agentId, p]) => {
    ...
    const avgSemSim = p.semanticSims.length > 0
...
export function computeClusters(
  profiles: Map<string, { agentId: string; reasoningPattern: ReasoningPattern; semanticSims: number[]; influence: number }>,
  ...
  const patternNames: ReasoningPattern[] = [
    ...
    'causal', 'adversarial', 'synthesis-heavy',
  ];
