...
export function computeInfluenceScores(
  ...
  for (const agentId of agents) {
    const agentClaimIds = new Set(
      (claimsByAgent.get(agentId) ?? []).map(c => c.id),
    );

    const uniqueEdgesFromAgent = edges.filter(
      e => agentClaimIds.has(e.from),
    );

    const agentTriggeredContradictions = edges.filter(
      e =>
        agentClaimIds.has(e.from) &&
        e.type === 'challenges',
    );
    ...
    const adoptionScore = Math.min(
      1,
      adoptedIndirectly.length / (agentClaimIds.size || 1),
    );
