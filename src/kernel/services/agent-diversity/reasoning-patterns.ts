import type { Claim, ClaimEdge } from '../debate-governor/types';
...
const DEDUCTIVE_INDICATORS = [
  ...
  'must be', 'necessarily', 'implies', 'proves that',
  'следовательно', 'значит', 'поэтому', 'отсюда следует',
];

const INDUCTIVE_INDICATORS = [
  'typically', 'generally', 'in most cases', 'often', 'tends to',
  'suggests that', 'indicates', 'pattern', 'observed',
  'обычно', 'как правило', 'в большинстве случаев', 'свидетельствует',
];

const ANALOGICAL_INDICATORS = [
  'similarly', 'likewise', 'by analogy', 'parallel', 'comparable',
  'just as', 'similar to', 'analogous',
  'аналогично', 'подобно', 'так же как', 'по аналогии',
];

const CAUSAL_INDICATORS = [
  'because', 'causes', 'leads to', 'results in', 'due to',
  'triggered by', 'as a result', 'effect of',
  'потому что', 'приводит к', 'вызвано', 'в результате',
];

const ADVERSARIAL_INDICATORS = [
  'however', 'but', 'nevertheless', 'on the contrary',
  'conversely', 'while you claim', 'your argument fails',
  'однако', 'но', 'напротив', 'вопреки', 'ваш аргумент',
];

const SYNTHESIS_INDICATORS = [
  'integrating', 'synthesizing', 'combining', 'unifying',
  'holistic', 'both sides', 'common ground', 'balanced view',
  'объединяя', 'синтезируя', 'целостный', 'оба подхода',
];
...
function detectPattern(text: string): { pattern: ReasoningPattern; score: number } {
  ...
  const scores: Array<{ pattern: ReasoningPattern; score: number }> = [
    { pattern: 'deductive', score: countMatches(lower, DEDUCTIVE_INDICATORS) / DEDUCTIVE_INDICATORS.length },
    { pattern: 'inductive', score: countMatches(lower, INDUCTIVE_INDICATORS) / INDUCTIVE_INDICATORS.length },
    { pattern: 'analogical', score: countMatches(lower, ANALOGICAL_INDICATORS) / ANALOGICAL_INDICATORS.length },
    { pattern: 'causal', score: countMatches(lower, CAUSAL_INDICATORS) / CAUSAL_INDICATORS.length },
    { pattern: 'adversarial', score: countMatches(lower, ADVERSARIAL_INDICATORS) / ADVERSARIAL_INDICATORS.length },
    { pattern: 'synthesis-heavy', score: countMatches(lower, SYNTHESIS_INDICATORS) / SYNTHESIS_INDICATORS.length },
  ];
...
export function extractReasoningSignature(
  ...
  for (const [p, s] of Object.entries(edgeScores)) {
    patternScores[p as ReasoningPattern] = (patternScores[p as ReasoningPattern] ?? 0) + s * 0.5;
  }
