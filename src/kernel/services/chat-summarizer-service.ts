...
export class ChatSummarizerService {
  ...
  async generateSummary(
    ...
    if (messages.length < 10) {
      LOGGER.debug('ChatSummarizer', 'Not enough messages to summarize', { sessionId, count: messages.length });
      return null;
    }
...
UNRESOLVED: [questions or topics that remain open]`;
  ...
  async manualSummarize(sessionId: string, messages: ChatMessage[]): Promise<ChatSummary | null> {
    return this.generateSummary(sessionId, messages);
  }

  async autoSummarize(sessionId: string, messages: ChatMessage[]): Promise<ChatSummary | null> {
    if (!this.shouldSummarize(sessionId, messages.length)) return this.getSummary(sessionId) ?? null;
    return this.generateSummary(sessionId, messages);
  }
  ...
  setConfig(config: Partial<SummarizationConfig>): void {
    ...
    LOGGER.info('ChatSummarizer', 'Configuration updated', this.config as unknown as Record<string, unknown>);
  }
