    ...
    switch (node.type) {
      case 'agent':
        return mode === 'simulation'
          ? await cognitiveService.executeAgentNode(node, { ...data, output: `[SIM] ${data.output || ''}` })
          : await cognitiveService.executeAgentNode(node, data);
      case 'router':
      ...
      case 'guardrail': {
        ...
        if (!approved) {
          eventBus.emit('system:notification', { message: `Guardrail violation: ${error}`, type: 'warning' });
  ...
  private async executeRouterNode(node: ISNode, data: NodeContext): Promise<string> {
    ...
    if (routeModel) {
      try {
        ...
        const decision = await cognitiveService.executeAgentNode(
