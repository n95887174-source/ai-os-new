...
self.onmessage = async (event: MessageEvent) => {
    ...
    try {
        ...
        // Uses string concatenation instead of template literal to prevent code injection
        // via backtick/$${...} in user-supplied code.
        const sandboxBody =
            '"use strict";' +
            'const Function=freeze(func(function(){}));' +
            'const AsyncFunction=freeze(func("return async function(){}")());' +
            'const GeneratorFunction=freeze(func("return function*(){}")());' +
            'const Object=freeze({});' +
            'const{fetch,eval,XMLHttpRequest,WebSocket,importScripts,indexedDB,postMessage,addEventListener,removeEventListener,Worker,MessageChannel,BroadcastChannel,EventSource,Notification,requestAnimationFrame,cancelAnimationFrame,Proxy,Reflect}={};' +
            'const self=freeze(proxySelf);' +
