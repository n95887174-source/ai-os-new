...
class MemoryService {
  ...
  private async load() {
    try {
      if ((await dexieDb.memories.count()) > 0) {
  ...
  async store(entry: Omit<MemoryEntry, 'id'>) {
    const newEntry: MemoryEntry = { ...entry, id: crypto.randomUUID().slice(0, 8) };
    try {
  ...
  async storeBatch(entries: Omit<MemoryEntry, 'id'>[]) {
    const newEntries = entries.map(e => ({ ...e, id: crypto.randomUUID().slice(0, 8) }));
    try {
  ...
  getStats(): MemoryStats {
    ...
    for (const m of this.memories) {
      ...
      const label = m.metadata.importance < 0.3 ? 'low' : m.metadata.importance < 0.6 ? 'medium' : m.metadata.importance < 0.9 ? 'high' : 'critical';
  ...
  async prune(options: MemoryPruneOptions): Promise<MemoryPruneResult> {
    ...
    if (options.olderThan) {
      ...
      const old = this.memories.filter(m => m.metadata.timestamp < cutoff);
      if (old.length > 0) {
        if (!options.dryRun) {
          await dexieDb.memories.where('metadata.timestamp').below(cutoff).delete();
          this.memories = this.memories.filter(m => m.metadata.timestamp >= cutoff);
    ...
    if (options.importanceBelow !== undefined) {
      const low = this.memories.filter(m => m.metadata.importance < options.importanceBelow!);
      if (low.length > 0 && !options.dryRun) {
        for (const m of low) {
          await dexieDb.memories.delete(m.id).catch((e) => console.warn('[Memory] Failed to delete low-importance memory', e));
        }
        this.memories = this.memories.filter(m => m.metadata.importance >= options.importanceBelow!);
      }
