...
export class MockAdapter implements LLMProviderAdapter {
  ...
  constructor(options?: MockAdapterOptions) {
    ...
    this.presetResponse = options?.presetResponse ?? 'This is a mock response from the sandbox adapter.';
  ...
  async checkHealth(_apiKey: string): Promise<HealthCheckResult> {
    ...
    if (this.healthStatus === 'error') {
      return { status: 'error', latency: this.simulateLatencyMs, models: [], error: 'Simulated health check failure' };
    }
