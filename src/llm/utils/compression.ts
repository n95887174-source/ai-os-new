...
export function compressText(text: string, options?: CompressOptions): CompressionResult {
  ...
  return { text: compressed, originalTokens, compressedTokens, ratio: compressedTokens / originalTokens };
}
...
export function compressMessages(
  ...
  if (result.length <= 1) {
    const compressed = compressText(result[0].content, { ...options, strategy: options?.strategy === 'drop-system' ? 'truncate-middle' : options?.strategy });
  ...
  return result.map(m => {
    ...
    const compressed = compressText(m.content, { maxTokens: perMsg, strategy: 'truncate-middle' });
    return { ...m, content: compressed.text };
  });
}

export function getCompressionStats(results: CompressionResult[]): { totalOriginal: number; totalCompressed: number; overallRatio: number } {
  ...
  return { totalOriginal, totalCompressed, overallRatio: totalOriginal > 0 ? totalCompressed / totalOriginal : 1 };
}
