    ...
    <>
        ...
        {configForm && (
          ...
          >
            ...
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, healthCheckStaleIntervalMs: v } : f))}
              min={1000}
              max={3600000}
              defaultValue={30000}
            />
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, latencyPenaltyThresholdMs: v } : f))}
              min={0}
              max={60000}
              defaultValue={1000}
            />
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, errorRatePenaltyThreshold: v } : f))}
              step="0.01"
              min={0}
              max={1}
              defaultValue={0.1}
            ...
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, successRatePenaltyFloor: v } : f))}
              step="0.01"
              min={0}
              max={1}
              defaultValue={0.5}
            ...
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, alertPenaltyPerAlert: v } : f))}
              min={0}
              max={100}
              defaultValue={5}
            />
            ...
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, metricsHistoryLimit: v } : f))}
              min={10}
              max={100000}
              defaultValue={100}
            />
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, metricsInterval: v } : f))}
              min={1000}
              max={3600000}
              defaultValue={60000}
            />
            ...
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, tracesMaxEntries: v } : f))}
              min={10}
              max={100000}
              defaultValue={1000}
            />
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, tracesDbLoadLimit: v } : f))}
              min={1}
              max={10000}
              defaultValue={100}
            />
            <ConfigInput
              ...
              onChange={(v) => setConfigForm((f) => (f ? { ...f, tracesTokenEstimateDivisor: v } : f))}
              min={1}
              max={10000}
              defaultValue={1000}
            />
      ...
      <SettingRow
        ...
        description={t('settings.secrets_backends_desc', {
          backend: secretsBackends.find((b) => b.active)?.label || t('common.not_available'),
        })}
      >
        <button
          type="button"
          ...
          style={{
            ...
            fontSize: '0.78rem',
            fontWeight: 600,
            background: showSecretsDetail ? 'rgba(139,92,246,0.15)' : 'rgba(139,92,246,0.05)',
          ...
          {secretsBackends.map((b) => (
            <div key={b.type} style={flexBetween}>
              <div>
                ...
                <span style={{ marginLeft: '0.5rem', color: b.healthy ? '#10b981' : '#ef4444' }}>{b.healthy ? '✓' : '✗'}</span>
              </div>
              {!b.active && (
                <button
                  type="button"
                  onClick={async () => {
                    ...
                    setSecretsBackends(await externalSecretsService.getStatus());
                  }}
                  style={{
              ...
              {b.active && (
                <span style={{ color: '#8b5cf6', fontWeight: 700, fontSize: '0.7rem', textTransform: 'uppercase' }}>
                  {t('common.active')}
                </span>
              )}
            </div>
      ...
      <SettingRow icon={<Settings size={20} aria-hidden="true" />} accent="#f59e0b" title={t('settings.reset_title')} description={t('settings.reset_desc')}>
        <button type="button" style={amberBtn} onClick={onResetDefaults} aria-label={t('settings.reset_aria')}>
      ...
      <SettingRow icon={<Database size={20} aria-hidden="true" />} accent="#ef4444" title={t('settings.factory_reset')} description={t('settings.factory_reset_desc')}>
        <button type="button" style={{ ...dangerBtn, display: 'flex', alignItems: 'center', gap: 6 }} onClick={onPurgeData} aria-label={t('settings.factory_aria')}>
