...
import { Search, Trash2, Download, Filter, MessageSquare, X, RefreshCw, AlertCircle, Hash, User, Bot } from 'lucide-react';
...
function highlight(text: string, query: string): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark style={{ background: 'rgba(245,158,11,0.3)', color: '#fcd34d', padding: '0 1px' }}>
...
export const MessageSearchPanel: React.FC = () => {
  ...
  }, [query, caseSensitive, useRegex, roleFilter, providerFilter, modelFilter, sessionFilter, fromDate, toDate, minTokens]);
  ...
  const assistantCount = useMemo(() => results.filter(r => r.role === 'assistant').length, [results]);
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '1rem', overflow: 'hidden' }}>
      <div style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '0.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.4rem' }}>
          ...
          <button onClick={handleExport} disabled={results.length === 0} style={btnSecondary}><Download size={12} /> {t('message_search.export')}</button>
          <button onClick={handleClearAll} disabled={total === 0} style={btnSecondaryRed}><Trash2 size={12} /> {t('message_search.clear')}</button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showFilters && (
          ...
          >
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.4rem', padding: '0.5rem', borderRadius: 8, border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.2)' }}>
              <FilterSelect label={t('message_search.f_role')} value={roleFilter} onChange={v => setRoleFilter(v as '' | 'user' | 'assistant')} options={[{ v: '', l: t('message_search.any_role') }, { v: 'user', l: t('message_search.user') }, { v: 'assistant', l: t('message_search.assistant') }]} />
              <FilterSelect label={t('message_search.f_provider')} value={providerFilter} onChange={v => setProviderFilter(v)} options={[{ v: '', l: t('message_search.any_provider') }, ...providers.map(p => ({ v: p, l: p }))]} />
              <FilterSelect label={t('message_search.f_model')} value={modelFilter} onChange={v => setModelFilter(v)} options={[{ v: '', l: t('message_search.any_model') }, ...models.map(p => ({ v: p, l: p }))]} />
              <FilterSelect label={t('message_search.f_session')} value={sessionFilter} onChange={v => setSessionFilter(v)} options={[{ v: '', l: t('message_search.any_session') }, ...sessions.slice(0, 30).map(s => ({ v: s.id, l: `${s.id.slice(0, 12)}… (${s.count})` }))]} />
              <FilterInput label={t('message_search.f_from')} type="date" value={fromDate} onChange={setFromDate} />
              <FilterInput label={t('message_search.f_to')} type="date" value={toDate} onChange={setToDate} />
              <FilterInput label={t('message_search.f_min_tokens')} type="number" value={minTokens.toString()} onChange={v => setMinTokens(parseInt(v) || 0)} />
              <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                <button onClick={() => { setRoleFilter(''); setProviderFilter(''); setModelFilter(''); setSessionFilter(''); setFromDate(''); setToDate(''); setMinTokens(0); }} style={btnSecondary}>
      ...
      {results.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.4rem' }}>
          <StatBox icon={<MessageSquare size={14} color="#06b6d4" />} label={t('message_search.results')} value={results.length} color="#06b6d4" />
          <StatBox icon={<User size={14} color="#3b82f6" />} label={t('message_search.user_msgs')} value={userCount} color="#3b82f6" />
          <StatBox icon={<Bot size={14} color="#10b981" />} label={t('message_search.assistant_msgs')} value={assistantCount} color="#10b981" />
        </div>
      )}

      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        <AnimatePresence>
          {results.map(r => (
            <motion.div
              key={r.id}
              ...
              exit={{ opacity: 0 }}
              style={{ padding: '0.5rem 0.75rem', borderRadius: 8, border: '1px solid rgba(255,255,255,0.05)', background: r.role === 'user' ? 'rgba(59,130,246,0.05)' : 'rgba(16,185,129,0.05)' }}
            >
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 4, flexWrap: 'wrap' }}>
                <span style={{ padding: '0.1rem 0.4rem', borderRadius: 4, background: r.role === 'user' ? 'rgba(59,130,246,0.15)' : 'rgba(16,185,129,0.15)', color: r.role === 'user' ? '#93c5fd' : '#6ee7b7', fontSize: '0.65rem', fontWeight: 600, textTransform: 'uppercase' }}>{r.role}</span>
                {r.provider && <span style={chipColor}>{r.provider}</span>}
                {r.model && <span style={chipMuted}>{r.model}</span>}
                {r.tokens !== undefined && <span style={chipMuted}>{r.tokens} tok</span>}
                {r.latencyMs !== undefined && <span style={chipMuted}>{r.latencyMs}ms</span>}
                <span style={textMutedXs}>{new Date(r.timestamp).toLocaleString()}</span>
                <span style={{ ...textMutedXs, marginLeft: 'auto' }}><Hash size={10} style={{ display: 'inline', verticalAlign: 'middle' }} /> {r.sessionId.slice(0, 16)}</span>
              </div>
              <div style={{ ...textWhiteXs, fontSize: '0.78rem', lineHeight: 1.4, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
...
const btnSecondaryRed: React.CSSProperties = {
  ...btnSecondary,
  color: '#fca5a5',
