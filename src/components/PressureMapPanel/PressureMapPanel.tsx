...
const PressureMapPanel: React.FC = () => {
  ...
  if (!snapshot) {
    return (
      <div style={{ padding: 24, color: '#64748b', textAlign: 'center' }}>{t('pressure_map.loading_text')}</div>
    );
  }
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1400, margin: '0 auto', height: '100%', overflowY: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
        ...
        <h2 style={{ margin: 0, fontSize: 18, fontWeight: 600 }}>{t('pressure_map.runtime_title')}</h2>
      </div>
      <p style={{ margin: '0 0 1.5rem 0', fontSize: '0.8rem', color: '#64748b' }}>{t('pressure_map.subtitle')}</p>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
        ...
        <button onClick={() => setActiveTab('providers')}
          ...
          <Server size={14} /> {t('pressure_map.tab.providers', { count: snapshot.providers.length })}
        </button>
        <button onClick={() => setActiveTab('sessions')}
          ...
          <MessageCircle size={14} /> {t('pressure_map.tab.sessions', { count: snapshot.sessions.length })}
        </button>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 16, marginBottom: 20 }}>
        <div style={{ ...CARD, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: '0.7rem', color: '#64748b' }}>
            <span>{t('pressure_map.providers_count', { count: snapshot.providers.length })}</span>
            <span>{t('pressure_map.sessions_count', { count: snapshot.sessions.length })}</span>
            <span style={{ color: snapshot.alertCount > 0 ? '#ef4444' : '#22c55e' }}>{t('pressure_map.alerts_count', { count: snapshot.alertCount })}</span>
          </div>
        </div>

        <div style={{ ...CARD }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b', marginBottom: 8 }}>{t('pressure_map.trend')}</div>
          {trends.length === 0 ? (
            <div style={{ color: '#64748b', fontSize: '0.75rem', textAlign: 'center', padding: 20 }}>{t('pressure_map.no_trend')}</div>
          ) : (
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#ef4444' }}>
              {t('pressure_map.active_alerts', { count: alerts.filter(a => !a.acknowledged).length })}
            </span>
          </div>
      ...
      {activeTab === 'global' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div style={CARD}>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b', marginBottom: 8 }}>{t('pressure_map.providers_label')}</div>
            {snapshot.providers.length === 0 ? (
              <div style={{ color: '#64748b', fontSize: '0.75rem' }}>{t('pressure_map.no_providers')}</div>
            ) : (
          ...
          <div style={CARD}>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b', marginBottom: 8 }}>{t('pressure_map.sessions_label')}</div>
            {snapshot.sessions.length === 0 ? (
              <div style={{ color: '#64748b', fontSize: '0.75rem' }}>{t('pressure_map.no_sessions')}</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {activeTab === 'providers' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 12 }}>
          {snapshot.providers.length === 0 ? (
            <div style={{ ...CARD, textAlign: 'center', color: '#64748b', padding: 40 }}>{t('pressure_map.no_provider_data')}</div>
          ) : snapshot.providers.map(p => {
            const c = pLevelColor(p.level);
            return (
      ...
      {activeTab === 'sessions' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 12 }}>
          {snapshot.sessions.length === 0 ? (
            <div style={{ ...CARD, textAlign: 'center', color: '#64748b', padding: 40 }}>{t('pressure_map.no_session_data')}</div>
          ) : snapshot.sessions.map(s => {
            const c = pLevelColor(s.level);
            return (
