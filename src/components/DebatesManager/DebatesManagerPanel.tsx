...
const STATUS_COLORS: Record<string, string> = {
  active: '#22c55e', paused: '#f59e0b', completed: '#3b82f6',
  failed: '#ef4444', cancelled: '#6b7280', created: '#64748b',
  deliberating: '#22c55e', consensus: '#8b5cf6', summarizing: '#a855f7',
};
...
const badge: React.CSSProperties = {
  fontSize: 11, padding: '2px 8px', borderRadius: 10, fontWeight: 600, display: 'inline-block',
};
...
function getStatusGroup(s: DebateSessionMeta): string {
  ...
  if (s.phase === 'completed' || s.phase === 'failed' || s.phase === 'cancelled') return 'completed';
  return 'active';
}

const DUMMY_PARTICIPANTS = [
  { id: 'p1', name: 'Critic', role: 'con' as const },
  { id: 'p2', name: 'Advocate', role: 'pro' as const },
  { id: 'p3', name: 'Analyst', role: 'neutral' as const },
];

export const DebatesManagerPanel: React.FC = () => {
  const navigate = useNavigate();
  const { sessions, activeSessionId, isLoaded, createSession, deleteSession, archiveSession, unarchiveSession, pauseSession, resumeSession, renameSession, tagSession, moveToFolder, setActiveSessionId, refresh } = useDebateSessionStore();
  ...
  const filtered = useMemo(() => {
    let list = sessions;
    if (search) { const q = search.toLowerCase(); list = list.filter(s => s.topic.toLowerCase().includes(q) || s.tags.some(t => t.toLowerCase().includes(q))); }
    return list;
  }, [sessions, search]);
  ...
  const groupOrder = Object.keys(grouped).sort((a, b) => (STATUS_ORDER[a] ?? 9) - (STATUS_ORDER[b] ?? 9));

  const handleCreate = useCallback(async () => {
    if (!newTopic.trim()) return;
    await createSession(newTopic.trim(), 'round_robin', DUMMY_PARTICIPANTS, { roundDelayMs: 2000, maxTokens: 4096, temperature: 0.7, debateTemperature: 0.7, useModerator: false, timeoutMs: 30000, language: 'ru' });
    setNewTopic('');
    setShowCreate(false);
  ...
  const handleLink = useCallback(async () => {
    ...
    try {
      const sm = runtime.getService<ISessionManager>('sessionManagerService');
      await sm.link(active.id, linkInput.trim(), 'debate_to_chat', `Linked from debate ${active.topic}`);
    ...
    <div style={{ display: 'flex', height: '100%', background: '#0f172a', color: '#e2e8f0' }}>
      <div style={sidebar}>
        <div style={{ padding: '12px 16px', display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button style={{ ...btn, background: '#3b82f6', whiteSpace: 'nowrap', padding: '8px 14px' }} onClick={() => setShowCreate(true)}>+ New</button>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px' }}>
          {!isLoaded ? (
            <div style={{ textAlign: 'center', padding: 32, color: '#64748b' }}>{t('common.loading')}</div>
          ) : filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 32, color: '#64748b' }}>{search ? t('common.no_results') : t('common.empty')}</div>
              ...
              {grouped[group].map(s => (
                <div key={s.id} style={s.id === activeSessionId ? sessionItemActive : sessionItem}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: STATUS_COLORS[s.phase] || '#64748b', flexShrink: 0 }} />
                    <span style={{ fontSize: 13, fontWeight: 500, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.topic}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ ...badge, background: `${STATUS_COLORS[s.phase] || '#64748b'}22`, color: STATUS_COLORS[s.phase] || '#64748b' }}>{s.phase}</span>
                    <span style={phaseLabel}>R{s.round} · {s.participants.length} agents · {new Date(s.updatedAt).toLocaleDateString()}</span>
                  </div>
                  {s.tags.length > 0 && <div style={{ marginTop: 2 }}>{s.tags.map(t => <span key={t} style={tagChip}>{t}</span>)}</div>}
                </div>
              ))}
            </div>
          ))}
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        {showCreate && (
          <div style={detailSection}>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>{t('common.create')}</div>
            <input style={{ ...searchInput, marginBottom: 8 }} placeholder="Debate topic" value={newTopic} onChange={e => setNewTopic(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleCreate(); }} autoFocus />
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={{ ...btn, background: '#3b82f6' }} onClick={handleCreate}>Create</button>
              <button style={{ ...btn, background: '#475569' }} onClick={() => setShowCreate(false)}>Cancel</button>
            </div>
          </div>
        )}
        ...
        {!active && !showCreate && (
          <div style={emptyState}>
            ...
            <div style={{ fontSize: 13, textAlign: 'center', maxWidth: 300 }}>{t('common.select_item')}</div>
          </div>
        )}
          ...
          <>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  {editingId === active.id ? (
                    ...
                    <div style={{ fontSize: 18, fontWeight: 600, cursor: 'pointer' }} onClick={() => { setEditingId(active.id); setEditTitle(active.topic); }}>{active.topic} ✏️</div>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 8, marginLeft: 16 }}>
                  {(active.phase === 'paused' || active.phase === 'created') && (
                    <button style={{ ...btn, background: '#22c55e' }} onClick={() => resumeSession(active.id)}>▶ Resume</button>
                  )}
                  {active.phase === 'active' || active.phase === 'deliberating' ? (
                    <button style={{ ...btn, background: '#f59e0b' }} onClick={() => pauseSession(active.id)}>⏸ Pause</button>
                  ) : null}
                  <button style={{ ...btn, background: '#3b82f6' }} onClick={() => navigate(`/debate-runtime?sessionId=${active.id}`)}>Open</button>
                  {active.isArchived ? (
                  ...
                  <button style={{ ...btn, background: '#ef4444' }} onClick={async () => { if (window.confirm(`Delete "${active.topic}"?`)) { await deleteSession(active.id); } }}>Delete</button>
                </div>
              </div>
            </div>
            ...
            <div style={detailSection}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 13 }}>
                <div><span style={{ color: '#64748b' }}>Status:</span> <span style={{ color: STATUS_COLORS[active.phase] || '#e2e8f0' }}>{active.phase}</span></div>
                <div><span style={{ color: '#64748b' }}>Strategy:</span> {active.strategy}</div>
                <div><span style={{ color: '#64748b' }}>Round:</span> {active.round}</div>
                <div><span style={{ color: '#64748b' }}>Participants:</span> {active.participants.length}</div>
            ...
            <div style={detailSection}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
                {active.tags.map(t => (
                  <span key={t} style={tagChip}>{t} <span style={{ cursor: 'pointer', marginLeft: 4, opacity: 0.6 }} onClick={async () => { await tagSession(active.id, active.tags.filter(x => x !== t)); }}>✕</span></span>
                ))}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div style={detailSection}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {active.participants.map(p => (
                  <span key={p.id} style={{ padding: '4px 10px', borderRadius: 6, background: 'rgba(255,255,255,0.05)', fontSize: 12 }}>
                    {p.name} <span style={{ color: '#64748b' }}>({p.role})</span>
                  </span>
                ))}
                {active.participants.length === 0 && <span style={{ color: '#64748b', fontSize: 13 }}>No participants saved in metadata</span>}
              </div>
            </div>
            ...
            <div style={detailSection}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                <input style={{ ...searchInput, flex: 1 }} placeholder="Chat session ID to link..." value={linkInput} onChange={e => setLinkInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleLink(); }} />
                <button style={{ ...btn, background: '#8b5cf6' }} onClick={handleLink}>Link Chat</button>
              </div>
            </div>
          </>
        )}
