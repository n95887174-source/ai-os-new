...
import {
    ...
    type RouterDiffReport,
} from '../../kernel/services/projections/router-shadow-diff';
...
const KeyStateDiffView: React.FC<{
    report: DiffReport;
    mismatchesBySeverity: Record<string, DiffEntry[]>;
}> = ({ report, mismatchesBySeverity }) => {
    const driftColor =
    ...
    if (
        ...
        report.missingInLegacy.length === 0
    ) {
        return (
            <div
                style={{
            ...
            >
                ...
                <div style={{ fontSize: '0.8rem', color: '#64748b' }}>
                    Legacy KeyStateStore and event-sourced projection are identical
                </div>
            </div>
        );
    }
        ...
        <div>
            ...
            >
                <div style={{ ...CARD, borderLeft: `3px solid ${driftColor}` }}>
                    <div>
                        <BarChart3 size={16} color={driftColor} />
                        <span>Drift Score</span>
                    </div>
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: driftColor }}>
                        {report.driftScore}
                        <span style={{ fontSize: '0.8rem', color: '#64748b' }}>/100</span>
                    </div>
                    <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>
                        {report.criticalCount} critical / {report.mismatches.length} total
                    </div>
                </div>
                <div style={CARD}>
                    <div>
                        <Activity size={16} color="#3b82f6" />
                        <span>Keys Compared</span>
                    </div>
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#e2e8f0' }}>
                        {report.totalKeys}
                    </div>
                    <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>
                        {report.matchedKeys} matched
                    </div>
                </div>
                <div style={CARD}>
                    <div>
                        <AlertTriangle size={16} color="#f59e0b" />
                        <span>Missing in Projection</span>
                    </div>
                    <div
                        style={{
                            fontSize: '1.5rem',
                            fontWeight: 800,
                            color: report.missingInProjection.length > 0 ? '#f59e0b' : '#22c55e',
                        }}
                    >
                        {report.missingInProjection.length}
                    </div>
                ...
                <div style={CARD}>
                    <div>
                        ...
                        <span>Missing in Legacy</span>
                    </div>
                    <div
                        style={{
                            fontSize: '1.5rem',
                            fontWeight: 800,
                            color: report.missingInLegacy.length > 0 ? '#a78bfa' : '#22c55e',
                        }}
                    >
                        {report.missingInLegacy.length}
                    </div>
                    <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>
                        keys only in projection
                    </div>
                </div>
            </div>
            ...
            {report.missingInProjection.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                    ...
                    >
                        Keys Only in Legacy ({report.missingInProjection.length})
                    </div>
                ...
                return (
                    <div key={sev} style={{ marginBottom: 12 }}>
                        ...
                        {entries.slice(0, 5).map((m, _i) => (
                            ...
                            >
                                <span style={{ fontWeight: 700, color: '#e2e8f0', minWidth: 100 }}>
                                    {m.keyId}
                                </span>
                                <span
                                    style={{
                                ...
                                <span style={{ color: '#94a3b8' }}>
                                    legacy:{' '}
                                    <span style={{ color: '#fca5a5' }}>{String(m.legacy)}</span>
                                </span>
                                <span style={{ color: '#64748b' }}>→</span>
                                <span style={{ color: '#94a3b8' }}>
                                    projection:{' '}
                                    <span style={{ color: '#22c55e' }}>{String(m.projection)}</span>
                                </span>
                            </div>
                        ))}
                    </div>
    ...
    return (
        <div>
            ...
            >
                <div style={{ ...CARD, borderLeft: `3px solid ${driftColor}` }}>
                    <div>
                        <GitBranch size={16} color={driftColor} />
                        <span>Drift Score</span>
                    </div>
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: driftColor }}>
                        {report.driftScore}
                ...
                <div style={CARD}>
                    ...
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#e2e8f0' }}>
                        {report.totalProjected}
                    </div>
                    <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>
                        decisions in event projection
                    </div>
                </div>
                <div style={CARD}>
                    <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <div>
                    ...
                    {diffReport ? (
                        <KeyStateDiffView
                            report={diffReport}
                            mismatchesBySeverity={mismatchesBySeverity}
                        />
                    ) : (
                        <div
                            style={{ ...CARD, textAlign: 'center', padding: 24, color: '#64748b' }}
                        >
                            No data
                        </div>
                    )}
