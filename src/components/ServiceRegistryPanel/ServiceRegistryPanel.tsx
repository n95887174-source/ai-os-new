...
const ServiceRegistryPanel: React.FC = () => {
    ...
    const groupedServices = useMemo(() => {
        ...
        for (const s of services) {
            if (CORE_SERVICES.has(s)) {
                core.push(s);
                continue;
            }
            const matched = Object.keys(UI_PANELS).some((key) =>
        ...
        >
                    ...
                    {[
                        {
                            label: 'Phase',
                            value: status.phase,
                            color: status.phase === 'ready' ? '#22c55e' : '#eab308',
                        },
                        {
                            label: 'Services',
            ...
            {search ? (
                ...
                >
                    <ServiceGroup
                        label="Core"
                        services={groupedServices.core}
                        deps={deps}
                        uiPathFn={serviceHasUI}
                    />
                    <ServiceGroup
                        label="Has UI"
                        services={groupedServices.hasUi}
                        deps={deps}
                        uiPathFn={serviceHasUI}
                    />
                    <ServiceGroup
                        label="No UI"
                        services={groupedServices.noUi}
                        deps={deps}
                        uiPathFn={serviceHasUI}
                    />
                </div>
            ...
            {selectedService && deps[selectedService] && deps[selectedService].length > 0 && (
                ...
                >
                    <div style={{ color: '#94a3b8', marginBottom: 6 }}>
                        Dependencies of <span style={{ color: '#60a5fa' }}>{selectedService}</span>:
                    </div>
...
const ServiceGroup: React.FC<{
    ...
    uiPathFn: (name: string) => string | null;
}> = ({ label, services, deps, uiPathFn }) => {
        ...
        >
            <div
                ...
                style={{
                    ...
                    padding: '8px 12px',
                    cursor: 'pointer',
                    borderBottom: collapsed ? 'none' : '1px solid rgba(148,163,184,0.1)',
                }}
            >
                <ChevronRight
                    size={14}
...
const ServiceRow: React.FC<{ name: string; uiPath: string | null; deps: string[] }> = ({
    name,
    uiPath,
    deps,
}) => (
    ...
    >
        <Circle
            size={8}
            color={uiPath ? '#22c55e' : '#f59e0b'}
            fill={uiPath ? '#22c55e' : '#f59e0b'}
        />
        <span style={{ fontSize: 13, fontFamily: 'monospace', color: '#e2e8f0', flex: 1 }}>
            {name}
        </span>
        {uiPath && (
            <a
