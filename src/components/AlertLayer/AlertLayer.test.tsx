...
describe('AlertLayer', () => {
  ...
  it('shows alert count badge when alerts exist', async () => {
    mockGetAlerts.mockReturnValue([
      { id: 'a1', type: 'quota_warning', severity: 'critical', message: 'Quota exceeded', timestamp: Date.now(), resolved: false },
      { id: 'a2', type: 'rate_limit', severity: 'high', message: 'Rate limited', timestamp: Date.now(), resolved: false },
    ]);
    const AlertLayer = (await import('./AlertLayer')).default;
    render(<AlertLayer />);
  ...
  it('expands alert list on click', async () => {
    mockGetAlerts.mockReturnValue([
      { id: 'a1', type: 'quota_warning', severity: 'critical', message: 'Test alert', timestamp: Date.now(), resolved: false },
    ]);
    const AlertLayer = (await import('./AlertLayer')).default;
    render(<AlertLayer />);
