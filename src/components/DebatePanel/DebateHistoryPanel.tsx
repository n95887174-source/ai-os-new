...
import { Clock, MessageSquare, Trash2, ChevronDown, ChevronRight, ChevronsDown, Check, X, Search, RotateCcw, Archive, Filter, BarChart3, Swords, Shield, Scale } from 'lucide-react';
...
const DebateHistoryPanel: React.FC<DebateHistoryPanelProps> = ({ history, expandedHistory, onToggleExpand, onRefresh, t }) => {
  ...
  const handleRestore = (id: string) => {
    const session = debateService.restoreSession(id);
  ...
  if (history.length === 0) {
    return (
      <div className="glass-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: 24, border: '1px solid rgba(255,255,255,0.05)', padding: '2rem' }}>
        <h3 style={{ margin: 0, marginBottom: '1.5rem', fontSize: '1.2rem', fontWeight: 800, color: '#f8fafc', display: 'flex', alignItems: 'center', gap: 10 }}>
  ...
  return (
    <div className="glass-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
      {/* Header + Stats */}
      <div style={{ flexShrink: 0, padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800, color: '#f8fafc', display: 'flex', alignItems: 'center', gap: 10 }}>
            <Clock size={20} color="#3b82f6" /> {t('debate_runtime.title')} <span style={{ fontSize: '0.85rem', color: '#64748b', fontWeight: 600 }}>({filtered.length})</span>
          </h3>
          <button onClick={onRefresh} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', padding: 6, borderRadius: 6 }} title={t('common.refresh')} aria-label={t('common.refresh')}>
            <RotateCcw size={16} />
          </button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem' }}>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, padding: '0.35rem 0.75rem', borderRadius: 10, background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border)' }}>
            <Search size={14} color="#64748b" />
            <input
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder={t('debate.search_history') + '...'}
              style={{ flex: 1, background: 'none', border: 'none', color: '#e2e8f0', fontSize: '0.8rem', outline: 'none' }}
          ...
          <select
            ...
            style={{ padding: '0.35rem 0.6rem', borderRadius: 10, border: '1px solid var(--border)', background: 'rgba(0,0,0,0.3)', color: '#e2e8f0', fontSize: '0.75rem', outline: 'none' }}
        ...
        {stats && (
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <div style={{ padding: '0.4rem 0.7rem', borderRadius: 8, background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.15)', fontSize: '0.7rem', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 4 }}>
              <BarChart3 size={12} color="#3b82f6" /> {stats.total} {t('debate.debates')}
            </div>
            <div style={{ padding: '0.4rem 0.7rem', borderRadius: 8, background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.15)', fontSize: '0.7rem', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 4 }}>
              <MessageSquare size={12} color="#10b981" /> Ø {stats.avgArgs} {t('debate.arg_short')}
            </div>
            <div style={{ padding: '0.4rem 0.7rem', borderRadius: 8, background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.15)', fontSize: '0.7rem', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 4 }}>
              <ChevronsDown size={12} color="#f59e0b" /> Ø {stats.avgRounds} {t('debate.rounds')}
            </div>
            <div style={{ padding: '0.4rem 0.7rem', borderRadius: 8, background: 'rgba(168,85,247,0.1)', border: '1px solid rgba(168,85,247,0.15)', fontSize: '0.7rem', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 4 }}>
              <Filter size={12} color="#a855f7" /> {t('debate.longest')}: {stats.longestRounds} {t('debate.rounds')} / {stats.longestArgs} {t('debate.arg_short')}
            </div>
          </div>
        )}

        {searchQuery && filtered.length === 0 && (
          <div style={{ padding: '0.75rem', fontSize: '0.8rem', color: '#ef4444', textAlign: 'center' }}>
            {t('debate.no_results')}
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
              ...
              >
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    {/* Restore button */}
                    <button
                      onClick={(e) => { e.stopPropagation(); handleRestore(h.id); }}
                      style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8, padding: 6, color: '#10b981', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 700 }}
                    ...
                    {/* Archive button */}
                    <button
                      onClick={(e) => { e.stopPropagation(); debateService.archiveSession(h.id); onRefresh(); }}
                      style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 8, padding: 6, color: '#f59e0b', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 700 }}
                    ...
                    <button
                      ...
                      style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, padding: 6, color: '#ef4444', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 700 }}
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1rem' }}>
                          {(h.participants ?? []).map(p => (
                            <span key={p.id} style={{
                              ...
                              background: p.role === 'pro' ? 'rgba(59,130,246,0.15)' : p.role === 'con' ? 'rgba(239,68,68,0.15)' : 'rgba(148,163,184,0.15)',
                        ...
                        {agentOptions.length > 1 && (
                          <div style={{ marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <select
                              ...
                              style={{ padding: '0.25rem 0.5rem', borderRadius: 6, border: '1px solid var(--border)', background: 'rgba(0,0,0,0.3)', color: '#e2e8f0', fontSize: '0.7rem', outline: 'none' }}
                            ...
                            <span style={{ fontSize: '0.7rem', color: '#64748b' }}>
                              {filteredArguments.length}/{h.arguments?.length ?? 0} {t('debate.arguments')}
                            </span>
                          </div>
                        )}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                          {visibleArguments.map((arg) => (
                            <div key={arg.id} style={{ display: 'flex', gap: '0.75rem', padding: '0.5rem 0.75rem', borderRadius: 12, background: 'rgba(255,255,255,0.03)', alignItems: 'flex-start' }}>
                              <div style={{ width: 24, height: 24, borderRadius: 8, background: arg.position === 'pro' ? 'rgba(59,130,246,0.15)' : arg.position === 'con' ? 'rgba(239,68,68,0.15)' : 'rgba(148,163,184,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '0.65rem', color: arg.position === 'pro' ? '#3b82f6' : arg.position === 'con' ? '#ef4444' : '#94a3b8' }}>
                                {getPositionIcon(arg.position)}
                              </div>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.2rem' }}>
                                  <span style={{ fontSize: '0.78rem', fontWeight: 800, color: '#e2e8f0' }}>{arg.agentName || arg.agentId}</span>
                                  <span style={{ fontSize: '0.65rem', color: '#64748b' }}>Round {arg.round}</span>
                                  <span style={{ fontSize: '0.62rem', color: '#64748b' }}>{Math.round(arg.confidence * 100)}%</span>
                                  {arg.provider && <span style={{ fontSize: '0.62rem', color: '#475569' }}>{arg.provider}/{arg.model}</span>}
                                </div>
                                <div style={{ fontSize: '0.85rem', color: '#cbd5e1', lineHeight: 1.5 }}>{arg.content.length > 200 ? arg.content.slice(0, 200) + '...' : arg.content}</div>
                              </div>
                            </div>
                          ))}
                        </div>
                        ...
                        {filteredArguments.length > getArgCount(h.id) ? (
                          <button
                            ...
                            style={{ marginTop: '0.75rem', background: 'none', border: '1px solid var(--border)', borderRadius: 8, padding: '0.35rem 1rem', color: '#64748b', fontSize: '0.75rem', cursor: 'pointer', width: '100%', fontWeight: 600 }}
                          >
                          ...
                          <button
                            ...
                            style={{ marginTop: '0.75rem', background: 'none', border: 'none', color: '#64748b', fontSize: '0.7rem', cursor: 'pointer', width: '100%' }}
                          >
        ...
        {hasMore && (
          <button
            ...
            style={{ width: '100%', padding: '1rem', background: 'none', border: 'none', color: '#64748b', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}
          >
            <ChevronsDown size={16} />
            {t('debate.load_more').replace('{0}', String(filtered.length - visible.length))}
          </button>
        )}
      </div>
    </div>
