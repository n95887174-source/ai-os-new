...
import {
  Users, Play, Loader2, Bot, Activity, CheckCircle2, Zap,
} from 'lucide-react';
    ...
    <div style={{ flex: 1, display: 'flex', padding: '3rem', overflowY: 'auto' }}>
      <div style={{ width: '100%', maxWidth: 750, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
        <div style={textCenter}>
          <div style={{
            display: 'inline-flex', padding: '1.25rem', background: 'rgba(168,85,247,0.1)',
            borderRadius: '50%', marginBottom: '1.5rem', border: '1px solid rgba(168,85,247,0.2)',
          ...
          <p style={{ color: '#94a3b8', fontSize: '1rem', maxWidth: '500px', margin: '0 auto' }}>{t('debate.config_desc')}</p>
        </div>

        <div style={{
          display: 'flex', flexDirection: 'column', gap: '1.5rem',
          background: 'rgba(0,0,0,0.3)', padding: '2.5rem', borderRadius: 24,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            ...
            <div>
              <label className="debate-label debate-label--block" style={{ marginTop: 6 }}>
                {t('debate.temperature_label')}: {['Pure Logic','Mostly Logic','Slightly Logical','Analytical','Leaning Logic','Balanced','Leaning Emotion','Passionate','Very Emotional','Intense','Pure Emotion'][debateTemperature]}
              </label>
              <input
                  ...
                  return (
                    <div key={id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.78rem' }}>
                      ...
                      >
                        ...
                        <option value="emotional_only">{t('debate.constraint_emotional')}</option>
                        <option value="data_driven">{t('debate.constraint_data')}</option>
                        <option value="ethical_framework">{t('debate.constraint_ethical')}</option>
                        <option value="first_principles">{t('debate.constraint_first')}</option>
                        <option value="pragmatic">{t('debate.constraint_pragmatic')}</option>
                      </select>
                    </div>
                  );
                })}
              ...
              <div style={{ marginTop: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#94a3b8', marginBottom: '0.25rem' }}>
                  ...
                  <span style={{ marginLeft: 8, color: '#64748b', fontWeight: 400 }}>
                    {Array.from(probeResults.values()).filter(r => r.status === 'ready').length}/{probeResults.size} {t('debate.probe_ready')}
                  </span>
                </div>
                    ...
                    <div key={id}>
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: isExpanded ? '8px 8px 0 0' : 8, background: 'rgba(0,0,0,0.2)',
                          cursor: 'pointer', fontSize: '0.78rem',
                          border: isExpanded ? '1px solid rgba(168,85,247,0.12)' : '1px solid transparent',
                      ...
                      >
                        ...
                        {preview ? (
                          ...
                          <span style={{ color: '#64748b', fontSize: '0.7rem', fontStyle: 'italic', flex: 1, minWidth: 0 }}>{t('debate.probe_no_response')}</span>
                        )}
                        <span style={{ color: '#475569', fontSize: '0.6rem', flexShrink: 0 }}>{isExpanded ? '\u25B2' : '\u25BC'}</span>
                      </div>
                      {isExpanded && (
                        <div style={{
                          padding: '8px 12px', borderRadius: '0 0 8px 8px', background: 'rgba(0,0,0,0.15)',
                          border: '1px solid rgba(168,85,247,0.12)', borderTop: 'none', fontSize: '0.78rem',
                          color: '#cbd5e1', whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: 150, overflowY: 'auto', lineHeight: 1.4,
                        }}>
                          {r.responseContent || <span style={textSecondaryItalic}>{t('debate.probe_no_response')}</span>}
                        </div>
                      )}
                    </div>
                  );
          ...
          <button
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginTop: '1rem',
          ...
          <div style={textCenter}>
            <button
              ...
              className="btn-secondary"
              style={{ padding: '0.6rem 1.2rem', borderRadius: 10, display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: '0.9rem' }}
            >
