...
const DebateSidebar: React.FC<DebateSidebarProps> = ({ isOpen = true }) => {
  ...
  return (
    <div style={{
      ...
      background: 'rgba(15,23,42,0.85)', borderRight: '1px solid rgba(100,116,139,0.2)',
      overflow: 'hidden',
    }}>
      {/* Header */}
      <div style={{ padding: '0.75rem', borderBottom: '1px solid rgba(100,116,139,0.15)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          ...
          <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#f1f5f9' }}>Rooms</span>
          {!showNewInput && (
            <button onClick={() => setShowNewInput(true)} style={{
              marginLeft: 'auto', padding: '3px 8px', borderRadius: 6, border: '1px solid rgba(168,85,247,0.3)',
        ...
        {showNewInput && (
          <div style={{ display: 'flex', gap: 4, marginBottom: 8 }}>
            <input
              ...
              placeholder="Topic..."
              style={{
                flex: 1, padding: '0.35rem 0.6rem', borderRadius: 6, border: '1px solid rgba(168,85,247,0.3)',
                background: 'rgba(15,23,42,0.8)', color: '#e2e8f0', fontSize: '0.78rem', outline: 'none',
              }}
            />
            <button onClick={() => { setShowNewInput(false); setNewTopic(''); }} style={{
              padding: '0.35rem', borderRadius: 6, border: 'none', background: 'transparent',
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            placeholder="Search..."
            style={{
              width: '100%', padding: '0.35rem 0.7rem 0.35rem 1.85rem', borderRadius: 6,
              border: '1px solid rgba(100,116,139,0.2)', background: 'rgba(15,23,42,0.4)',
          ...
          {search && (
            <button onClick={() => setSearch('')} style={{
              ...
              padding: 0, border: 'none', background: 'transparent', color: '#64748b', cursor: 'pointer',
            }}>
              <X size={13} />
            </button>
          )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem' }}>
        {filteredRooms.length === 0 ? (
          <div style={{ padding: '2rem 0.5rem', textAlign: 'center', color: '#64748b', fontSize: '0.78rem' }}>
            {search ? 'No rooms match your search.' : 'No debate rooms yet.'}
          </div>
        ) : (
          Array.from(groups.entries()).map(([label, groupRooms]) => (
            <div key={label} style={{ marginBottom: 6 }}>
              <div style={{ padding: '0.2rem 0.5rem', fontSize: '0.6rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 2 }}>
                {label}
              </div>
              {groupRooms.map(room => (
                <div
                  ...
                  style={{
                    display: 'flex', alignItems: 'center', gap: 8, padding: '0.45rem 0.5rem',
                    borderRadius: 8, cursor: 'pointer', transition: 'all 0.15s',
                    background: room.id === currentRoomId ? 'rgba(168,85,247,0.12)' : hoveredRoomId === room.id ? 'rgba(255,255,255,0.03)' : 'transparent',
                    border: room.id === currentRoomId ? '1px solid rgba(168,85,247,0.2)' : '1px solid transparent',
                  }}
                >
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: PHASE_COLORS[room.status] || '#64748b', flexShrink: 0 }} />
