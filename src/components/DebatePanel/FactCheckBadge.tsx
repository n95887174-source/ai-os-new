...
const VERDICT_CONFIG: Record<FactVerdict, { icon: React.ReactNode; color: string; bg: string; label: string }> = {
  verified: { icon: <CheckCircle2 size={12} />, color: '#10b981', bg: 'rgba(16,185,129,0.12)', label: 'Verified' },
  disputed: { icon: <AlertTriangle size={12} />, color: '#f59e0b', bg: 'rgba(245,158,11,0.12)', label: 'Disputed' },
  false: { icon: <XCircle size={12} />, color: '#ef4444', bg: 'rgba(239,68,68,0.12)', label: 'False' },
  no_evidence: { icon: <HelpCircle size={12} />, color: '#64748b', bg: 'rgba(100,116,139,0.12)', label: 'Unverified' },
  pending: { icon: <Loader2 size={12} className="provider-spin" />, color: '#8b5cf6', bg: 'rgba(139,92,246,0.12)', label: 'Checking...' },
  error: { icon: <AlertTriangle size={12} />, color: '#ef4444', bg: 'rgba(239,68,68,0.12)', label: 'Error' },
};
...
export const FactCheckBadge: React.FC<FactCheckBadgeProps> = ({ argumentId }) => {
  ...
  const dominantVerdict = (Object.entries(verdictCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || 'no_evidence') as FactVerdict;
          ...
          {results.map((r, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                background: 'rgba(30,30,50,0.6)', border: `1px solid ${vc.color}20`,
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
                  <span style={{ color: vc.color, display: 'flex' }}>{vc.icon}</span>
                  <span style={{ fontSize: 10, fontWeight: 600, color: vc.color }}>{vc.label}</span>
                ...
                {r.reasoning && (
                  <div style={{ fontSize: 9, color: '#64748b', marginTop: 2, fontStyle: 'italic' }}>
                    {r.reasoning}
                  </div>
                )}
              </div>
