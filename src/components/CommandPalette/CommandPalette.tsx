...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ open, onClose, t }) => {
  ...
  const grouped = !query.trim() && recentItems.length > 0
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            <span><kbd style={kbdStyle}>↑↓</kbd> {t('palette.nav')}</span>
            <span><kbd style={kbdStyle}>↵</kbd> {t('palette.select')}</span>
            <span><kbd style={kbdStyle}>ESC</kbd> {t('palette.close')}</span>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
