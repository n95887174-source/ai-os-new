  ...
  return (
    <div className="glass-panel" style={{ padding: '1.25rem', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem' }}>
        ...
        <span style={{ fontSize: '0.85rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)' }}>Анализ лимитов (Прогноз)</span>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.8rem' }}>
            <span style={{ color: 'var(--text-muted)' }}>Дневной бюджет токенов</span>
        ...
        <div style={{ background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.03)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 600 }}>ОСТАЛОСЬ ВРЕМЕНИ (ПРИМ.)</div>
              <div style={{ fontSize: '1.1rem', fontWeight: 900 }}>~{hoursRemaining} ч.</div>
            </div>
          </div>
        </div>
        ...
        {percent > 70 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#f59e0b', fontSize: '0.7rem' }}>
            <AlertCircle size={12} />
            <span>Обнаружен высокий расход. Система может активировать троттлинг.</span>
          </div>
        )}
      </div>
    </div>
