...
function layoutTopology(topo: ISTopology): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  const graphNodes: GraphNode[] = nodes.map(n => {
    ...
    const icon = n.id === 'entry'
      ? <Bot size={16} />
      : (NODE_ICONS[n.type] ?? <Brain size={16} />);
...
const IntelligenceGraph: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubActive = eventBus.onSafe<Record<string, unknown>>('cognitive:step:active', (d) => {
      if (d?.nodeId) {
    ...
    const unsubComplete = eventBus.onSafe<Record<string, unknown>>('cognitive:step:completed', (d) => {
      if (d?.nodeId) {
  ...
  if (!topology) {
    return (
      <div style={{ width: '100%', height: 320, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
        No topology mounted
      </div>
    );
  }
  ...
  return (
    <div style={{ width: '100%', height: 320, position: 'relative', overflow: 'hidden' }}>
          ...
          return (
            <g key={node.id}>
              <motion.circle
                ...
                r={isAgent ? 24 : 18}
                fill="rgba(0,0,0,0.4)"
                stroke={node.status === 'active' ? '#3b82f6' : node.status === 'error' ? '#ef4444' : 'rgba(255,255,255,0.1)'}
                strokeWidth={2}
              ...
              <foreignObject x={node.x - 12} y={node.y - 12} width={24} height={24}>
                <div style={{ color: node.status === 'active' ? '#3b82f6' : node.status === 'error' ? '#ef4444' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {node.icon}
                </div>
              </foreignObject>
              <text
      ...
      <div style={{ position: 'absolute', bottom: 10, left: 10, fontSize: '0.6rem', color: 'var(--text-muted)', display: 'flex', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#3b82f6' }} /> Active
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#ef4444' }} /> Error
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <div style={{ width: 10, height: 2, background: 'rgba(255,255,255,0.1)' }} /> Error Edge
        </div>
      </div>
    </div>
  );
