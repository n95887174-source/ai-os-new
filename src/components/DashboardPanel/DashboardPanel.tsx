                ...
                >
                    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                            ...
                            <div style={progressBar8}>
                                <div
                                    style={{
                                        ...
                                        height: '100%',
                                        background:
                                            providerCounts.error > 0 ? '#ef4444' : '#10b981',
                                        borderRadius: 4,
                            ...
                            <div style={progressBar8}>
                                {(() => {
                                    const maxQuota = Math.max(
                                        ...
                                        ...keys.map(
                                            (k) =>
                                                FREE_TIER_LIMITS[k.provider]?.requestsPerDay || 1,
                                        ),
                                    );
                                    const totalUsed = keys.reduce(
                                        (s, k) =>
                                            s + (k.stats?.extended?.usageToday?.requests || 0),
                                        0,
                                    );
                        ...
                        <div style={flex1Min100}>
                            ...
                            <div style={progressBar8}>
                                {(() => {
                                    const avgLat =
                                        ...
                                        Math.max(1, keys.filter((k) => k.latency).length);
                                    return (
                                        <div
                                            style={{
                            ...
                            <div style={textSmMutedMarginTop}>
                                {keys.filter((k) => k.latency).length > 0
                                    ? `${Math.round(keys.filter((k) => k.latency).reduce((s, k) => s + (k.latency || 0), 0) / Math.max(1, keys.filter((k) => k.latency).length))}${t('dashboard.ms_avg')}`
                                    : t('dashboard.dash')}
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        >
                            <div style={metricBox}>
                                ...
                                <div
                                    style={{
                                        ...
                                        fontWeight: 800,
                                        color:
                                            rps > 10 ? '#10b981' : rps > 3 ? '#f59e0b' : '#64748b',
                                    }}
                                >
                                    {rps}
                                </div>
                            ...
                            <div style={metricBox}>
                                ...
                                >
                                    {keys.filter((k) => k.latency).length > 0
                                        ? `${Math.round(keys.filter((k) => k.latency).reduce((s, k) => s + (k.latency || 0), 0) / Math.max(1, keys.filter((k) => k.latency).length))}${t('chat.latency_ms')}`
                                        : t('dashboard.dash')}
                                </div>
                            </div>
                            <div style={metricBox}>
