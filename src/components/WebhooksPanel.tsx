    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: '#94a3b8', margin: 0, fontSize: '0.85rem' }}>{t('webhooks.subtitle')}</p>
        </div>
        <button onClick={() => setShowForm(!showForm)} style={{ ...button, display: 'flex', alignItems: 'center', gap: 6, background: showForm ? '#64748b' : '#a855f7' }}>
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} style={{ padding: '1.5rem', borderRadius: 16, border: '1px solid rgba(168,85,247,0.2)', background: 'rgba(168,85,247,0.03)' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: 700, color: '#e2e8f0', margin: '0 0 1rem' }}>{t('webhooks.form_title')}</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            <div>
              <div style={textSecondaryXs}>{t('webhooks.form_name')}</div>
              <input type="text" value={formName} onChange={e => setFormName(e.target.value)} placeholder={t('webhooks.name_placeholder')} style={input} />
            </div>
            <div>
              <div style={textSecondaryXs}>{t('webhooks.form_url')}</div>
              <input type="text" value={formUrl} onChange={e => setFormUrl(e.target.value)} placeholder={t('webhooks.url_placeholder')} style={input} />
            </div>
            <div>
              <div style={textSecondaryXs}>{t('webhooks.form_provider')}</div>
              <select value={formProvider} onChange={e => setFormProvider(e.target.value as WebhookProvider)} style={selectBase}>
          ...
          <div style={{ marginBottom: '1rem' }}>
            <div style={{ ...textSecondaryXs, marginBottom: '0.5rem' }}>{t('webhooks.form_events')}</div>
            <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
              {WEBHOOK_EVENTS.map(evt => (
                <button key={evt} onClick={() => toggleEvent(evt)} style={{ padding: '0.3rem 0.6rem', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: formEvents.includes(evt) ? 'rgba(168,85,247,0.2)' : 'transparent', color: formEvents.includes(evt) ? '#c084fc' : '#94a3b8', cursor: 'pointer', fontSize: '0.75rem' }}>
                  {evt}
                </button>
              ))}
            </div>
          </div>
          <button onClick={handleAdd} style={{ ...button, background: '#a855f7', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div key={wh.id} style={{ padding: '1rem 1.25rem', borderRadius: 12, border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.15)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ padding: '0.25rem 0.5rem', borderRadius: 6, fontSize: '0.7rem', fontWeight: 600, background: wh.provider === 'slack' ? 'rgba(74, 74, 74, 0.2)' : wh.provider === 'telegram' ? 'rgba(0, 136, 204, 0.15)' : 'rgba(114, 137, 218, 0.15)', color: wh.provider === 'slack' ? '#9ca3af' : wh.provider === 'telegram' ? '#60a5fa' : '#818cf8', textTransform: 'uppercase' }}>
                      {wh.provider}
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                    <button onClick={() => handleTest(wh.id)} disabled={testing === wh.id} style={{ padding: '0.35rem 0.6rem', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#94a3b8', cursor: 'pointer', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: 4 }}>
                      {testing === wh.id ? <Loader2 size={14} /> : <Play size={14} />}
                      {t('webhooks.test')}
                    </button>
                    {testResult && (
                      <span style={{ fontSize: '0.75rem', color: testResult.ok ? '#10b981' : '#ef4444', fontWeight: 600 }}>
                        {testResult.ok ? `${t('webhooks.ok')}${testResult.status ? ` (${testResult.status})` : ''}` : `${t('webhooks.fail')}${testResult.status ? ` (${testResult.status})` : ''}`}
                      </span>
                    )}
                    <button onClick={() => handleToggle(wh)} style={{ padding: '0.35rem 0.6rem', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: '0.75rem', background: wh.enabled ? 'rgba(16,185,129,0.15)' : 'rgba(100,116,139,0.15)', color: wh.enabled ? '#10b981' : '#64748b', fontWeight: 600 }}>
                      {wh.enabled ? t('webhooks.enabled') : t('webhooks.disabled')}
                    </button>
                    <button onClick={() => handleRemove(wh.id)} style={dismissBtnRed} title={t('webhooks.remove')}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap' }}>
                  {wh.events.map(evt => (
                    <span key={evt} style={{ padding: '0.15rem 0.4rem', borderRadius: 4, fontSize: '0.65rem', background: 'rgba(168,85,247,0.1)', color: '#a78bfa' }}>
                      {evt}
                    </span>
                  ))}
                </div>
