...
import {
  ReactFlow, Controls, Background, Panel, MarkerType,
  type Node, type Edge,
} from '@xyflow/react';
        ...
        label: (
          <div style={{ maxWidth: 260, fontSize: 11, lineHeight: 1.4, color: '#e2e8f0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
              <span style={{ background: statusColor, width: 8, height: 8, borderRadius: '50%', display: 'inline-block' }} />
              <span style={{ fontWeight: 600, fontSize: 10, color }}>{c.speaker}</span>
              <span style={{ color: '#64748b', fontSize: 9 }}>r{c.round}</span>
              <span style={{ marginLeft: 'auto', fontSize: 9, padding: '1px 6px', borderRadius: 999, background: `${statusColor}22`, color: statusColor, border: `1px solid ${statusColor}44` }}>
                {c.status}
              </span>
            </div>
            <div>{shortText}</div>
            <div style={{ marginTop: 4, display: 'flex', gap: 8, fontSize: 9, color: '#64748b' }}>
...
function mapGovStateToEdges(state: GovernorState): Edge[] {
  ...
  for (const e of state.graph.edges) {
    ...
    edges.push({
      ...
      style: { stroke: color, strokeWidth: e.type === 'challenges' ? 2 : 1, strokeDasharray: e.type === 'refines' ? '4 2' : undefined },
...
const ArgumentGraphPanel: React.FC = () => {
  const [govState, setGovState] = useState<GovernorState | null>(debateService.getGovernorState());
  ...
  const filteredState = useMemo(() => {
    ...
    const edges = govState.graph.edges.filter(e => claimIds.has(e.from) && claimIds.has(e.to));
    const contradictions = showContradictions
      ? govState.contradictions
      : [];
  ...
  if (!govState || Object.keys(govState.graph.claims).length === 0) {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', background: '#0f172a', color: '#f1f5f9' }}>
        <div style={{ ...flexBetween, ...panel, margin: '0.75rem', borderRadius: 8, background: '#1e293b' }}>
          <span style={{ ...label }}>Argument Graph</span>
          <span style={{ ...textMutedXs }}>{govState ? 'No claims yet' : 'No active debate'}</span>
        </div>
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', ...emptyState }}>
          <div style={{ textAlign: 'center', maxWidth: 400 }}>
            <div style={{ fontSize: '2rem', marginBottom: '0.5rem', opacity: 0.3 }}>◈</div>
            <div style={{ ...textMuted }}>No debate claims to visualize. Start a debate in the Debate Panel first.</div>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', background: '#0f172a', color: '#f1f5f9', position: 'relative' }}>
      ...
      <div style={{ ...flexBetween, ...panel, margin: '0.75rem 0.75rem 0', borderRadius: 8, background: '#1e293b' }}>
          ...
          <div style={{ display: 'flex', gap: 4, fontSize: 10 }}>
            ...
            {neutralCount > 0 && <span style={{ color: '#10b981' }}>neutral {neutralCount}</span>}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => setShowContradictions(v => !v)} style={{ ...buttonGhost, fontSize: 10, background: showContradictions ? 'rgba(239,68,68,0.1)' : undefined }}>
      ...
      <div style={flex1RelativeMargin075}>
        ...
        >
          ...
          <Controls style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: 4, fill: '#fff' }} />
          <Panel position="bottom-left" style={{ background: 'rgba(15,23,42,0.85)', padding: '6px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)', fontSize: 10, display: 'flex', gap: 10 }}>
      ...
      {showDetail && selectedClaim && (
        <div
          style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
        ...
        >
          <div style={{ ...panel, maxWidth: 560, width: '90%', background: '#1e293b' }} onClick={e => e.stopPropagation()}>
            <div style={{ ...flexBetween, marginBottom: 12 }}>
              ...
              <button onClick={() => setShowDetail(false)} style={{ ...iconBtn, color: '#94a3b8' }}>✕</button>
            </div>
            <div style={{ fontSize: 13, lineHeight: 1.6, color: '#e2e8f0', marginBottom: 12 }}>
              {selectedClaim.text}
            </div>
