    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: '#94a3b8', margin: 0, fontSize: '0.85rem' }}>{t('rotations.subtitle')}</p>
        </div>
      </div>
      ...
      {keys.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#64748b', fontSize: '0.9rem', fontStyle: 'italic', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: 16 }}>
          ...
          <div style={{ fontWeight: 600, marginBottom: '0.5rem', color: '#94a3b8' }}>{t('rotations.empty')}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {keys.map(key => {
            ...
            return (
              <div key={key.id} style={{ padding: '1rem 1.25rem', borderRadius: 12, border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.15)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ padding: '0.25rem 0.5rem', borderRadius: 6, fontSize: '0.7rem', fontWeight: 600, background: 'rgba(59, 130, 246, 0.15)', color: '#60a5fa', textTransform: 'uppercase' }}>
                      {key.provider}
                    </div>
                    <div style={{ fontWeight: 600, color: '#f1f5f9', fontSize: '0.9rem' }}>{key.label}</div>
                    <div style={{ padding: '0.15rem 0.4rem', borderRadius: 4, fontSize: '0.65rem', background: status.active ? 'rgba(16,185,129,0.15)' : 'rgba(100,116,139,0.15)', color: status.active ? '#10b981' : '#64748b' }}>
                      {status.active ? t('rotations.active') : t('rotations.inactive')}
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', gap: '0.3rem', alignItems: 'center' }}>
                    <input
                      ...
                      onChange={e => setTtlInputs(prev => ({ ...prev, [key.id]: e.target.value }))}
                      placeholder={t('rotations.ttl_placeholder')}
                      style={{ ...input, width: 80, padding: '0.3rem 0.5rem', fontSize: '0.75rem' }}
                      min={1}
                    />
                    <button onClick={() => handleSetTTL(key.id)} style={{ ...buttonSm, background: '#3b82f6' }}>{t('rotations.set_ttl')}</button>
                  </div>
                  {status.active && (
                    <button onClick={() => handleCancelRotation(key.id)} style={{ ...buttonSm, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#94a3b8' }}>{t('rotations.cancel')}</button>
                  )}
                  <button onClick={() => handleRotateNow(key.id)} disabled={rotating === key.id} style={{ ...buttonSm, background: '#f59e0b', display: 'flex', alignItems: 'center', gap: 4 }}>
                    {rotating === key.id ? <Loader2 size={14} /> : <RotateCcw size={14} />}
                    {t('rotations.rotate_now')}
                  </button>
                  <button onClick={() => toggleHistory(key.id)} style={{ ...buttonSm, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ marginTop: '0.75rem', padding: '0.75rem', borderRadius: 8, background: 'rgba(0,0,0,0.2)' }}>
                      {history.length === 0 ? (
                        <div style={textMutedXs}>{t('rotations.no_history')}</div>
                      ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', maxHeight: 200, overflowY: 'auto' }}>
                          {history.map(evt => (
                            <div key={evt.id} style={{ fontSize: '0.75rem', display: 'flex', justifyContent: 'space-between', padding: '0.3rem 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                              <span style={{ color: '#94a3b8' }}>{new Date(evt.timestamp).toLocaleString()}</span>
                              <span style={{ padding: '0.1rem 0.3rem', borderRadius: 3, fontSize: '0.65rem', background: evt.result === 'success' ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)', color: evt.result === 'success' ? '#10b981' : '#ef4444' }}>
                                {evt.type}
                              </span>
                              <span style={{ color: '#64748b' }}>{evt.fromStatus} → {evt.toStatus}</span>
                            </div>
                          ))}
                        </div>
                      )}
