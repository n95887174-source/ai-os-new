        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                {consistencyReport && (
                    ...
                    >
                        ...
                        <span style={{ color: '#64748b' }}>
                            drift{' '}
                            {consistencyReport.driftScore > 0.01
                                ? (consistencyReport.driftScore * 100).toFixed(0) + '%'
                                : '0%'}
                        </span>
                        <span style={{ color: '#64748b' }}>
                            {consistencyReport.mismatches.length} mismatch
                            {consistencyReport.mismatches.length !== 1 ? 'es' : ''}
                        </span>
                        {consistencyReport.mismatches.length > 0 && (
                            <span style={{ color: '#94a3b8' }}>
                                (
                                {
                                    consistencyReport.mismatches.filter(
                ...
                {consistencyReport && consistencyReport.mismatches.length > 0 && (
                    ...
                    >
                        {consistencyReport.mismatches.slice(0, 5).map((m, _i) => (
                            <div
                            ...
                            >
                                {m.provider}.{m.field}: kernel={String(m.kernelValue).slice(0, 30)}{' '}
                                proj={String(m.projectionValue).slice(0, 30)} [{m.severity}]
                            </div>
                        ))}
                    </div>
                )}
                ...
                >
                    ...
                    {filtered.map((t) => {
                        ...
                        return (
                            <button
                                key={t.causalId}
                                onClick={() => handleSelect(t.causalId)}
                                style={{
                                    ...CARD,
                    ...
                    >
                        ...
                        <div style={detailGrid2}>
                            <div style={CARD}>
                                ...
                                <pre style={preBlockMono}>
                                    {JSON.stringify(
                                        selectedTrace.entry.before.keyState.data,
                                        null,
                                        2,
                                    ).slice(0, 2000)}
                                </pre>
                            ...
                            <div style={CARD}>
                                ...
                                <pre style={preBlockMono}>
                                    {JSON.stringify(
                                        selectedTrace.entry.before.routerState.data,
                                        null,
                                        2,
                                    ).slice(0, 2000)}
                                </pre>
                        ...
                        <div style={detailGrid2}>
                            <div style={CARD}>
                                ...
                                <pre style={preBlockMono}>
                                    {JSON.stringify(
                                        selectedTrace.entry.after.keyState.data,
                                        null,
                                        2,
                                    ).slice(0, 2000)}
                                </pre>
                            ...
                            <div style={CARD}>
                                ...
                                <pre style={preBlockMono}>
                                    {JSON.stringify(
                                        selectedTrace.entry.after.routerState.data,
                                        null,
                                        2,
                                    ).slice(0, 2000)}
                                </pre>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                    ...
                                    <div style={flexCenterGap8}>
                                        ...
                                        >
                                            {replayTrace.frames.map((f, i) => (
                                                <div
                                                    ...
                                                    style={{
                                                        position: 'absolute',
                                                        left: `${replayTrace.frames.length > 1 ? (i / (replayTrace.frames.length - 1)) * 100 : 50}%`,
                                                        top: 0,
                                                        width: 6,
                                                        height: 8,
                                                        borderRadius: 3,
                                                        background: f.rescored
                                                            ? '#22d3ee'
                                                            : 'rgba(255,255,255,0.15)',
                                                        transform: 'translateX(-50%)',
                                                        ...(replayTrace.flipFrame === f.index
                                                            ? {
                                                                  background: '#f59e0b',
                                                                  width: 8,
                                                                  height: 10,
                                            ...
                                            <div
                                                style={{
                                                    position: 'absolute',
                                                    left: `${replayTrace.frames.length > 1 ? (replayFrame / (replayTrace.frames.length - 1)) * 100 : 50}%`,
                                                    top: -3,
                                                    width: 12,
                                                    height: 14,
                                                    borderRadius: 2,
                                        ...
                                        <FastForward
                                            ...
                                            style={{ cursor: 'pointer' }}
                                            onClick={() =>
                                                setReplayFrame(replayTrace.frames.length - 1)
                                            }
                                        />
                                    </div>
                                    ...
                                    {replayTrace.frames.some((f) => f.scoreState) &&
                                        (() => {
                                            ...
                                            if (!cf?.scoreState)
                                                return (
                                                    <div style={textXxsMuted}>
                                                        No score data at this frame
                                                    </div>
                                                );
                                            const maxScore = Math.max(
                                                ...Object.values(cf.scoreState.scores),
                                                1,
                                            );
                                            return (
                                                <div
                                                ...
                                                >
                                                    ...
                                                    {Object.entries(cf.scoreState.scores).map(
                                                        ([provider, score]) => {
                                                            const isLeader =
                                                                cf.scoreState!.ranking[0] ===
                                                                provider;
                                                            return (
                                                                <div
                                                                    key={provider}
                                    ...
                                    {/* Flip point info */}
                                    {replayTrace.flipFrame !== null &&
                                        replayFrame >= replayTrace.flipFrame && (
                                            <div
                                                style={{
                                                    fontSize: '0.65rem',
                                            ...
                                            >
                                                Flip point reached at Frame{' '}
                                                {replayTrace.flipFrame + 1} — {replayTrace.winner}{' '}
                                                took the lead
                                            </div>
                                        )}
