...
export const MemoryContextPanel: React.FC<MemoryContextPanelProps> = ({ isOpen, onClose }) => {
    ...
    const [recentEntries] = useState(() => {
        try {
            return memoryService.getMemories(10);
        } catch {
            return [];
        }
    });
    ...
    const totalEntries = (() => {
        try {
            return memoryService.getMemories().length;
        } catch {
            return 0;
        }
    })();
                    ...
                    <div style={{ marginBottom: 16 }}>
                        ...
                        {results.map((r, i) => (
                            ...
                            >
                                ...
                                <div
                                    style={{ fontSize: '0.65rem', color: '#475569', marginTop: 4 }}
                                >
                                    {r.matchedOn} ·{' '}
                                    {new Date(
                                        r.entry.metadata?.timestamp || 0,
                                    ).toLocaleDateString()}
                                </div>
                            </motion.div>
                        ))}
