...
const mockTraces = [
  { id: 'trace-1', traceId: 'tr-1', startTime: Date.now() - 10000, input: 'Test request', status: 'completed', steps: [{ id: 's1', label: 'Step 1', status: 'done', timestamp: Date.now() }], decisionGraph: { nodes: [], edges: [] }, totalLatency: 500, totalTokens: 100, estimatedCost: 0.01, semanticConfidence: 0.95 },
  { id: 'trace-2', traceId: 'tr-2', startTime: Date.now() - 5000, input: 'Another request', status: 'running', steps: [{ id: 's2', label: 'Step 1', status: 'active', timestamp: Date.now() }], decisionGraph: { nodes: [], edges: [] }, totalLatency: 0, totalTokens: 0, estimatedCost: 0, semanticConfidence: 0.8 },
...
describe('TracesPanel', () => {
  ...
  it('search filters traces', async () => {
    ...
    render(<TracesPanel />);
    await screen.findByText('Test request');
    const search = document.querySelector('input[placeholder*="Search traces"]') as HTMLInputElement;
