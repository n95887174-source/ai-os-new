...
type SessionItem = {
  ...
  preview?: string; linkedCount: number; linkedId?: string; isPinned?: boolean;
};
...
export const SessionHubPanel: React.FC = () => {
  ...
  const [ctxMenu, setCtxMenu] = useState<{ x: number; y: number; item: SessionItem } | null>(null);
  ...
  const items = useMemo((): SessionItem[] => {
    ...
    for (const s of chatSessions) {
      ...
      result.push({
        ...
        linkedId: s.linkedDebateId, isPinned: s.isPinned,
      });
    }
    for (const s of debateMeta) {
      result.push({
        ...
        linkedId: s.linkedSessionIds[0], isPinned: s.isPinned,
      });
    }
  ...
  const handleRename = useCallback((id: string, type: 'chat' | 'debate') => {
  ...
  const handleRenameSubmit = useCallback((id: string, type: 'chat' | 'debate') => {
    if (editTitle.trim()) {
  ...
  const handlePinToggle = useCallback((id: string, type: 'chat' | 'debate') => {
    if (type === 'chat') chatPinSession(id);
    else debatePinSession(id);
  }, [chatPinSession, debatePinSession]);

  const handleDelete = useCallback((id: string, type: 'chat' | 'debate') => {
    if (type === 'chat') chatDeleteSession(id);
    else debateDeleteSession(id);
  }, [chatDeleteSession, debateDeleteSession]);

  const getContextMenuActions = useCallback((item: SessionItem): ContextMenuAction[] => [
    {
    ...
    {
      id: 'pin', label: item.isPinned ? 'Unpin' : 'Pin', icon: <Pin size={14} />,
      onClick: () => handlePinToggle(item.id, item.type),
    },
    {
      id: 'delete', label: 'Delete', icon: <Trash2 size={14} />, danger: true, divider: true,
      onClick: () => handleDelete(item.id, item.type),
    },
  ], [handleOpen, handleRename, handlePinToggle, handleDelete]);

  const handleKeyDown = useCallback((e: React.KeyboardEvent, id: string, type: 'chat' | 'debate') => {
    if (e.key === 'Enter') { e.preventDefault(); handleRenameSubmit(id, type); }
            ...
            {filtered.map(item => {
              ...
              return (
                <div
                  ...
                  style={cardBase}
                  onClick={() => { if (!isEditing) handleOpen(item); }}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                    {isEditing ? (
                      <input
                        ...
                        onBlur={() => handleRenameSubmit(item.id, item.type)}
                        onKeyDown={(e) => handleKeyDown(e, item.id, item.type)}
                        onClick={e => e.stopPropagation()}
                      />
                    ) : (
                      <span style={{ fontSize: 14, fontWeight: 600, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}>
                        {item.isPinned && <Star size={12} color="#f59e0b" fill="#f59e0b" />}
                        {item.title}
                      </span>
                    )}
                    <span style={{
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 11, color: '#64748b' }}>
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); handlePinToggle(item.id, item.type); }}
                      style={{ marginLeft: 'auto', padding: 2, background: 'transparent', border: 'none', cursor: 'pointer', color: item.isPinned ? '#f59e0b' : '#475569', display: 'flex' }}
