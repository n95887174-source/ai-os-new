...
const ALL_PERMISSIONS: RolePermission[] = [
  ...
  'agents:spawn', 'agents:manage',
  'roles:manage',
  'connectors:use', 'connectors:manage',
  'skills:use', 'skills:manage',
  'settings:read', 'settings:write',
  'system:admin', 'system:monitor',
  'debate:participate', 'orchestration:design',
];
...
export const PermissionMatrix: React.FC<PermissionMatrixProps> = ({ roles, onUpdate }) => {
  ...
  const togglePerm = useCallback((roleId: string, perm: RolePermission) => {
  ...
  const getCellFromEvent = (e: React.MouseEvent): { row: number; col: number } | null => {
    ...
    for (const cell of cells) {
      ...
      if (e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom) {
        return { row: parseInt(cell.getAttribute('data-row') || '0'), col: parseInt(cell.getAttribute('data-col') || '0') };
      }
    }
    return null;
  };
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ padding: '0.5rem', background: 'rgba(168,85,247,0.15)', borderRadius: 10, border: '1px solid rgba(168,85,247,0.3)' }}>
            <ShieldCheck size={20} color="#a855f7" />
          </div>
          <div>
            <h3 style={{ fontSize: '0.95rem', fontWeight: 800, color: '#f8fafc', margin: 0 }}>Permission Matrix</h3>
            <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: 0 }}>
              {roles.length} roles × {ALL_PERMISSIONS.length} permissions — drag to select range
            </p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <button
            ...
            style={{ padding: '0.3rem 0.6rem', fontSize: '0.7rem', fontWeight: 700, background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: 6, color: '#60a5fa', cursor: 'pointer' }}
          >
            Presets
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); selectAll(); }}
            style={{ padding: '0.3rem 0.6rem', fontSize: '0.7rem', fontWeight: 700, background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 6, color: '#10b981', cursor: 'pointer' }}
          >
            All
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); selectNone(); }}
            style={{ padding: '0.3rem 0.6rem', fontSize: '0.7rem', fontWeight: 700, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 6, color: '#ef4444', cursor: 'pointer' }}
          >
            None
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); exportMatrix(); }}
            style={{ padding: '0.3rem', borderRadius: 6, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8', cursor: 'pointer' }}
            title="Export JSON"
          >
            <Download size={14} />
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); copyMatrix(); }}
            style={{ padding: '0.3rem', borderRadius: 6, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8', cursor: 'pointer' }}
        ...
        {showPresets && (
          ...
          >
            <div style={{ padding: '0.75rem 1.5rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
              <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: 700, marginRight: '0.5rem' }}>Apply category defaults to all roles:</span>
              {Object.keys(PERM_CATEGORIES).map(cat => (
                <button
                  key={cat}
                  onClick={() => applyPreset(cat)}
                  style={{ padding: '0.25rem 0.5rem', fontSize: '0.65rem', fontWeight: 700, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, color: '#e2e8f0', cursor: 'pointer' }}
                >
                  {cat}
                </button>
              ))}
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: `200px repeat(${roles.length}, 60px)`, gap: 0, marginBottom: '0.25rem', position: 'sticky', top: 0, zIndex: 2, background: 'rgba(15,23,42,0.95)', backdropFilter: 'blur(8px)' }}>
                ...
                {roles.map((role, _ci) => (
                  <div key={role.id} style={{ textAlign: 'center', padding: '0.5rem 0.25rem', borderLeft: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <div style={{ fontSize: '0.55rem', color: '#64748b', fontFamily: 'monospace' }}>
                ...
                return (
                  <div key={cat}>
                    ...
                    <div
                      ...
                      style={{ display: 'grid', gridTemplateColumns: `200px repeat(${roles.length}, 60px)`, gap: 0, cursor: 'pointer', userSelect: 'none', background: 'rgba(255,255,255,0.02)' }}
                    >
                      <div style={{ padding: '0.4rem 0.75rem', fontSize: '0.65rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                        {isCollapsed ? <ChevronRight size={12} /> : <ChevronDown size={12} />}
                        {cat}
                        <span style={{ fontSize: '0.55rem', color: '#64748b', fontWeight: 600 }}>({perms.length})</span>
                      </div>
                      {roles.map((role) => {
                        const catPerms = perms;
                        const activeCount = catPerms.filter(p => role.permissions.includes(p)).length;
                        const allHave = activeCount === catPerms.length;
                        const noneHave = activeCount === 0;
                        return (
                          <div
                            key={role.id}
                            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', borderLeft: '1px solid rgba(255,255,255,0.05)', padding: '0.25rem' }}
                          >
                            <button
                              onClick={(e) => { e.stopPropagation(); toggleAllInCategory(cat, role); }}
                              style={{ width: 18, height: 18, borderRadius: 4, border: `1px solid ${allHave ? '#10b981' : noneHave ? 'rgba(255,255,255,0.15)' : '#f59e0b'}`, background: allHave ? '#10b98120' : noneHave ? 'transparent' : '#f59e0b20', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', padding: 0 }}
                              title={allHave ? 'Remove all' : noneHave ? 'Grant all' : `Grant ${activeCount}/${catPerms.length}`}
                            >
                              {allHave && <Check size={10} color="#10b981" />}
                              {!allHave && !noneHave && <span style={{ fontSize: '0.45rem', fontWeight: 800, color: '#f59e0b' }}>{activeCount}</span>}
                            </button>
                          </div>
                        );
                      })}
                    ...
                    {!isCollapsed && perms.map((perm) => {
                      return (
                        <div
                          key={perm}
                          style={{ display: 'grid', gridTemplateColumns: `200px repeat(${roles.length}, 60px)`, gap: 0 }}
                        >
                          <div style={{ padding: '0.35rem 0.75rem', fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: '0.5rem', borderLeft: `3px solid ${permColor(perm)}` }}>
                            <span style={{ color: '#e2e8f0', fontWeight: 600 }}>{perm.split(':')[1]}</span>
                            <span style={{ fontSize: '0.55rem', color: '#64748b', fontFamily: 'monospace' }}>{perm.split(':')[0]}</span>
                          </div>
                          {roles.map((role, ci) => {
                            const has = role.permissions.includes(perm);
                            const row = ALL_PERMISSIONS.indexOf(perm);
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  cursor: 'pointer',
                                  background: selected ? 'rgba(168,85,247,0.2)' : has ? `${permColor(perm)}10` : 'transparent',
                              ...
                              >
                                <div style={{
                                  ...
                                  background: has ? `${permColor(perm)}25` : 'transparent',
                                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                                  transition: 'all 0.15s',
                                }}>
                                  {has && <Check size={12} color={permColor(perm)} />}
                                </div>
                              </div>
                            );
                          })}
            ...
            {/* Coverage bar */}
            <div style={{ padding: '0.75rem 1.5rem', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: '1.5rem', alignItems: 'center', fontSize: '0.7rem', color: '#94a3b8', background: 'rgba(0,0,0,0.15)' }}>
              <span style={{ fontWeight: 700 }}>Coverage:</span>
              {Object.entries(PERM_CATEGORIES).slice(0, 6).map(([cat, perms]) => {
                const total = perms.length * roles.length;
                const active = perms.reduce((sum, p) => sum + roles.filter(r => r.permissions.includes(p)).length, 0);
                ...
                return (
                  <div key={cat} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                    ...
                    <div style={{ width: 40, height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.1)', overflow: 'hidden' }}>
                      <div style={{ width: `${pct}%`, height: '100%', background: pct === 100 ? '#10b981' : pct > 50 ? '#f59e0b' : '#ef4444', borderRadius: 2, transition: 'width 0.3s' }} />
                    </div>
                    <span style={{ fontSize: '0.6rem', fontFamily: 'monospace', color: pct === 100 ? '#10b981' : '#f59e0b' }}>{pct}%</span>
                  </div>
                );
              })}
            </div>
