  ...
  return (
    <div style={{ padding: '1.5rem', height: '100%', overflow: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
        ...
        <div>
          <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 600 }}>{t('session_bindings.title')}</h2>
          <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '0.85rem' }}>{t('session_bindings.subtitle')}</p>
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{bindings.length} {t('session_bindings.count_suffix')}</span>
          <button
            onClick={handleRefresh}
            title={t('session_bindings.refresh')}
            style={{
              background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '0.35rem 0.6rem',
      ...
      {bindings.length === 0 ? (
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {ENRICHED_HEADERS.map(h => (
                  <th key={h} style={{ padding: '0.5rem 0.75rem', textAlign: 'left', whiteSpace: 'nowrap', color: 'var(--text-muted)', fontWeight: 500 }}>
            ...
            <tbody>
              {bindings.map(b => (
                <tr key={`${b.sessionId}::${b.participantId ?? ''}`} style={{ borderBottom: '1px solid var(--border)' }}>
