...
const statRow = (label: string, a: string, b: string, higherBetter = true) => {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'center', padding: '0.4rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      <div style={{ textAlign: 'right', fontSize: '0.85rem', fontWeight: 700, color: aWins ? '#10b981' : '#e2e8f0' }}>{a}</div>
      <div style={{ fontSize: '0.65rem', color: '#64748b', textAlign: 'center', minWidth: 80 }}>{label}</div>
      <div style={{ fontSize: '0.85rem', fontWeight: 700, color: bWins ? '#10b981' : '#e2e8f0' }}>{b}</div>
    </div>
  );
};
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
        style={{ background: '#1e293b', borderRadius: 16, border: '1px solid rgba(255,255,255,0.1)', width: '100%', maxWidth: 700, maxHeight: '80vh', overflow: 'auto' }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: '0.9rem', color: '#e2e8f0' }}>Agent Comparison</span>
          </div>
          <button onClick={onClose} style={{ padding: 6, borderRadius: 8, background: 'rgba(255,255,255,0.05)', border: 'none', color: '#94a3b8', cursor: 'pointer' }}>
            <X size={16} />
          </button>
        </div>

        {/* Agent Names */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, padding: '1rem 1.5rem', alignItems: 'center' }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontWeight: 800, fontSize: '1rem', color: '#60a5fa' }}>{a.name}</div>
            <div style={{ fontSize: '0.7rem', color: '#64748b' }}>{a.role}</div>
          </div>
          <div style={{ fontSize: '0.7rem', color: '#475569', fontWeight: 700 }}>VS</div>
          <div>
            <div style={{ fontWeight: 800, fontSize: '1rem', color: '#a855f7' }}>{b.name}</div>
        ...
        <div style={{ padding: '0 1.5rem 1rem' }}>
          <div style={{ fontSize: '0.65rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>Performance</div>
          {statRow('Calls', a.stats.calls.toLocaleString(), b.stats.calls.toLocaleString())}
          {statRow('Tokens', a.stats.tokens > 1000 ? `${(a.stats.tokens / 1000).toFixed(1)}K` : String(a.stats.tokens), b.stats.tokens > 1000 ? `${(b.stats.tokens / 1000).toFixed(1)}K` : String(b.stats.tokens))}
          {statRow('Latency', `${a.stats.latency}ms`, `${b.stats.latency}ms`, false)}
          {statRow('Success', `${successA}%`, `${successB}%`)}
          {statRow('Errors', String(a.stats.errors || 0), String(b.stats.errors || 0), false)}
        </div>

        {/* Config */}
        <div style={{ padding: '0 1.5rem 1rem' }}>
          <div style={{ fontSize: '0.65rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>Configuration</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: '0.5rem' }}>
            <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>
              <span style={{ color: '#64748b' }}>Provider: </span>{a.providerId}
            </div>
            <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>
              <span style={{ color: '#64748b' }}>Provider: </span>{b.providerId}
            </div>
          </div>
        </div>
