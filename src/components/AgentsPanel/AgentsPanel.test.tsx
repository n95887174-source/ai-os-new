...
vi.mock('../../kernel/instances', () => ({
  ...
  keyService: {
    ...
    getRoutingPolicy: vi.fn(() => ({ globalSLAMode: 'BALANCED' as const, latencyThreshold: 1500 })),
  },
  settingsService: {
...
describe('AgentsPanel', () => {
  ...
  it('renders quick start template buttons', async () => {
    render(<AgentsPanel />);
    expect(await screen.findByText('agents.quick_start_label')).toBeDefined();
  ...
  it('renders search input with aria-label', async () => {
    render(<AgentsPanel />);
    expect(await screen.findByLabelText('agents.search_aria')).toBeDefined();
  });

  it('filters agents by search query', async () => {
    render(<AgentsPanel />);
    await waitForAgentCards();
    const search = screen.getByLabelText('agents.search_aria');
  ...
  it('filters agents by status filter', async () => {
    render(<AgentsPanel />);
    await waitForAgentCards();
    const pausedBtn = screen.getByText('agents.filter_paused');
  ...
  it('has aria-pressed on filter buttons', async () => {
    render(<AgentsPanel />);
    await waitForAgentCards();
    const allBtn = screen.getByText('agents.filter_all');
    expect(allBtn.getAttribute('aria-pressed')).toBe('true');
    const activeBtn = screen.getByText('agents.filter_active');
  ...
  it('opens modal on card click', async () => {
    ...
    expect(screen.getByText('agents.tab_config')).toBeDefined();
  });
