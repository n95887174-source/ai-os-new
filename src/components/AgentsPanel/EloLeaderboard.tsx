...
const MiniLineChart: React.FC<{ data: number[]; width?: number; height?: number }> = ({ data, width = 200, height = 50 }) => {
  if (data.length < 2) return <div style={{ width, height, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', color: '#64748b' }}>No data</div>;
...
export const EloLeaderboard: React.FC = () => {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.01)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.05)', padding: '1.25rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '1rem' }}>
        <Trophy size={18} color="#f59e0b" />
        <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#f8fafc' }}>ELO Leaderboard</span>
        <span style={{ fontSize: '0.65rem', color: '#64748b', marginLeft: 'auto' }}>{entries.length} agents ranked</span>
      </div>
      ...
      {entries.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {/* Header */}
          <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr 70px 80px 60px 24px', gap: 8, padding: '0 12px 6px', fontSize: '0.6rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            <span>#</span>
            <span>Agent</span>
              ...
              <React.Fragment key={entry.agentId}>
                <div
                  ...
                  style={{
                    ...
                    padding: '8px 12px',
                    borderRadius: 8,
                    background: isTop3 ? `${MEDAL_COLORS[idx]}08` : 'rgba(255,255,255,0.02)',
                ...
                >
                  <span style={{ fontSize: '0.75rem', fontWeight: 700, color: isTop3 ? MEDAL_COLORS[idx] : '#64748b', textAlign: 'center' }}>
                    {isTop3 ? `🏆` : idx + 1}
                  </span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                    <div style={{
                      width: 28, height: 28, borderRadius: '50%',
                      background: isTop3 ? `linear-gradient(135deg, ${MEDAL_COLORS[idx]}40, ${MEDAL_COLORS[idx]}15)` : 'rgba(255,255,255,0.05)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: '0.7rem', fontWeight: 800, color: isTop3 ? MEDAL_COLORS[idx] : '#94a3b8',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}>
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: isTop3 ? MEDAL_COLORS[idx] : '#f8fafc' }}>
                      {entry.elo}
                    </span>
                    <EloTrend history={entry.history} />
                  </div>
                  <div style={{ fontSize: '0.7rem', color: '#94a3b8', textAlign: 'center', fontFamily: 'monospace' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: '#64748b', textAlign: 'right' }}>{entry.matches}</span>
                  <span style={{ color: '#64748b', display: 'flex', justifyContent: 'center' }}>
                    {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                  </span>
                </div>

                {isExpanded && (
                  <div style={{ padding: '12px 16px', background: 'rgba(0,0,0,0.15)', borderRadius: 8, margin: '0 4px' }}>
                    <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#94a3b8', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    ...
                    {expandedHistory.length > 0 && (
                      <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap' }}>
                        {expandedHistory.slice(-6).map((h, i: number) => (
                          <div key={i} style={{ fontSize: '0.6rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: 4 }}>
                            <span style={{
                              width: 6, height: 6, borderRadius: '50%',
                              background: h.result === 'win' ? '#10b981' : h.result === 'loss' ? '#ef4444' : '#f59e0b',
                            }} />
                            <span style={{ color: h.result === 'win' ? '#10b981' : h.result === 'loss' ? '#ef4444' : '#f59e0b', fontWeight: 600 }}>
                              {h.elo}
                            </span>
                            {h.opponent && <span style={{ color: '#475569' }}>vs {h.opponent.slice(0, 8)}</span>}
                          </div>
                        ))}
                      </div>
                    )}
