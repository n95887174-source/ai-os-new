...
const ConfigHistoryView: React.FC = () => {
  ...
  const handleCaptureNow = () => {
    snapshotService.capture('manual', 'manual', `Manual snapshot ${new Date().toLocaleString()}`);
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', height: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <h2 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800, color: '#f8fafc' }}>Config History & Rollback</h2>
          <span style={{ fontSize: '0.7rem', color: '#64748b' }}>({snapshots.length} snapshots)</span>
        </div>
        <button
          onClick={handleCaptureNow}
          style={{
            padding: '0.5rem 1rem', borderRadius: 8, border: '1px solid rgba(245,158,11,0.3)',
      ...
      {restoreError && (
        <div style={{ padding: '0.5rem 1rem', borderRadius: 8, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#fca5a5', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {snapshots.length > 0 ? snapshots.map(s => (
          <div
            ...
            style={{
              ...
              background: restoring === s.id ? 'rgba(245,158,11,0.1)' : 'rgba(0,0,0,0.15)',
              border: `1px solid ${restoring === s.id ? 'rgba(245,158,11,0.3)' : 'rgba(255,255,255,0.03)'}`,
              display: 'flex', alignItems: 'center', gap: '0.75rem',
              transition: 'all 0.2s',
            }}
          >
            <div style={{ padding: '0.4rem', borderRadius: 8, background: 'rgba(245,158,11,0.1)', color: '#f59e0b', display: 'flex', flexShrink: 0 }}>
              <Clock size={16} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: '0.7rem', color: '#64748b', marginTop: '0.15rem', fontFamily: 'monospace' }}>
                {new Date(s.timestamp).toLocaleString()} · {s.traceId} · {s.stepId}
              </div>
              <div style={{ fontSize: '0.65rem', color: '#64748b', marginTop: '0.1rem' }}>
                {Object.keys(s.runtime.kernel.providers || {}).length} providers · {s.runtime.kernel.totalRequests} requests · ${s.runtime.kernel.estimatedCost.toFixed(4)} cost
              </div>
            </div>
            <button
              ...
              style={{
                padding: '0.5rem 0.8rem', borderRadius: 8, border: '1px solid rgba(245,158,11,0.3)',
                background: restoring === s.id ? 'rgba(245,158,11,0.2)' : 'rgba(245,158,11,0.1)',
                color: '#f59e0b', cursor: restoring === s.id ? 'default' : 'pointer',
          ...
          <div style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>
            ...
            <div style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.5rem' }}>No Config History</div>
            <div style={{ fontSize: '0.85rem' }}>Snapshots are captured automatically during topology changes. Click "Snapshot Now" to capture the current state.</div>
          </div>
        )}
      </div>
    </div>
