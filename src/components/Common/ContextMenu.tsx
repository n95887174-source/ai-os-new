          ...
          <React.Fragment key={action.id}>
            ...
            <button
              ...
              style={{
                ...
                color: action.danger ? '#ef4444' : action.disabled ? '#475569' : '#e2e8f0',
                fontSize: '0.8rem',
                fontWeight: 500,
            ...
            >
              {action.icon && <span style={{ width: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{action.icon}</span>}
              <span style={{ flex: 1, textAlign: 'left' }}>{action.label}</span>
              {action.shortcut && <span style={{ fontSize: '0.65rem', color: '#64748b', marginLeft: 12 }}>{action.shortcut}</span>}
            </button>
          </React.Fragment>
        ))}
      </motion.div>
