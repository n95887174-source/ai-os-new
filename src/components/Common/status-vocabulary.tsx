  ...
  return (
    <span
      style={{
        ...
        padding: s.padding, borderRadius: 100, fontSize: s.fontSize,
...
export function ThresholdBar({ pct, height = 4, radius = 3, color }: BarProps) {
  ...
  return (
    <div style={{ width: '100%', height, background: 'rgba(255,255,255,0.05)', borderRadius: radius, overflow: 'hidden' }}>
      <div style={{ width: `${Math.min(100, Math.max(0, pct))}%`, height: '100%', background: barColor, borderRadius: radius }} />
    </div>
  );
}
...
export function TagPill({ tag }: { tag: string }) {
  ...
  return (
    <span style={{ fontSize: '0.6rem', padding: '0.15rem 0.4rem', borderRadius: 4,
...
export function activeToggleStyle(active: boolean, accent = '#3b82f6'): React.CSSProperties {
  return {
    padding: '0.4rem 0.8rem', fontSize: '0.75rem', borderRadius: 8, cursor: 'pointer',
