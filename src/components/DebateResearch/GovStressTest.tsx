      ...
      <div style={{ padding: '0.6rem 1.25rem', display: 'flex', gap: 8, borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
        <div style={{ flex: 1, padding: '0.5rem 0.7rem', borderRadius: 8, background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.15)', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1rem', fontWeight: 700, color: '#60a5fa' }}>{liveSnapshot.livePolicyCount}</div>
          </div>
        </div>
        <div style={{ flex: 1, padding: '0.5rem 0.7rem', borderRadius: 8, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.15)', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1rem', fontWeight: 700, color: '#f59e0b' }}>{liveSnapshot.liveViolationCount}</div>
          </div>
        </div>
        <div style={{ flex: 1, padding: '0.5rem 0.7rem', borderRadius: 8, background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.15)', display: 'flex', alignItems: 'center', gap: 6 }}>
          <Shield size={14} color="#a855f7" />
          <div>
            <div style={{ fontSize: '0.6rem', color: '#64748b', textTransform: 'uppercase' }}>Roles</div>
            <div style={{ fontSize: '1rem', fontWeight: 700, color: '#a855f7' }}>{liveSnapshot.roleCount}</div>
          </div>
        </div>
        <div style={{ flex: 1, padding: '0.5rem 0.7rem', borderRadius: 8, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.15)', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div>
            <div style={{ fontSize: '0.6rem', color: '#64748b', textTransform: 'uppercase' }}>Scenarios</div>
            <div style={{ fontSize: '1rem', fontWeight: 700, color: '#10b981' }}>{SCENARIO_COUNT}</div>
          </div>
        </div>
      </div>
      ...
      {results.length > 0 && (
        <div style={{ padding: '0.6rem 1.25rem', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
          <div style={{ display: 'flex', gap: 4, height: 24 }}>
            {summary.passed > 0 && (
              <div style={{ flex: summary.passed, background: '#10b98125', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #10b98130' }}>
            ...
            {summary.warned > 0 && (
              <div style={{ flex: summary.warned, background: '#f59e0b25', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #f59e0b30' }}>
            ...
            {summary.blocked > 0 && (
              <div style={{ flex: summary.blocked, background: '#ef444425', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #ef444430' }}>
                <span style={{ fontSize: '0.6rem', color: '#f87171', fontWeight: 700 }}>
      ...
      <div style={{ padding: '0.5rem 1.25rem', display: 'flex', gap: 4, flexWrap: 'wrap', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
        <button
          ...
          style={{
            ...
            borderRadius: 5,
            border: categoryFilter === 'all' ? '1px solid rgba(255,255,255,0.15)' : '1px solid transparent',
            background: categoryFilter === 'all' ? 'rgba(255,255,255,0.04)' : 'transparent',
            color: '#94a3b8',
            cursor: 'pointer',
            fontSize: '0.68rem',
            fontWeight: 600,
        ...
        >
          <Filter size={10} /> All{' '}
          {results.length > 0 && <span style={{ color: '#64748b', fontSize: '0.6rem' }}>({results.length})</span>}
        </button>
          ...
          return (
            <button
              ...
              style={{
                padding: '0.25rem 0.5rem',
                borderRadius: 5,
                border: categoryFilter === cat ? `1px solid ${color}40` : '1px solid transparent',
      ...
      <div style={{ padding: '0.5rem 1.25rem', display: 'flex', gap: 6, alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(0,0,0,0.3)', borderRadius: 5, padding: '3px 7px' }}>
          ...
          <input
            ...
            placeholder="Search..."
            style={{ width: 130, background: 'none', border: 'none', outline: 'none', color: '#e2e8f0', fontSize: '0.72rem' }}
          />
          {searchQuery && (
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0.6rem 1.25rem' }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#475569', fontSize: '0.85rem' }}>
            {running ? (
              <>
                <Loader2 size={16} style={{ display: 'block', margin: '0 auto 8px' }} /> Running scenarios...
              </>
            ) : (
              ...
              {r.violatedRules.length > 0 && (
                <div style={{ marginTop: 4, display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                  {r.violatedRules.map((rule, j) => (
                    <span key={j} style={{ fontSize: '0.62rem', padding: '0.1rem 0.35rem', borderRadius: 3, background: 'rgba(239,68,68,0.06)', color: '#ef4444' }}>
                      {rule}
                    </span>
                  ))}
                </div>
