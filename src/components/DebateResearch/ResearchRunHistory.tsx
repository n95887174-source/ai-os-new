...
const ResearchRunHistory: React.FC<ResearchRunHistoryProps> = ({ module, runService }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      <AnimatePresence>
        {runs.map(run => (
          <motion.div key={run.id} initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
            style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
            <div onClick={() => setExpandedId(expandedId === run.id ? null : run.id)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0.6rem 0.75rem', cursor: 'pointer', fontSize: '0.75rem' }}>
              <span style={{ color: statusColor(run.status) }}>{statusIcon(run.status)}</span>
              <span style={{ color: '#e2e8f0', fontWeight: 600, flex: 1 }}>
                {run.module}
                {run.summary && <span style={{ color: '#64748b', fontWeight: 400 }}> — {run.summary.slice(0, 60)}</span>}
              </span>
              <span style={{ color: '#64748b', fontSize: '0.65rem' }}>
                {new Date(run.startedAt).toLocaleString()}
              </span>
              {expandedId === run.id ? <ChevronUp size={14} color="#64748b" /> : <ChevronDown size={14} color="#64748b" />}
            </div>
            <AnimatePresence>
              {expandedId === run.id && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                  style={{ padding: '0 0.75rem 0.75rem', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginTop: '0.5rem' }}>
                    <div><strong>Status:</strong> <span style={{ color: statusColor(run.status) }}>{run.status}</span></div>
                    {run.completedAt && <div><strong>Duration:</strong> {((run.completedAt - run.startedAt) / 1000).toFixed(1)}s</div>}
                    {run.error && <div style={{ color: '#ef4444' }}><strong>Error:</strong> {run.error}</div>}
                    {run.findings && run.findings.length > 0 && (
                      <div style={{ marginTop: '0.5rem' }}>
                        <strong>Findings:</strong>
                        <ul style={{ margin: '0.25rem 0 0', paddingLeft: '1rem' }}>
                          {run.findings.map((f, i) => <li key={i}>{f}</li>)}
                        </ul>
                      </div>
                    )}
                    {run.summary && <div style={{ marginTop: '0.25rem' }}><strong>Summary:</strong> {run.summary}</div>}
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
                    <button onClick={(e) => { e.stopPropagation(); handleDelete(run.id); }}
                      style={{ fontSize: '0.65rem', padding: '0.3rem 0.6rem', borderRadius: 6, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#ef4444', cursor: 'pointer' }}>
