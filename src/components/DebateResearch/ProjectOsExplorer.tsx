...
const KEYWORD_HIGHLIGHT: Record<
    string,
...
const ProjectOsExplorer: React.FC = () => {
    ...
    const handleCopyPath = async (path: string) => {
        try {
            ...
            console.warn('[ProjectOsExplorer] copy path error:', e);
        }
        setContextMenu(null);
    };
    ...
    const flattenTree = useCallback(function flattenTreeRec(
        nodes: FileNode[],
        base: FileNode[] = [],
    ): FileNode[] {
        for (const n of nodes) {
            base.push(n);
            if (n.type === 'dir' && n.children) flattenTreeRec(n.children, base);
        }
        return base;
    }, []);
    ...
    const allFiles = useMemo(
        () => flattenTree(tree).filter((n) => n.type === 'file'),
        [tree, flattenTree],
    );
    ...
    function highlightCode(content: string, ext: string): React.ReactNode[] {
        ...
        if (!rules || ext === 'tsx') {
            return content.split('\n').map((line, i) => (
                <span key={`line-${i}`} data-line={i + 1} style={{ display: 'block' }}>
                    {line || ' '}
                </span>
            ));
        }
        return content.split('\n').map((line, i) => {
            ...
            for (const rule of rules) {
                ...
                const re = new RegExp(
                    `\\b(${rule.keywords.join('|').replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})\\b`,
                    'g',
                );
                let m;
            ...
            for (const m of matches) {
                if (m.start > pos)
                    parts.push(<span key={`${i}-${pos}`}>{line.slice(pos, m.start)}</span>);
                parts.push(
                    <span key={`${i}-${m.start}`} style={{ color: m.color }}>
            ...
            return (
                <div key={node.path}>
                    <div
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                                if (isDir) handleToggleDir(node.path);
        ...
        >
            ...
            {!attached ? (
                ...
                >
                    ...
                    >
                        Attach a folder to browse its structure, inspect source code, configs, and
                        documentation for research analysis.
                    </span>
                    <button
                                    ...
                                    <div>
                                        ...
                                        {searchResults.map((path) => (
                                            <div
                                                ...
                                                onKeyDown={(e) => {
                                                    if (e.key === 'Enter') handleSelectFile(path);
                                                }}
                                                role="button"
                                                tabIndex={0}
                                                style={{
                                            ...
                                            >
                                                <span
                                                    style={{
                                                        color:
                                                            EXT_ICONS[getExt(path)]?.color ||
                                                            '#64748b',
                                                    }}
                                                >
                                                    {EXT_ICONS[getExt(path)]?.icon || (
                                                        <File size={11} />
                                                    )}
                                                </span>
                                                <span
                                        ...
                                        <div style={{ flex: 1, overflow: 'auto', padding: '1rem' }}>
                                            ...
                                            >
                                                {[
                                                    ...
                                                    [
                                                        'Extension',
                                                        getExt(selectedPath).toUpperCase() ||
                                                            '(none)',
                                                    ],
                                                    [
                                                        'Size',
                                            ...
                                            >
                                                ...
                                                <button
                                                    onClick={() =>
                                                        handleCreateHypothesis(selectedPath)
                                                    }
                                                    style={{
                                                        padding: '0.4rem 0.8rem',
                                                        borderRadius: 5,
                                                ...
                                                >
                                                    <Lightbulb size={11} /> Hypothesis from File
                                                </button>
                                            </div>
                                        </div>
                                    ) : (
            ...
            {contextMenu && (
                ...
                >
                    ...
                    <div
                        onClick={() => {
                            handleCreateHypothesis(contextMenu.path);
                        }}
                        style={{
                            padding: '0.4rem 0.7rem',
                            borderRadius: 4,
