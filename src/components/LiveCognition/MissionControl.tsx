...
const MissionControl: React.FC = () => {
  ...
  return (
    <div style={{ height: '100%', display: 'grid', gridTemplateColumns: '1fr 340px', gap: '1rem', overflow: 'hidden' }}>
      
      {/* Primary Intelligence Feed */}
      <div style={{ display: 'grid', gridTemplateRows: '1fr 300px', gap: '1rem', overflow: 'hidden' }}>
        <div className="glass-panel" style={{ position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: 15, left: 15, zIndex: 10, display: 'flex', gap: 10 }}>
            <div style={{ padding: '0.4rem 0.8rem', background: 'rgba(59,130,246,0.1)', borderRadius: 8, fontSize: '0.7rem', fontWeight: 800, color: '#3b82f6', border: '1px solid rgba(59,130,246,0.2)' }}>
        ...
        <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '0.8rem' }}>
          System monitoring idle — view detailed knowledge graph in <strong style={{ marginLeft: 4 }}>Knowledge</strong> panel
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div className="glass-panel" style={{ padding: '1.5rem', background: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.2)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            ...
            <span style={{ fontSize: '0.65rem', fontWeight: 800, color: '#f59e0b', background: 'rgba(245,158,11,0.1)', padding: '0.2rem 0.5rem', borderRadius: 4 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {suggestions.slice(0, 3).map(s => (
              <div key={s.id} style={{ padding: '1rem', background: 'rgba(0,0,0,0.2)', borderRadius: 10, border: '1px solid var(--border)' }}>
                <div style={{ fontSize: '0.8rem', fontWeight: 700, marginBottom: '0.25rem' }}>{s.title}</div>
                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: '0.75rem' }}>{s.description}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.6rem', fontWeight: 800, color: s.impact === 'high' ? '#ef4444' : '#f59e0b' }}>IMPACT: {s.impact.toUpperCase()}</span>
                  <button onClick={() => handleExecuteFix(s.id)} className="btn-primary" style={{ padding: '0.3rem 0.6rem', fontSize: '0.6rem' }}>Execute Fix</button>
                </div>
              </div>
            ))}
            {suggestions.length === 0 && (
              <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'center', padding: '2rem' }}>
