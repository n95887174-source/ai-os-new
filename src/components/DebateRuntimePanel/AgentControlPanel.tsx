...
const OVERRIDE_PRESETS: Array<{ id: string; label: string; description: string; config: Record<string, unknown> }> = [
  { id: 'strengthen_critic', label: 'Strengthen Critic', description: 'Lower temperature, require evidence', config: { temperature: 0.3, tools: ['critique', 'fact-check'] as unknown[] } },
  { id: 'lower_creativity', label: 'Lower Creativity', description: 'Temperature 0.2, focused reasoning', config: { temperature: 0.2 } },
  { id: 'require_sources', label: 'Require Sources', description: 'Push for citations and data', config: { temperature: 0.4, tools: ['web-search', 'citation'] as unknown[] } },
  { id: 'creative_brainstorm', label: 'Creative Brainstorm', description: 'High temperature, speculative', config: { temperature: 0.9 } },
  { id: 'balanced', label: 'Balanced', description: 'Reset to default parameters', config: { temperature: 0.7, tools: [] } },
];
...
export const AgentControlPanel: React.FC<AgentControlPanelProps> = ({ session }) => {
  ...
  const handleInject = useCallback(async (agentId: string) => {
    ...
    for (const agent of agents) {
      ...
      if (preset) {
        ...
        if (preset.config.temperature !== undefined) {
          setLocalTemps(prev => ({ ...prev, [agent.id]: preset.config.temperature as number }));
        }
      }
    }
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {OVERRIDE_PRESETS.map(preset => (
            <button
              ...
              style={{
                padding: '0.35rem 0.7rem', borderRadius: 6, border: '1px solid rgba(139,92,246,0.25)',
                background: presetApplied === preset.id ? 'rgba(16,185,129,0.15)' : 'rgba(139,92,246,0.08)',
            ...
            >
              {presetApplied === preset.id ? <Check size={12} /> : <Brain size={12} />}
              {preset.label}
            </button>
          ))}
        </div>
        ...
        {agents.map(agent => {
          ...
          return (
            <div key={agent.id} style={{
              ...
              background: isPaused ? 'rgba(100,116,139,0.06)' : 'rgba(255,255,255,0.03)',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <span style={{
                  ...
                  background: isPaused ? '#64748b' : agent.status === 'busy' ? '#22c55e' : agent.status === 'thinking' ? '#3b82f6' : '#64748b',
                  flexShrink: 0,
                }} />
                <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#e2e8f0', flex: 1 }}>
                  {agent.name}
                </span>
                <span style={{ fontSize: '0.65rem', color: '#64748b', padding: '2px 6px', borderRadius: 4, background: 'rgba(255,255,255,0.04)' }}>
                  {agent.role}
                </span>
                <button
                  onClick={() => handleToggle(agent.id)}
                  style={{
                    padding: '4px 8px', borderRadius: 6, border: `1px solid ${isPaused ? 'rgba(34,197,94,0.3)' : 'rgba(100,116,139,0.3)'}`,
                    background: isPaused ? 'rgba(34,197,94,0.1)' : 'rgba(100,116,139,0.1)',
                    color: isPaused ? '#22c55e' : '#64748b',
                    cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.7rem', fontWeight: 600,
                  }}
                >
                ...
                <button
                  ...
                  style={{
                    padding: '4px', borderRadius: 6, border: '1px solid rgba(100,116,139,0.2)',
                    background: 'transparent', color: '#64748b', cursor: restarting === agent.id ? 'not-allowed' : 'pointer',
                    display: 'flex', alignItems: 'center', opacity: restarting === agent.id ? 0.5 : 1,
                  }}
                >
                  {restarting === agent.id ? <Loader2 size={12} className="animate-spin" /> : <RotateCw size={12} />}
                </button>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: 8 }}>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: '#64748b', marginBottom: 2 }}>
                    <span><Thermometer size={10} /> Temperature</span>
                    <span style={{ fontWeight: 600, color: '#e2e8f0' }}>{temp.toFixed(1)}</span>
                  </div>
                  <input
                    type="range"
                    min={0}
                    ...
                    step={0.1}
                    value={temp}
                    onChange={e => handleTempChange(agent.id, parseFloat(e.target.value))}
                    style={{ width: '100%', height: 4, cursor: 'pointer' }}
                  />
                </div>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: '#64748b', marginBottom: 2 }}>
                    <span><FileText size={10} /> Max Tokens</span>
                    <span style={{ fontWeight: 600, color: '#e2e8f0' }}>{maxTokens}</span>
                  </div>
                  <input
                    type="range"
                    min={100}
                    ...
                    step={100}
                    value={maxTokens}
                    onChange={e => handleMaxTokensChange(agent.id, parseInt(e.target.value))}
                    style={{ width: '100%', height: 4, cursor: 'pointer' }}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <input
                  ...
                  onChange={e => setInjectText(prev => ({ ...prev, [agent.id]: e.target.value }))}
                  onKeyDown={e => { if (e.key === 'Enter' && (injectText[agent.id] ?? '').trim() && !injecting[agent.id]) handleInject(agent.id); }}
                  placeholder={`Message to ${agent.name}...`}
                  style={{
                    flex: 1, padding: '0.3rem 0.5rem', borderRadius: 6, border: '1px solid rgba(100,116,139,0.2)',
                    background: 'rgba(15,23,42,0.4)', color: '#e2e8f0', fontSize: '0.75rem', outline: 'none',
                  }}
                />
                <button
                  ...
                  style={{
                    padding: '0.3rem 0.6rem', borderRadius: 6, border: '1px solid rgba(59,130,246,0.3)',
                    background: (injectText[agent.id]?.trim() && !injecting[agent.id]) ? 'rgba(59,130,246,0.15)' : 'rgba(100,116,139,0.1)',
                    color: (injectText[agent.id]?.trim() && !injecting[agent.id]) ? '#60a5fa' : '#64748b',
                    cursor: (injectText[agent.id]?.trim() && !injecting[agent.id]) ? 'pointer' : 'not-allowed',
                    display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.7rem', fontWeight: 600,
                  }}
                >
                  {injecting[agent.id] ? <Loader2 size={12} className="animate-spin" /> : <Send size={12} />}
                  Send
                </button>
              </div>
            </div>
