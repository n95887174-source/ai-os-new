...
const BulkImportStep: React.FC<BulkImportStepProps> = ({ bulkInput, setBulkInput, error, loading, bulkReport, bulkProgress, onBack, onImport, onClose }) => {
  ...
  return (
    <>
      ...
      <div className="modal-actions">
        <button type="button" onClick={onBack} className="btn-secondary" style={{ padding: '0.75rem 1.25rem' }} disabled={loading}>
          {t('add_key.back')}
        </button>
        <button type="button" onClick={onImport} className="btn-primary" style={{ flex: 1, padding: '0.75rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }} disabled={loading}>
