...
export default function OpenRouterKeyTable() {
    ...
    const load = useCallback(async () => {
        ...
        setKeys(all.filter((k) => k.provider.toLowerCase() === 'openrouter'));
    }, []);
    ...
    const handleTest = (apiKey: ApiKey) => {
        ...
        const reqId = `openrouter-test-${apiKey.id}-${crypto.randomUUID().slice(0, 6)}`;
    ...
    const allModels = [
        ...new Set([
            ...(PROVIDER_DEFAULT_MODELS[providerKey] ? [PROVIDER_DEFAULT_MODELS[providerKey]] : []),
            ...(PROVIDER_PREFERRED_MODELS[providerKey] || []),
        ]),
    ];
    ...
    return (
        <div>
            ...
            >
                <span style={{ color: OPENROUTER }}>◈</span> OpenRouter Keys
            </h3>
            {keys.length === 0 ? (
                <div
                    style={{
                ...
                >
                    No OpenRouter keys configured. Add one in Providers.
                </div>
            ) : (
                                        ...
                                        <button
                                            ...
                                            style={{
                                                ...
                                                background: test?.loading
                                                    ...
                                                    : `${OPENROUTER}22`,
                                                color: test?.loading ? '#6b7280' : OPENROUTER,
                                            }}
                                        >
                                            {test?.loading ? (
