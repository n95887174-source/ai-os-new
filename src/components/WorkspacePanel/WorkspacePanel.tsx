      ...
      return (
        <div key={node.path}>
          <div
            ...
            style={{
              display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px', paddingLeft,
          ...
          >
            ...
            {isDir
              ? (isExpanded ? <FolderOpen size={14} color="#a855f7" /> : <FolderClosed size={14} color="#64748b" />)
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '1.5rem 1.5rem 0.75rem', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: '1rem', fontWeight: 700, color: '#f8fafc' }}>Workspace</span>
        </div>
        {attached && (
          <button
            onClick={handleDetach}
            style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', padding: 4, borderRadius: 6, display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.78rem' }}
      ...
      {!attached ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', padding: '3rem', color: '#64748b' }}>
          ...
          <span style={{ fontSize: '0.95rem', fontWeight: 600, textAlign: 'center' }}>No workspace attached</span>
          <span style={{ fontSize: '0.82rem', color: '#475569', textAlign: 'center', maxWidth: 300 }}>
            Attach a local folder for agents to browse and read files during chats and debates.
          </span>
          <button
            onClick={handleAttach}
            className="btn-primary"
            style={{ padding: '0.75rem 1.5rem', borderRadius: 12, fontSize: '0.9rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, marginTop: '0.5rem' }}
          >
        ...
        <>
          ...
          <div style={{ padding: '0.75rem 1.25rem', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
            ...
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#10b981', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {workspaceName}
            </span>
            <button
              onClick={refreshTree}
              style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', padding: 2, fontSize: '0.75rem' }}
              title="Refresh"
          ...
          <div style={{ padding: '0.5rem 1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(0,0,0,0.3)', borderRadius: 8, padding: '6px 10px', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <input
                ...
                style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: '#e2e8f0', fontSize: '0.82rem' }}
              />
              {searching && <Loader2 size={12} className="spinning" />}
              {searchQuery && (
                <button onClick={() => handleSearch('')} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', padding: 0 }}>
                  <X size={12} />
                </button>
              )}
            </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 0.5rem 0.5rem' }}>
            {searchQuery ? (
                ...
                <div>
                  ...
                  {searchResults.map(path => (
                    <div
                      ...
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleSelectFile(path); } }}
                      role="button"
                      tabIndex={0}
                      style={{
                        display: 'flex', alignItems: 'center', gap: 6, padding: '4px 12px', cursor: 'pointer', borderRadius: 6, fontSize: '0.82rem',
                        color: selectedPath === path ? '#a855f7' : '#94a3b8',
                        background: selectedPath === path ? 'rgba(168,85,247,0.12)' : 'transparent',
                      }}
                    >
                      <File size={14} />
              ...
              <div style={{ padding: '2rem', textAlign: 'center', color: '#475569', fontSize: '0.82rem' }}>Empty directory</div>
            ) : (
          ...
          {/* Preview panel */}
          {selectedPath && (
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', maxHeight: '45%' }}>
              <div style={{ padding: '0.5rem 1rem', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                <File size={12} color="#64748b" />
                <span style={{ fontSize: '0.78rem', color: '#94a3b8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{selectedPath}</span>
                <button onClick={() => { setSelectedPath(null); setPreviewContent(null); setPreviewError(null); }} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', padding: 0 }}>
                  <X size={14} />
                </button>
              </div>
              <div style={{ flex: 1, overflow: 'auto', padding: '0.75rem 1rem' }}>
                {previewLoading ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#64748b', fontSize: '0.82rem' }}>
                    <Loader2 size={14} className="spinning" /> Loading...
                  </div>
                ) : previewError ? (
                  <div style={{ color: '#ef4444', fontSize: '0.82rem' }}>{previewError}</div>
                ) : (
                  <pre style={{ margin: 0, fontSize: '0.78rem', lineHeight: 1.5, color: '#cbd5e1', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
                    {previewContent}
                  </pre>
                )}
              </div>
