...
const DiagnosticPanel: React.FC = () => {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1400, margin: '0 auto', height: '100%', overflowY: 'auto' }}>
      ...
      <p style={{ margin: '0 0 1.5rem 0', fontSize: '0.8rem', color: '#64748b' }}>System-wide health diagnostics, active issues, and diagnostic history</p>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12, marginBottom: 20 }}>
        <div style={{ ...CARD, borderLeft: `3px solid ${healthColor}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            <Shield size={16} color={healthColor} />
            <span style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b' }}>System Health</span>
          </div>
          <div style={{ fontSize: '1.3rem', fontWeight: 700, color: healthColor }}>{diagnostic?.health || '—'}</div>
          {diagnostic && <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>Score: {(diagnostic.score * 100).toFixed(0)}%</div>}
        </div>

        <div style={CARD}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            <Activity size={16} color="#3b82f6" />
            <span style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b' }}>Counts</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, fontSize: '0.75rem' }}>
            <span style={{ color: '#64748b' }}>Sessions</span><span style={{ color: '#cbd5e1' }}>{diagnostic?.sessionCount ?? 0}</span>
            <span style={{ color: '#64748b' }}>Providers</span><span style={{ color: '#cbd5e1' }}>{diagnostic?.providerCount ?? 0}</span>
            <span style={{ color: '#64748b' }}>Active Issues</span><span style={{ color: (diagnostic?.activeIssueCount ?? 0) > 0 ? '#ef4444' : '#22c55e' }}>{diagnostic?.activeIssueCount ?? 0}</span>
          </div>
        </div>
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b' }}>Actions</span>
          </div>
          <button onClick={handleRunDiagnostic} disabled={running}
            style={{ padding: '0.5rem 1rem', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 600, background: 'rgba(16,185,129,0.2)', color: '#10b981', display: 'flex', alignItems: 'center', gap: 6, width: '100%', justifyContent: 'center', opacity: running ? 0.5 : 1 }}>
            {running ? <Loader2 size={14} className="spin" /> : <RefreshCw size={14} />} Run Diagnostic
          </button>
        </div>
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748b' }}>Session Lookup</span>
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            <input value={sessionId} onChange={e => setSessionId(e.target.value)} placeholder="Session ID..." style={{
              flex: 1, padding: '0.4rem 0.6rem', borderRadius: 6, background: 'rgba(0,0,0,0.3)',
              border: '1px solid rgba(148,163,184,0.15)', color: '#e2e8f0', fontSize: '0.75rem', outline: 'none',
            }} />
            <button onClick={handleLookupSession} style={{ padding: '0.4rem 0.6rem', borderRadius: 6, border: 'none', cursor: 'pointer', background: 'rgba(168,85,247,0.2)', color: '#a855f7' }}>
              <Search size={14} />
            </button>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {issues.map((issue, i) => {
              ...
              return (
                <div key={i} style={{ ...CARD, borderLeft: `3px solid ${sc.text}`, padding: '0.6rem 0.75rem', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ ...BADGE, background: sc.bg, color: sc.text }}>{issue.severity}</span>
                  <span style={{ ...BADGE, background: 'rgba(139,92,246,0.1)', color: '#a78bfa' }}>{issue.type}</span>
                  <span style={{ fontSize: '0.75rem', color: '#cbd5e1', flex: 1 }}>{issue.message}</span>
                  <span style={{ fontSize: '0.65rem', color: '#64748b' }}>{new Date(issue.timestamp).toLocaleTimeString()}</span>
                </div>
              );
            })}
          </div>
      ...
      {issues.length === 0 && (
        <div style={{ ...CARD, marginBottom: 20, textAlign: 'center', padding: 24, borderColor: 'rgba(34,197,94,0.2)' }}>
          <CheckCircle2 size={24} color="#22c55e" style={{ marginBottom: 8 }} />
          <div style={{ fontSize: '0.85rem', color: '#22c55e', fontWeight: 600 }}>All systems operational</div>
          <div style={{ fontSize: '0.75rem', color: '#64748b' }}>No active issues detected</div>
        </div>
      )}
      ...
      <ModuleInfo moduleKey="diagnostics" relatedModules={['health', 'debate_runtime', 'pressure_map']} />
    </div>
  );
};
