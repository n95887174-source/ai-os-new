    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: '#94a3b8', margin: 0, fontSize: '0.85rem' }}>{t('cache.subtitle')}</p>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '0.3rem', background: 'rgba(0,0,0,0.3)', padding: '0.3rem', borderRadius: 10, border: '1px solid rgba(255,255,255,0.05)', alignItems: 'center' }}>
            <input
              ...
              style={{ background: 'transparent', border: 'none', color: '#e2e8f0', padding: '0.4rem 0.5rem', fontSize: '0.8rem', outline: 'none', width: 140 }}
            />
          </div>
          <button
            onClick={handleClear}
            disabled={clearing}
            style={{ padding: '0.5rem 0.9rem', borderRadius: 8, border: 'none', background: '#ef4444', color: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: 6 }}
          >
            {clearing ? <Loader2 size={16} /> : <Trash2 size={16} />}
            {clearing ? t('cache.clearing') : modelFilter.trim() ? t('cache.invalidate_model') : t('cache.clear_all')}
          </button>
          <button
            onClick={handleRefresh}
            style={{ padding: '0.5rem 0.75rem', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#94a3b8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
            <RefreshCw size={16} />
          </button>
        </div>
        ...
        {statCards.map(stat => (
          ...
          >
            ...
            <div style={{ fontSize: '2rem', fontWeight: 800, color: '#f8fafc', marginTop: '0.5rem' }}>
              {stat.value}{stat.suffix && <span style={{ fontSize: '0.85rem', fontWeight: 400, color: '#94a3b8', marginLeft: 4 }}>{stat.suffix}</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '1.5rem', borderRadius: 16, border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.2)' }}>
        <h3 style={{ fontSize: '1rem', fontWeight: 700, color: '#f1f5f9', margin: '0 0 1rem' }}>{t('cache.config_title')}</h3>
        ...
        <div style={{ marginTop: '0.75rem', padding: '0.5rem 0.75rem', borderRadius: 8, background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.15)' }}>
          <div style={{ fontSize: '0.75rem', color: '#93c5fd', fontWeight: 500 }}>{t('cache.tip')}</div>
        </div>
      </div>
